...
- **Database is the spec.** Status enums, column shapes — probe before assuming. See [Probing technique](docs/brain/README.md#probing-technique). PM-flow code reads `public.specs` + `public.spec_phases` via `getSpec` / `listSpecs` (`src/lib/specs-table.ts`) — never `docs/brain/specs/*.md`. Enforced by `npm run check:pm-md-reads` (`scripts/_check-pm-md-reads.ts`); call graph at [recipes/pm-flow-data-sources.md](docs/brain/recipes/pm-flow-data-sources.md). See [retire-md-reads-from-pm-flow](docs/brain/specs/retire-md-reads-from-pm-flow.md).
