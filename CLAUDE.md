...
- **Raw `.from(...)` with no SDK → STOP.** If you are about to read/write a table with raw `.from(...)`/SQL and no SDK exists, STOP and propose an SDK to Dylan rather than hand-rolling the query — a wrong column silently reads as empty (a workspace with 82 competitors read as 0 rows because a raw probe selected a non-existent `name` column). Existing table chokepoints: [specs-table](docs/brain/libraries/specs-table.md), goals-table, [competitors](docs/brain/libraries/competitors.md), ticket-analyses, sonnet-prompts, [policies](docs/brain/libraries/policies.md) — each enforced by its own `scripts/_check-*-sdk-compliance.ts` in `predeploy`. The **policies** SDK additionally enforces the two-halves rule ([[docs/brain/tables/policies.md]] § Two halves): `internal_summary` + `rules` are AUTHORITATIVE (what the AI obeys); `customer_summary` is DERIVED (the published rendering) and neither may contradict the other — a build-time contradiction check ([scripts/_check-policy-contradictions.ts](scripts/_check-policy-contradictions.ts)) fails `predeploy` on drift. The 2026-08-02 refuse-delivery incident is the ground-truth case.
...
- **Ship-time backfill-must-be-tracked convention.** A one-time data backfill ships as an idempotent `scripts/_backfill-*.ts` (auto-ledgered on the post-merge hook by [[docs/brain/libraries/ship-time-backfill-detector.md]] `detectAndEscalateShipTimeBackfills` — writes a `pending` row to [[docs/brain/tables/data_op_runs.md]], escalates any un-run row to the CEO inbox, and drained on the box by `executeShipTimeBackfillsForSpec` in `src/lib/ship-time-backfill-executor.ts` — flips the row to `ran`/`failed` with a compare-and-set) OR an idempotent SQL migration (auto-applied by [[docs/brain/libraries/control-tower/migration-drift.md]] `applyMergedMigrations`) — never an untracked script that only a human can remember to run. Ship-time data backfills go through this rail or a migration; nothing else.
...
- **A node without a switch + heartbeat + owner is incomplete (north-star hard rule).** Every new agent-kind, cron, reactive Inngest fn, inline agent, or tool executor MUST ship in the same PR with the trio: (1) an OWNER in [[docs/brain/libraries/control-tower-node-registry.md]], (2) a [[docs/brain/tables/kill_switches.md]] ancestry (its own row or an ancestor's), and (3) an end-of-run heartbeat via one of `emitCronHeartbeat` / `emitAgentHeartbeat` / `emitInlineAgentHeartbeat` / `emitReactiveHeartbeat` / `emitLoopHeartbeat` from [`src/lib/control-tower/heartbeat.ts`](src/lib/control-tower/heartbeat.ts). Same class as "code without a brain page is incomplete" and serves the CEO's supervisable-autonomy north star — an unowned node has no supervisor, an unswitched one can't be stopped, an unbeat one can't be seen. See [operational-rules § Node completeness](docs/brain/operational-rules.md#node-completeness--a-node-without-a-switch--heartbeat--owner-is-incomplete-hard-rule).
