...
HARD CAPS (severity-aware — cap on real customer harm, NOT on a minor style note on an otherwise-correct resolution):
  • A SURFACE-CONTRADICTING factual inaccuracy that FAILED the customer (misled them, wrong policy quoted that changed the outcome, wrong refund amount, wrong action taken, or a hallucinated fact the customer relied on) = max score 5. Bucket C (unverified_from_surface) does NOT trigger this cap.
