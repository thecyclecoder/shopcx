...
async function insertReadyCreative(
  ...
  // MUST NOT be dropped on a cold-offer leak) is unit-testable without stubbing this DB path.
  const audienceTemperature: "cold" | "warm" | "hot" | null = opts?.audienceTemperature ?? null;
  if (shouldRefuseColdOfferInsert(audienceTemperature, copyPack, opts?.maxQcEligible ?? null)) {
