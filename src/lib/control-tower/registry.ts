...
export const MONITORED_LOOPS: MonitoredLoop[] = [
  ...
  { id: "agent:spec-review", kind: "agent-kind", owner: "platform", agentKind: "spec-review", label: "Agent — spec review", description: "The quality gate ahead of the build pipeline: the moment a spec enters In Review (newly authored, or flagged back by anyone who spots an issue), Vale reviews it against the authoring guidelines read-only → fixes a malformed spec in place (mangled phases / missing owner+parent / blockers / DB-companion) → routes it to Planned or Deferred. An in_review spec can NEVER build until Vale clears it. Reactive on the in_review transition + a cron backstop. Vale is watched too.", expectedCadence: "on demand", stuckThresholdMs: 60 * MIN },
