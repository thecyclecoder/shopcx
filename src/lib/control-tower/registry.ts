...
export const MONITORED_LOOPS: MonitoredLoop[] = [
  ...
  },
  {
    ...
    kind: "reactive",
    owner: "retention",
    label: "ShopCX contract ingest",
    description: "Turns a Shopify-born subscription contract (a PDP checkout with a selling plan) into a ShopCX subscription. If this stops, new subscribers exist on Shopify and nowhere here — no portal, no renewal, no analytics — and Shopify charges nothing on its own.",
    expectedCadence: "event-driven (shopify/subscription-contract-created)",
    livenessWindowMs: 30 * HOUR,
  },
  {
    id: "shopcx-contract-sync",
    kind: "reactive",
    owner: "retention",
    label: "ShopCX contract sync",
    description: "Mirrors a Shopify-side contract edit back onto the subscriptions row. Registered so its kill switch resolves — an UNREGISTERED node resolves to {off:false}, a switch that silently can never fire.",
    expectedCadence: "event-driven (shopify/subscription-contract-updated)",
    livenessWindowMs: 30 * HOUR,
  },
  {
    id: "one-time-charge-cron",
    kind: "cron",
    owner: "retention",
    label: "One-time charges",
    description: "Hourly fan-out of due one-time charges against vaulted Shopify payment methods (public.one_time_charges). Separate from subscription renewals by design — these rows are deliberately NOT in public.subscriptions.",
    expectedCadence: "hourly (20 * * * *)",
    livenessWindowMs: 3 * HOUR,
  },
  {
    id: "one-time-charge-attempt",
    kind: "reactive",
    owner: "retention",
    label: "One-time charge — attempt",
    description: "Per-charge attempt: build a throwaway Shopify contract, bill it once, cancel it. Registered so its kill switch actually resolves — an UNREGISTERED node resolves to {off:false}, i.e. a switch that silently can never fire.",
    expectedCadence: "event-driven (one-time-charge/attempt)",
    livenessWindowMs: 30 * HOUR,
  },
  {
