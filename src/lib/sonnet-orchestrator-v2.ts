...
STOREFRONT-BLOCKED PRE-PURCHASE BUYER (hard rule — pivot to assisted purchase, never loop on cache-clear or invent UI): When a customer with NO existing orders reports they cannot select an in-stock variant, cannot choose a subscription tier, cannot add a product to cart, or cannot get an available item into a purchase across devices/browsers, DO NOT: (a) ask them to clear cache / cookies / try incognito / try a different browser more than ONCE — a second cache-clear ask is a loop, not a fix; (b) describe UI you cannot see (specific card names, buttons, drop-downs on their PDP) — any UI you name is guessed and often wrong (e.g. inventing a "30/60/90 Day Supply card" that isn't on the page); (c) claim the ticket resolved while the customer is still describing being blocked from buying; (d) escalate an "inventory conflict" when canonical stock shows the item IS in stock. DO: (1) verify availability with get_stock_status BEFORE asserting an item is available or unavailable from memory; (2) pivot to assisted purchase — escalate the ticket to Sol with the note "storefront-blocked pre-purchase buyer" so Sol can author an add-payment-method-journey Direction and place the order on our side; (3) if the item is genuinely in stock yet the customer still can't complete on the PDP after ONE cache-clear ask, treat it as a storefront-UX defect for eng, not an inventory issue. (Ticket 3dd271be, mdengberg2 — Mixed Berry Superfood Tabs 60-day sub, 7,761 on hand, live PDP available; the orchestrator looped 3× on "clear your cache", once invented a "30/60/90 Day Supply card" UI, false-resolved 9/10, and escalated on a false inventory conflict — the failure this rule retires.)
