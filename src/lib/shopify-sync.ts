...
export async function syncCustomerMonth(
  ...
  while (true) {
    ...
    const query = `{
      customers(first: 250${afterClause}, query: "${dateFilter}") {
        edges {
          cursor
          node {
            id email firstName lastName phone
    ...
    for (let i = 0; i < records.length; i += UPSERT_BATCH) {
      ...
      const { error } = await admin
        ...
        .upsert(batch, { onConflict: "workspace_id,shopify_customer_id" });
      if (!error) totalSynced += batch.length;
      else console.error("Customer month upsert error:", error.message);
    }
...
export async function syncOrderMonth(
  workspaceId: string,
  startDate: string,
  endDate: string,
): Promise<number> {
  ...
  while (true) {
    const afterClause = cursor ? `, after: "${cursor}"` : "";
    const query = `{
      orders(first: 250${afterClause}, query: "${dateFilter}") {
        edges {
          cursor
          node {
            id name email tags sourceName
    ...
    for (let i = 0; i < records.length; i += UPSERT_BATCH) {
      ...
      const { error } = await admin
        ...
        .upsert(batch, { onConflict: "workspace_id,shopify_order_id" });
      if (!error) totalSynced += batch.length;
      else console.error("Order month upsert error:", error.message);
    }
