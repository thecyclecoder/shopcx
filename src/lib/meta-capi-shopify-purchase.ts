 ...
 * derive the SAME `shopify_purchase_{orderId}` event id, so Meta collapses them
 * into one conversion and keeps the richer copy — which is this one, because the
 * order record carries hashed email, phone, name and address that the browser
 * never sees.
 *
...
export function buildShopifyPurchaseEvent(
  ...
  opts?: { fbp?: string | null; fbc?: string | null; fbclid?: string | null; nowMs?: number },
  ...
  return {
    ...
    event: {
      eventName: "Purchase",
      // MUST match the web pixel's `shopify_purchase_${checkout.order.id}`.
      eventId: `shopify_purchase_${orderId}`,
      eventTimeSec,
      ...
      userData: {
        ...
        fbp: opts?.fbp ?? null,
        fbc: deriveFbc(opts?.fbc ?? null, opts?.fbclid ?? null, nowMs),
      },
      customData: {
...
export async function sendShopifyPurchase(
  workspaceId: string,
  payload: Record<string, unknown>,
  opts?: { fbp?: string | null; fbc?: string | null; fbclid?: string | null },
