...
/**
 * Standing-pass BACKSTOP for the post-merge `diff` security review
 * ([[../specs/vault-post-merge-diff-backstop]] Phase 1 + [[../specs/fix-vault-post-merge-diff-backstop-7fbde0]]
 * cheap if-due cron backstop). Sibling of `enqueueSpecTestIfDue` / `enqueueSpecReviewIfDue`: the merge-hook
 * enqueue is a fire-and-forget send — if it drops (a Vercel deploy reaps the Inngest sync mid-flight, box
 * down, transient error) the merged commit never gets its post-merge security pass and nothing re-checks.
 *
 * Enumerate recently-merged `claude/*` builds (`agent_jobs` kind='build', status='merged', updated_at
 ...
 * `backstopPreMergeChecks`, hung off the daily [[../inngest/security-dep-watch]] cron as a second net,
 * AND driven by the cheap [[../inngest/security-diff-backstop-cron]] every 15 minutes so a merged commit
 * can't sit unreviewed for a full day between the daily-cron beats when both the merge hook AND the
 * standing pass have dropped. Does NOT replace the reactive merge-hook enqueue — this is purely additive
 * backstop coverage. Best-effort + never throws.
 */
