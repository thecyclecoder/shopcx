 ...
 *
 *   1. `SecurityBranchInstructions` carries a `head_sha` field (the SHA the review covered).
 *   2. `enqueueSecurityReviewJob` in branch mode records `input.headSha` onto the inserted
 *      `agent_jobs.instructions.head_sha` — the enqueue-time stamp. `null` when the caller
 ...
 *   3. `enqueueSecurityReviewJob` in DIFF mode is UNCHANGED — no head_sha stamped (diff mode
...
test("enqueueSecurityReviewJob (branch mode) records the caller-supplied headSha onto instructions.head_sha", async () => {
  ...
  assert.equal(instr.head_sha, sha, "head_sha must be recorded on the inserted row");
});

test("enqueueSecurityReviewJob (branch mode) records head_sha=null when caller supplies none (server-side callers with no git access)", async () => {
  const { admin, inserts } = makeAdmin();
  const r = await enqueueSecurityReviewJob(admin as never, {
  ...
  const instr = JSON.parse(inserts[0].instructions) as SecurityBranchInstructions;
  assert.equal(instr.head_sha, null, "missing headSha input MUST record null (not undefined) so Phase 2 can key on absence");
});
...
test("enqueueSecurityReviewJob (branch mode) accepts headSha=null explicitly — treated the same as omission", async () => {
  ...
  assert.equal(r.enqueued, true);
  const instr = JSON.parse(inserts[0].instructions) as SecurityBranchInstructions;
  assert.equal(instr.head_sha, null);
});

test("enqueueSecurityReviewJob (DIFF mode) does NOT record head_sha (branch-mode-only field)", async () => {
  const { admin, inserts } = makeAdmin();
  const r = await enqueueSecurityReviewJob(admin as never, {
  ...
  assert.equal(r.enqueued, true);
  const instr = JSON.parse(inserts[0].instructions) as { mode: string; head_sha?: unknown; merge_sha?: string };
  assert.equal(instr.mode, "diff");
  assert.equal("head_sha" in instr, false, "diff-mode instructions must NOT carry head_sha (it is a branch-mode-only field)");
});

test("SecurityBranchInstructions type carries an OPTIONAL head_sha field (string | null | undefined) — pinned at the type layer so callers get a compile-time signal", () => {
  ...
  assert.equal(withHead.head_sha, "0".repeat(40));
  assert.equal(withoutHead.head_sha, undefined);
  assert.equal(explicitNull.head_sha, null);
});
