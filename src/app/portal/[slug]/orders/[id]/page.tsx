/**
 * Order detail page — `/orders/{uuid}` on the portal subdomain. The
 * middleware rewrites the bare path to this route. Mirrors the
 * subscription detail page: server-side auth + link-group ownership
 * check, then delegates to PortalClient with detailOrderId set so the
 * shell (sidebar + nav + branding) is shared with the rest of the
 * portal.
 *
 * `id` is our internal `orders.id` (UUID) — the same key the commerce
 * SDK's detail op reads.
 */
...
export default async function OrderDetailPage({
  ...
  // Linked accounts — the requested order might sit on a sibling
  // profile in the customer's link group. Same treatment as the
  // subscription detail page: read access spans the whole link group.
  ...
  // ── Authorize the requested order ──
  // notFound on no row OR if it belongs to someone outside the link
  // group — never leak the existence of someone else's order.
  const { data: orderRow } = await admin
    ...
    .maybeSingle();
  if (!orderRow || !orderRow.customer_id || !linkedIds.includes(orderRow.customer_id)) {
    return notFound();
  }
  ...
  // Initial subs + orders so the rest of the portal nav has hydrated
  // data if the customer clicks into another section without a hard
  // reload.
