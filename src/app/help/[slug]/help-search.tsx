...
export default function HelpSearch({ slug, basePath }: { slug: string; basePath: string }) {
  const [query, setQuery] = useState("");
  const router = useRouter();

  return (
    <form onSubmit={(e) => { e.preventDefault(); if (query.trim()) router.push(`${basePath || "/"}?search=${encodeURIComponent(query)}`); }}>
