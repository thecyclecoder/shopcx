...
// Hard cap on the widget-articles Lambda so a slow knowledge_base search or
// pooler stall cannot ride Vercel's default 300s ceiling and hard-fail the
// shopper's browser. Each individual admin.from() chain below is additionally
// bounded to 8s via AbortSignal.timeout(8000). See
// [[docs/brain/libraries/widget-articles-route]].
...
export async function GET(
  ...
  try {
    ...
    // Storefront-widget graceful degradation: any thrown error (including the
    // AbortError from AbortSignal.timeout) becomes an empty article list. The
    // widget already tolerates an empty payload — better a fast empty response
    // than a hung Lambda or a 5xx on the shopper's browser.
