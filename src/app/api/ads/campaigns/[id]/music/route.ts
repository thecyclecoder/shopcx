...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Optional free-text style for the music bed (else the default Lyria prompt).
  const prompt = typeof body.prompt === "string" ? body.prompt.trim() : "";

  await inngest.send({
    name: "ad-tool/music-requested",
    data: { workspace_id: workspaceId as string, campaign_id: id, prompt: prompt || undefined },
  });
