...
/** Generate Meta ad copy (4 headlines + 4 primary texts + description) for a campaign. */
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const copy = await generateMetaCopy(workspaceId as string, id);
  if (!copy) return NextResponse.json({ error: "copy_generation_failed" }, { status: 500 });
