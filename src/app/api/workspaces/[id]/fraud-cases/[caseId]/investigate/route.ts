...
export async function GET(
  ...
  const { data: orders } = await admin
    .from("orders")
    .select("id, shopify_order_id, order_number, customer_id, total_cents, line_items, fulfillments, billing_address, shipping_address, payment_details, amplifier_order_id, amplifier_status, amplifier_shipped_at, created_at, source_name, discount_codes")
    .eq("workspace_id", workspaceId)
