...
import { debitStoreCredit } from "@/lib/store-credit";
...
export async function POST(request: Request) {
  ...
  const result = await debitStoreCredit({
    workspaceId,
  ...
  if (!result.ok) {
    return NextResponse.json({ error: result.error || "Failed to debit store credit" }, { status: 400 });
  }
