    ...
    <div className="p-8">
              ...
              <div key={rule.id} className="flex items-center justify-between rounded-lg border border-zinc-200 bg-white px-4 py-3 dark:border-zinc-800 dark:bg-zinc-900">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`rounded-full px-2 py-0.5 text-sm font-medium ${rule.enabled ? "bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400" : "bg-zinc-100 text-zinc-400 dark:bg-zinc-800"}`}>
                      {rule.enabled ? "Active" : "Disabled"}
                    </span>
                  </div>
                  <div className="mt-0.5 flex flex-wrap gap-1">
                    {rule.trigger_events.map(e => (
                      <span key={e} className="rounded bg-zinc-100 px-1.5 py-0.5 text-sm text-zinc-500 dark:bg-zinc-800">{TRIGGER_OPTIONS.find(t => t.value === e)?.label || e}</span>
                    ))}
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => handleToggle(rule)} className="text-sm text-zinc-400 hover:text-zinc-600">{rule.enabled ? "Disable" : "Enable"}</button>
                  <button onClick={() => { setEditingRule(rule); setIsNew(false); }} className="text-sm text-indigo-600 hover:underline dark:text-indigo-400">Edit</button>
                  <button onClick={() => handleDelete(rule.id)} className="text-sm text-red-500 hover:underline">Delete</button>
                </div>
              </div>
            ))
          )}
      ...
      {editingRule && (
        <div className="mt-6 rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
          ...
          {/* Trigger events */}
          <div className="mt-4">
            <label className="block text-sm font-medium text-zinc-500">When (trigger events)</label>
          ...
          {/* Conditions */}
          <div className="mt-4">
            <label className="block text-sm font-medium text-zinc-500">If (conditions)</label>
                  ...
                  {gi > 0 && (
                    <div className="mb-2 flex items-center gap-2">
                      <button
                        ...
                        className="rounded bg-zinc-200 px-2 py-0.5 text-sm font-bold text-zinc-600 dark:bg-zinc-700 dark:text-zinc-300"
                      >
                      ...
                      <div key={ci} className="flex items-center gap-2">
                        {ci > 0 && (
                          <button
                            ...
                            className="shrink-0 rounded bg-zinc-200 px-1.5 py-0.5 text-sm font-bold text-zinc-500 dark:bg-zinc-600 dark:text-zinc-300"
                          >
                            {group.operator}
                          </button>
                        )}
                        <select
                          ...
                          className="rounded-md border border-zinc-300 bg-white px-2 py-1 text-sm dark:border-zinc-600 dark:bg-zinc-700 dark:text-zinc-100"
                        >
                        ...
                        {!["is_empty", "is_not_empty"].includes(cond.op) && (
                          <input
                            ...
                            placeholder="value"
                            className="flex-1 rounded-md border border-zinc-300 bg-white px-2 py-1 text-sm dark:border-zinc-600 dark:bg-zinc-700 dark:text-zinc-100"
                          />
                        )}
                        <button
                          type="button"
          ...
          {/* Actions */}
          <div className="mt-4">
            <label className="block text-sm font-medium text-zinc-500">Then (actions)</label>
          ...
          <div className="mt-4 flex items-center gap-4">
            <div>
              ...
              <input
                ...
                className="mt-1 w-20 rounded-md border border-zinc-300 bg-white px-2 py-1 text-sm dark:border-zinc-600 dark:bg-zinc-700 dark:text-zinc-100"
              />
            </div>
            <label className="flex items-center gap-1.5 pt-4 text-sm text-zinc-500">
              <input
                type="checkbox"
