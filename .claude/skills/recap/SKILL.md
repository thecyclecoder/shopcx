...
3. **Pipe the JSON to `pulse-recap.ts`.** Compose the SessionDigest as a JSON object and invoke the write path via a bash heredoc:
   ...
   - **An explicit (1) or harness (2) id is AUTHORITATIVE and never falls to the mtime guess.** Its transcript is located across **every** `~/.claude/projects/*` dir (not just the current cwd's slug), so a recap invoked from a **different cwd than the session's own** — e.g. from the main repo after the session's git worktree was removed — still resolves correctly with **no `--project-dir` needed**. If the transcript is genuinely gone, the script STILL honors the stated id (`via=harness-env-no-transcript` / `flag-no-transcript`, best-effort null boundary timestamps) rather than guess a different session. This is the fix for the **2026-07-05 second occurrence**: a recap run from the main repo after a worktree was removed didn't find the env-id's transcript under the current `projectDir`, fell to mtime, and clobbered a concurrent session's `/recap` row.
   ...
   - Prints one summary line: `[pulse-recap] upserted session=… via={flag|flag-no-transcript|harness-env|harness-env-no-transcript|mtime-unique} workspace=… digest_model=session-authored threads=N refs=M`. The `via=` field surfaces which resolver step fired: a `*-no-transcript` value means the id was honored but its transcript wasn't found on disk (a removed worktree) — the digest still lands, boundary timestamps are just null.
...
- **Refuse-on-ambiguity, don't guess.** If the script exits with `refusing to guess the current session_id: N transcripts were modified in the last 60s`, that means NEITHER `--session-id` NOR `CLAUDE_CODE_SESSION_ID` was set (an authoritative id is now honored across every project dir and never reaches the mtime branch). Do NOT retry with a shorter window. Pass `--session-id=…` (grab it from `CLAUDE_CODE_SESSION_ID` or the `~/.claude/projects/{cwd-slug}/{uuid}.jsonl` filename) and re-run. `--project-dir` is no longer needed for a cwd/worktree mismatch — the resolver searches all project dirs itself.
