...
## 🔒 Core invariant — author through `authorSpecRowStructured` (aka `submitSpec`), never raw `upsertSpec`
...
**This is now RUNTIME-ENFORCED, not just convention/CI.** As of the harden-spec-submission hotfix, `upsertSpec` is self-gating: before any DB write it THROWS `UngatedSpecAuthorError` (exported from `specs-table.ts`) if any phase's effective `verification` is empty, if the spec's effective `why`/`what` is empty, or if there are zero phases. So a raw bypass that skipped the author-spec gates — the exact path that produced 4 verification-NULL specs on 2026-07-10 that Vale bounced and nobody re-authored — now fails fast at the writer instead of silently landing an untestable spec. The guard is REDUNDANT with the gates below for sanctioned paths (they assert the same fields first) and only bites a raw bypass. Note it requires per-phase `verification` + SPEC-LEVEL `why`/`what` only — NOT per-phase intent (the markdown author path legitimately leaves phase `why`/`what` NULL). Still author through the chokepoint: you want the full gate set + brain-refs + re-open, not just the floor.
