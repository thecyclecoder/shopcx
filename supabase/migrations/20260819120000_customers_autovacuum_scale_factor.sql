-- Tighten per-table autovacuum on `public.customers` — DB Health Agent surface-don't-apply fix for
-- the `dbhealth:bloat:customers` signature (cause `bloat_vacuum_lag`). See
-- docs/brain/recipes/db-vacuum-tune-customers.md for the full sizing math + rollback + verification.
--
...
-- The apply-script also runs a one-off `VACUUM (ANALYZE) public.customers` to clear the current
-- bloat immediately; the reloptions below stop it from recurring. **No data is deleted** by any
-- part of this fix — VACUUM reclaims dead-tuple space and refreshes planner stats, it does not
-- remove rows.
--
-- ── Evidence quoted verbatim from the DB Health Agent bloat pass ─────────────────────────────
--   Signature: dbhealth:bloat:customers
...
-- of the table is dead — on a hot, write-heavy table like `customers` (~620k rows with churn from
-- retention_score / ltv_cents / subscription_status / marketing_status / last_order_at rewrites
-- on every order + every lifecycle transition) that leaves the plan reading through a large
-- minority of dead rows between runs. The bloat pass's finding IS this lag.
--
-- ── The tune (per-table, no cluster-wide change) ─────────────────────────────────────────────
-- Scoped to `customers` via ALTER TABLE ... SET (reloptions). Cluster defaults for every other
-- table stay put — a per-table tune isolates the bounded change to the hot table.
...
-- At customers' scale the scale_factor term dominates, so a 4× tighter scale_factor gives roughly
-- 4× more autovacuum passes — enough to hold the dead-tuple ratio below the DB Health Agent's
...
-- autovacuum continuously and steal write throughput from the checkout / retention-score / phone-
-- and email-match paths that read `customers` on every request.
--
...
-- `dbhealth:bloat:customers` is no longer flagged — `n_dead_tup / (n_live_tup + n_dead_tup)` drops
