-- Companion to the klaviyo_profile_events → profile_events rename
-- (20260518180000). Postgres tracks function bodies as opaque text, so the
-- earlier rename did NOT update the FROM clause inside rebuild_engagement_summary.
-- Re-create the function so it points at the renamed table.
--
-- Note: this RPC currently times out at 2M+ rows even before the rename — a
-- separate optimization is needed (chunked rebuild or materialized view). But
-- updating the body now means a future fix lands cleanly without an extra
...
BEGIN
  ...
  SELECT
    ...
    NOW(),
    NOW()
  FROM profile_events   -- was klaviyo_profile_events before 20260518180000
