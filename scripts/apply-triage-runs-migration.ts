...
const MIGRATIONS = ["20260620160000_triage_runs.sql"];
...
async function main() {
  ...
  try {
    ...
    const { rows } = await c.query(
      "select count(*)::int as n from information_schema.tables where table_name='triage_runs'",
    );
    console.log(`✓ triage_runs table present: ${rows[0].n === 1}`);
  } finally {
    await c.end();
  }
}
