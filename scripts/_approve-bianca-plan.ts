/**
 * Approves Pia's plan for the bianca-temperature-aware-campaign-structure goal
 * (agent_jobs 30d4128c…, kind=plan, needs_approval) — the CEO tapping "approve" on
 * each of the 9 roadmap-inbox actions. Once all approved the job flips to queued_resume
 * and the box worker authors the 9 specs to public.specs. Founder-approved 2026-07-15.
 * Mirrors scripts/_approve-dahlia-copy-plan.ts. Dry by default; APPLY=1 to approve.
 */
...
async function main() {
  ...
  const { data: job } = await admin.from("agent_jobs").select("id, status, pending_actions").eq("id", JOB).maybeSingle();
