// apply-orders-autovacuum-migration — owner-approval-only tune of the per-table autovacuum
// settings on `public.orders`, plus a one-off `VACUUM (ANALYZE)` to clear the current bloat.
// DB Health Agent signature `dbhealth:bloat:orders`; see docs/brain/recipes/db-vacuum-tune-orders.md.
//
...
// public.orders as its own statement — VACUUM cannot run inside a transaction, so it is issued
...
const MIGRATION = "20260704120000_orders_autovacuum_scale_factor.sql";
...
async function main() {
  ...
  try {
    ...
    console.log(`✓ orders reloptions: ${reloptions.length ? reloptions.join(" · ") : "(null)"}`);
    for (const [key, value] of Object.entries(EXPECTED_RELOPTIONS)) {
      const expected = `${key}=${value}`;
      if (!reloptions.includes(expected)) {
        throw new Error(
          `expected reloption ${expected} on public.orders, got: ${reloptions.length ? reloptions.join(" · ") : "(null)"}`,
        );
      }
    }
    ...
    console.log(`… running VACUUM (ANALYZE) public.orders — clears current bloat, no rows deleted`);
    await c.query(`VACUUM (ANALYZE) public.orders`);
    console.log(`✓ VACUUM (ANALYZE) public.orders complete`);
