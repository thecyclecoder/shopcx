...
const sql = readFileSync(resolve(__dirname, "../supabase/migrations/20260601190000_review_polished_at.sql"), "utf8");
async function main() {
  const c = new Client({ connectionString: cs });
  await c.connect();
  try { await c.query(sql); console.log("✓ applied 20260601190000_review_polished_at.sql"); }
  finally { await c.end(); }
}
