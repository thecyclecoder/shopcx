/**
 * ads-supervisor fix — product `f55a1cb1-f3ca-4e0d-9c64-ecd1cd865efb` (Ashwavana Guru
 * Focus, workspace `fdc11e10-...`): the ready-to-test bin is thin (3 vs DEFAULT_BIN_FLOOR
 * of 4). Authored to satisfy fix-spec
 * [[../docs/brain/specs/ads-supervisor-fix-fdc11e10-dahlia-bin-f55a1cb1-f3ca-4e0d-9c64-ecd1cd865efb]].
 *
 * The daily `ad-creative-cadence-cron` (0 11 UTC) already self-heals this — on its next
 * tick it enqueues one `kind='ad-creative'` job per product below floor. This script exists
 * so the fix can be applied on-demand (dry-run by default) and produces the diagnostic
 * required by the spec body:
 *   1) `agent_jobs` where `kind='ad-creative'` AND `instructions->>'product_id' = <PID>`
 *      over the last 24h — did Dahlia get dispatched?
 *   2) `product_ad_angles` scoped to `workspace_id` + `product_id` — an empty set is the
 *      intelligence gap blocking Dahlia's generate step.
...
const PRODUCT_ID = "f55a1cb1-f3ca-4e0d-9c64-ecd1cd865efb";
