...
const sql = readFileSync(resolve(__dirname, "../supabase/migrations/20260601210000_auth_otp.sql"), "utf8");
async function main() {
  const c = new Client({ connectionString: cs });
  await c.connect();
  try { await c.query(sql); console.log("✓ applied 20260601210000_auth_otp.sql"); }
  finally { await c.end(); }
}
