...
const MIGRATIONS = ["20260622220000_worker_heartbeats_accounts.sql"];
...
async function main() {
  ...
  try {
    ...
    const { rows } = await c.query(
      "select count(*)::int as n from information_schema.columns where table_name='worker_heartbeats' and column_name = 'accounts'",
    );
    console.log(`✓ worker_heartbeats.accounts present: ${rows[0].n === 1}`);
  } finally {
    await c.end();
  }
}
