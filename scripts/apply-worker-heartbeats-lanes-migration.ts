...
const MIGRATIONS = ["20260619150000_worker_heartbeats_lanes.sql"];
...
async function main() {
  ...
  try {
    ...
    const { rows } = await c.query(
      "select count(*)::int as n from information_schema.columns where table_name='worker_heartbeats' and column_name in ('build_lanes','fold_lanes','lanes')",
    );
    console.log(`✓ worker_heartbeats lane columns present: ${rows[0].n === 3}`);
  } finally {
    await c.end();
  }
}
