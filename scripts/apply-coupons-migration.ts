...
async function main() {
  ...
  const sql = readFileSync(resolve(__dirname, "../supabase/migrations/20260608120000_coupons.sql"), "utf8");
  await client.query(sql);
  const r = await client.query("select column_name from information_schema.columns where table_name='coupons' order by ordinal_position");
  console.log("✓ coupons table columns:", r.rows.map((x:{column_name:string})=>x.column_name).join(", "));
  const idx = await client.query("select indexname from pg_indexes where tablename='coupons'");
  console.log("  indexes:", idx.rows.map((x:{indexname:string})=>x.indexname).join(", "));
  await client.end();
}
