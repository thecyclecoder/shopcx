...
const MIGRATIONS = ["20260622150000_error_events.sql"];
...
async function main() {
  ...
  try {
    ...
    const { rows } = await c.query(
      "select count(*)::int as n from information_schema.tables where table_name = 'error_events'",
    );
    console.log(`✓ error_events table present: ${rows[0].n}/1`);
  } finally {
    await c.end();
  }
}
