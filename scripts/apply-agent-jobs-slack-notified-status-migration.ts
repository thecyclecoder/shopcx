...
const MIGRATIONS = ["20260619120000_agent_jobs_slack_notified_status.sql"];
...
async function main() {
  ...
  try {
    ...
    const { rows } = await c.query(
      "select count(*)::int as n from information_schema.columns where table_name='agent_jobs' and column_name='slack_notified_status'",
    );
    console.log(`✓ agent_jobs.slack_notified_status present: ${rows[0].n === 1}`);
  } finally {
    await c.end();
  }
}
