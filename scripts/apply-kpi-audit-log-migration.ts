...
const MIGRATIONS = ["20260727120000_kpi_audit_log_and_loop_alerts_owner.sql"];
...
async function main() {
  ...
  try {
    ...
    const { rows: tbl } = await c.query(
      "select count(*)::int as n from information_schema.tables where table_name = 'kpi_audit_log'",
    );
    console.log(`✓ kpi_audit_log table present: ${tbl[0].n}/1`);
    const { rows: col } = await c.query(
      "select count(*)::int as n from information_schema.columns where table_name = 'loop_alerts' and column_name = 'owner'",
    );
    console.log(`✓ loop_alerts.owner column present: ${col[0].n}/1`);
  } finally {
    await c.end();
  }
}
