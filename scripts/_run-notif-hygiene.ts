...
import { sweepExpiredReports, sweepSettledChargebacks, DAILY_SUMMARY_TTL_DAYS } from "../src/lib/notification-hygiene";
  ...
  if (APPLY) {
    ...
    await admin.from("director_activity").insert({
      ...
      reason:
        ...
        `(${DAILY_SUMMARY_TTL_DAYS}d TTL) and ${cbs.dismissed} SETTLED chargeback alert(s), keeping ${cbs.kept} unsettled. ` +
      ...
      metadata: { reports: reports.dismissed, chargebacks_retired: cbs.dismissed, chargebacks_kept: cbs.kept, autonomous: false },
    });
