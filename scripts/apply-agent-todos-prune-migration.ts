...
const MIGRATIONS = ["20260620160100_agent_todos_prune_action_types.sql"];
...
async function main() {
  ...
  try {
    ...
    const { rows } = await c.query(
      "select pg_get_constraintdef(oid) as def from pg_constraint where conname='agent_todos_action_type_check'",
    );
    console.log(`✓ constraint: ${rows[0]?.def ?? "(missing)"}`);
  } finally {
    await c.end();
  }
}
