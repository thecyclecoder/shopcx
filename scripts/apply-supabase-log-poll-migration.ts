...
const MIGRATIONS = ["20260622160000_supabase_log_poll.sql"];
...
async function main() {
  ...
  try {
    ...
    const { rows: tbl } = await c.query(
      "select count(*)::int as n from information_schema.tables where table_name = 'error_feed_supabase_config'",
    );
    console.log(`✓ error_feed_supabase_config table present: ${tbl[0].n}/1`);
