...
const MIGRATIONS = ["20260619170000_drop_workspace_slack_roadmap_list.sql"];
...
async function main() {
  ...
  try {
    ...
    const { rows } = await c.query(
      "select count(*)::int as n from information_schema.columns where table_name='workspaces' and column_name='slack_roadmap_list'",
    );
    console.log(`✓ workspaces.slack_roadmap_list dropped: ${rows[0].n === 0}`);
  } finally {
    await c.end();
  }
}
