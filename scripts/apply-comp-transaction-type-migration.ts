...
const MIGRATIONS = ["20260622170000_comp_transaction_type.sql"];
...
async function main() {
  ...
  try {
    ...
    const { rows } = await c.query(
      "select pg_get_constraintdef(oid) as def from pg_constraint where conname = 'transactions_type_check'",
    );
    const def = rows[0]?.def || "";
    console.log(`✓ transactions_type_check: ${def}`);
    console.log(`✓ admits 'comp': ${def.includes("'comp'")}`);
  } finally {
    await c.end();
  }
}
