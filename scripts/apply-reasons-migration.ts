...
async function main() {
  ...
  const sql = readFileSync(resolve(__dirname, "../supabase/migrations/20260616120000_advertorial_pages_reasons.sql"), "utf8");
  await client.query(sql);
  const r = await client.query("select column_name from information_schema.columns where table_name='advertorial_pages' and column_name='reasons'");
  console.log("✓ advertorial_pages.reasons present:", r.rows.length === 1);
  await client.end();
}
