...
const SUMMARIES: Record<string, string> = {
  ...
  "klaviyo-attribution-compute.ts": "⚠️ RETIRED no-op — returns `KLAVIYO_RETIRED_RESULT`. Recomputed `klaviyo_sms_campaign_history.initial_revenue_cents` by joining Placed Orders via `attributed_klaviyo_campaign_id`. Deleted in Phase B.",
  "klaviyo-engagement-backfill.ts": "⚠️ RETIRED no-op — returns `KLAVIYO_RETIRED_RESULT`. Was a 180d historical engagement backfill. Deleted in Phase B.",
  "klaviyo-engagement-sync.ts": "⚠️ RETIRED no-op — beats then returns `KLAVIYO_RETIRED_RESULT` so Control Tower reads *retired*, not *no beats*. Was the daily 4am CST engagement delta into `profile_events`. Deleted with its MONITORED_LOOPS row in Phase B.",
  "klaviyo-events-import.ts": "⚠️ RETIRED no-op — returns `KLAVIYO_RETIRED_RESULT`. Pulled Placed Order events with UTM-attribution parsing into `klaviyo_events` (frozen 2026-05-14). Deleted in Phase B.",
  "klaviyo-sms-import.ts": "⚠️ RETIRED no-op — returns `KLAVIYO_RETIRED_RESULT`. Was an on-demand pull of historical Klaviyo SMS campaigns. Deleted in Phase B.",
  ...
  "sync-reviews.ts": "⚠️ RETIRED no-op — beats then returns `KLAVIYO_RETIRED_RESULT` so Control Tower reads *retired*, not *no beats*. Was the nightly Klaviyo review sync into `product_reviews`. The AI-summary pass (`generateMissingSummaries`, Haiku, max 15 words) is table-only and survives. Deleted with its MONITORED_LOOPS row in Phase B.",
