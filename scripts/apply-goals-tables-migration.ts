...
// + triggers + the parent_goal_id cycle guard + the specs.milestone_id FK constraint
...
// alter-add-constraint is guarded by an information_schema lookup).
...
async function main() {
  ...
  try {
    ...
    console.log(`✓ specs.milestone_id FK present: ${fk.length === 1 ? "yes" : "MISSING"}`);
  } finally {
    await c.end();
  }
}
