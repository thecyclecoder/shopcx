...
const MIGRATIONS = ["20260622170000_spec_drift.sql"];
...
async function main() {
  ...
  try {
    ...
    const { rows } = await c.query(
      "select count(*)::int as n from information_schema.tables where table_name='spec_drift'",
    );
    console.log(`✓ spec_drift table present: ${rows[0].n === 1}`);
  } finally {
    await c.end();
  }
}
