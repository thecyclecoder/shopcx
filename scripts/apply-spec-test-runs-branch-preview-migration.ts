...
const MIGRATIONS = ["20260627120000_spec_test_runs_branch_preview.sql"];
...
async function main() {
  ...
  try {
    ...
    const { rows } = await c.query(
      "select column_name from information_schema.columns where table_name='spec_test_runs' and column_name in ('spec_branch','preview_url') order by column_name",
    );
    console.log(`✓ spec_test_runs columns present: ${rows.map((r) => r.column_name).join(", ")}`);
  } finally {
    await c.end();
  }
}
