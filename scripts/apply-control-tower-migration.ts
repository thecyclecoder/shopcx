...
const MIGRATIONS = ["20260622120000_control_tower.sql"];
...
async function main() {
  ...
  try {
    ...
    const { rows } = await c.query(
      "select count(*)::int as n from information_schema.tables where table_name in ('loop_heartbeats','loop_alerts')",
    );
    console.log(`✓ control-tower tables present: ${rows[0].n}/2`);
  } finally {
    await c.end();
  }
}
