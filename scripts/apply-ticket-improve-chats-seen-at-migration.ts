...
const MIGRATIONS = ["20260620180000_ticket_improve_chats_seen_at.sql"];
...
async function main() {
  ...
  try {
    ...
    const { rows } = await c.query(
      "select column_name from information_schema.columns where table_name='ticket_improve_chats' order by ordinal_position",
    );
    console.log(`✓ ticket_improve_chats columns: ${rows.map((r) => r.column_name).join(", ")}`);
  } finally {
    await c.end();
  }
}
