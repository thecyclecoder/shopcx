...
const MIGRATIONS = ["20260707120000_error_events_resolution.sql"];
...
async function main() {
  ...
  try {
    ...
    const { rows } = await c.query(
      "select column_name, data_type from information_schema.columns where table_name = 'error_events' and column_name in ('resolved_at','resolution_reason') order by column_name",
    );
    console.log(`✓ error_events resolution columns: ${rows.map((r) => `${r.column_name}:${r.data_type}`).join(", ") || "MISSING"}`);
  } finally {
    await c.end();
  }
}
