...
const MIGRATIONS = ["20260619140000_worker_heartbeats.sql"];
...
async function main() {
  ...
  try {
    ...
    const { rows } = await c.query(
      "select count(*)::int as n from information_schema.tables where table_name='worker_heartbeats'",
    );
    console.log(`✓ worker_heartbeats table present: ${rows[0].n === 1}`);
  } finally {
    await c.end();
  }
}
