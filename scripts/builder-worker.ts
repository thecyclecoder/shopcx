...
async function runSpecTestJob(job: Job) {
  ...
  try {
    const prompt = [
      ...
      `\`needs_human\` is now EXACTLY TWO cases: (a) VISUAL/AESTHETIC judgment (looks good / renders nicely / big enough — human taste is the only instrument), and (b) an IRREVERSIBLE PROD SIDE-EFFECT with NO already-observable evidence AND NO local-harness equivalent (a real SMS/email/charge actually reaching an external carrier/processor). Everything else attempts a non-destructive verification.`,
      ...
      `You have the box's read-only QA toolkit: repo Read/Grep + \`npx tsc --noEmit\`, the \`gh\` CLI for CI/PR status, the \`vercel\` CLI for deploy READY + logs + \`vercel env ls\`, read-only DB probes via \`npx tsx scripts/spec-test-db-probe.ts "<select …>"\`, GET/read-only endpoint hits, and a non-destructive local harness (\`_\`-prefixed scratch \`npx tsx\` script importing pure code from src/). NEVER mutate prod, NEVER mark the spec verified/archived, NEVER run a mutating prod check (those are needs_human).`,
