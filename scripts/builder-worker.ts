...
interface Job {
  ...
  kind: "build" | "plan" | "fold" | "product-seed" | "ticket-improve" | "spec-chat";
  status: JobStatus;
...
async function main() {
  ...
  console.log(`builder-worker up — repo ${REPO} @ ${RUNNING_SHA || "?"}, up to ${MAX_CONCURRENT} build lanes + ${MAX_FOLD} fold + ${MAX_SEED} seed + ${MAX_SPEC_CHAT} spec-chat + ${MAX_TICKET_IMPROVE} ticket-improve lane, polling every ${POLL_MS}ms`);
  ...
  const countOther = () => [...active.values()].filter((v) => v.kind !== "fold" && v.kind !== "product-seed" && v.kind !== "spec-chat" && v.kind !== "ticket-improve").length;
