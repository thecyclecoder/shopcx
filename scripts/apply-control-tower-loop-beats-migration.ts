...
const MIGRATIONS = ["20260622160000_control_tower_loop_beats.sql"];
...
async function main() {
  ...
  try {
    ...
    // Smoke-test the new grouped read: it should return at most p_history_limit rows per loop.
    const { rows } = await c.query("select count(*)::int as n from public.control_tower_loop_beats(10)");
    console.log(`✓ control_tower_loop_beats() returns ${rows[0].n} beat rows`);
  } finally {
    await c.end();
  }
}
