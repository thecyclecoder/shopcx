...
async function main(){
  ...
  const { data:spec } = await admin.from("specs").select("id,last_merge_sha,merged_pr").eq("workspace_id",WS).eq("slug","spec-timecard-ledger-and-sdk").single();
  console.log("M1 last_merge_sha:", (spec as any)?.last_merge_sha ?? "-", " merged_pr:", (spec as any)?.merged_pr ?? "-");
