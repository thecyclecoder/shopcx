// apply-ticket_messages-autovacuum-migration — owner-approval-only tune of the per-table
// autovacuum settings on `public.ticket_messages`, plus a one-off `VACUUM (ANALYZE)` to clear the
// current bloat. DB Health Agent signature `dbhealth:bloat:ticket_messages` (trend variant); see
// docs/brain/recipes/db-vacuum-tune-ticket_messages.md.
//
...
//   npx tsx scripts/apply-ticket_messages-autovacuum-migration.ts
//
// Two-step: (1) apply the reloptions migration inside the pool connection; (2) VACUUM (ANALYZE)
// public.ticket_messages as its own statement — VACUUM cannot run inside a transaction, so it is
// issued separately after the ALTER TABLE.
//
...
const MIGRATION = "20261215160000_ticket_messages_autovacuum_scale_factor.sql";
...
async function main() {
  ...
  try {
    ...
    const { rows: relRows } = await c.query(
      `select reloptions
         from pg_class
        where relname = 'ticket_messages'
          and relnamespace = 'public'::regnamespace`,
    );
    const reloptions = ((relRows[0]?.reloptions ?? []) as string[]) ?? [];
    console.log(`✓ ticket_messages reloptions: ${reloptions.length ? reloptions.join(" · ") : "(null)"}`);
    for (const [key, value] of Object.entries(EXPECTED_RELOPTIONS)) {
      const expected = `${key}=${value}`;
      if (!reloptions.includes(expected)) {
        throw new Error(
          `expected reloption ${expected} on public.ticket_messages, got: ${reloptions.length ? reloptions.join(" · ") : "(null)"}`,
        );
      }
    }
    ...
    console.log(`… running VACUUM (ANALYZE) public.ticket_messages — clears current bloat, no rows deleted`);
    await c.query(`VACUUM (ANALYZE) public.ticket_messages`);
    console.log(`✓ VACUUM (ANALYZE) public.ticket_messages complete`);
