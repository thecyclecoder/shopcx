/**
 * ads-supervisor autonomous fix — product `Ashwavana Zen Relax`
 * (product_id `48bfa48c-b8db-42f9-9303-19c70ab8e7a1`, workspace `fdc11e10-…`).
 *
 * The every-3h supervisor pass classified this product's ready-to-test bin as
 * thin (depth=2 vs floor=DEFAULT_BIN_FLOOR=4) and authored the fix-spec
 * `ads-supervisor-fix-fdc11e10-dahlia-bin-48bfa48c-b8db-42f9-9303-19c70ab8e7a1`
 ...
 *
 *   npx tsx scripts/ads-supervisor-fix-fdc11e10-dahlia-bin-48bfa48c.ts
 */
import "./_bootstrap";
...
const PRODUCT_ID = "48bfa48c-b8db-42f9-9303-19c70ab8e7a1";

async function main() {
  const admin = createAdminClient();

  // ── Diagnostic 0 — confirm the product is still is_advertised. ─────────
  const advertised = await isAdvertisedProduct(admin, PRODUCT_ID);
  if (!advertised) {
    console.log(`✗ product ${PRODUCT_ID} is not is_advertised — supervisor should not have flagged it; no fix.`);
    return;
  }

  // ── Diagnostic 1 — confirm the bin is still thin. ──────────────────────
  ...
  // ── Diagnostic 2 — is Dahlia already dispatched in the last 24h? ───────
  ...
  const activeJob = jobsForProduct.find((j) => ACTIVE_MEDIA_BUYER_JOB_STATUSES.has(j.status));
  console.log(
  ...
  // ── Diagnostic 3 — does the product have any ad intelligence to generate from? ─
