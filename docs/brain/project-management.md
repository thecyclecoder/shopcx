...
- **Optional `Blocked-by` — build prerequisites** ([[specs/spec-blockers]]). A spec that depends on another spec shipping first adds a header line `**Blocked-by:** [[spec-a]], [[spec-b]]` (parsed like Owner/Parent, [[libraries/brain-roadmap]] `parseSpec`). A blocker is **cleared** once its spec ships (or is archived/folded); until then the build enqueue chokepoint (`queueRoadmapBuild`, [[libraries/roadmap-actions]]) **refuses to queue a build** for the dependent spec and the [[dashboard/roadmap|board]] shows a "🔒 Blocked by …" chip with the Build button disabled. This formalizes the manual "queue after X merges" chaining and stops two builds colliding on the same files. Pointing `Blocked-by` at an already-shipped/archived spec is a no-op (treated as cleared). **When the *last* blocker ships, the dependent's build auto-queues itself** (`agent-jobs.autoQueueUnblockedBy`, fired from `reconcileMergedJobs` — the same merge/board-load hook spec-test uses): merge the prerequisite and the chained build fires hands-off, de-duped to one auto-queue per spec. To keep a spec manual-only, add `**Auto-build:** off` under its H1 (the auto-queue skips it; the manual Build button still works).
  - **Goal-blocker rule** ([[specs/one-off-spec-depending-on-goal-work-blocks-on-the-goal-not-the-member-spec]]). When an **outside** (not-in-goal) spec's `Blocked-by` names a spec that is a MEMBER of goal G, the EFFECTIVE blocker is the **goal**, not the member spec — the member spec doesn't land on `main` on its own; it lands via the atomic goal→main promotion. [[libraries/brain-roadmap]] `resolveBlockedBy` normalizes the entry to a `kind:"goal"` blocker keyed on `goals.main_merge_sha`; every enqueue path (queueRoadmapBuild, `enqueueBuildIfDue`, autobuild sweeps) refuses to claim the dependent while `main_merge_sha` is null, and the reactive `autoQueueUnblockedByGoal` fan-out fires from `promoteCompleteGoalsToMain` the instant the goal ships. Goal-mate dependencies (dependent + blocker in the same goal) are unaffected — the intra-goal serializer still handles them.
...
**Human QA is advisory, never a gate** ([[tables/spec_test_human_checks]] + [[specs/every-spec-writer-authors-machine-runnable-verifications]] Phase 2). A bullet the box QA agent can't auto-test (visual/UX or prod-mutating) is flagged **👤 needs-human** and surfaced on the **Human QA (optional)** queue — a place to run extra human checks if you want. It **never** blocks the fold, the brain, or the spec's progression; the owner can clear an item whenever (or never). Since Phase 2, subjective "eyeball this after ship" prompts also have a first-class home: the OPTIONAL, non-blocking spec-level `specs.human_review` column, rendered on the spec card + a post-ship founder surface and **NEVER read by the fold gate / promote gate / deterministic spec-check runner** — machine-runnable `spec_phase_checks` remain the sole ship gate. Why machine-pass is sufficient: fold is non-destructive (a fold you'd want to undo is just a follow-up spec), so the per-phase `## Verification` checks graded green by the spec-test agent are enough verification to extract the knowledge.
  ...
  phases: [
    {
      ...
      checks: [
        // MACHINE-RUNNABLE — every phase MUST carry ≥1 typed row (Phase 1 of
        // [[specs/every-spec-writer-authors-machine-runnable-verifications]]). exec_kind is one
        // of tsc|grep|ci_status|http_get|db_probe_readonly|unit_test|build; params is validator-
        // enforced ([[libraries/spec-phase-checks-executable]]). Prose-only / needs_human-only
        // phases throw MissingMachineCheckError at author time.
        { position: 1, kind: "auto"  as const, description: "tsc clean",
          exec_kind: "tsc" as const, params: null },
        { position: 2, kind: "auto"  as const, description: "the new resolver is wired",
          exec_kind: "grep" as const,
          params: { pattern: "resolveThing", path: "src/lib/thing.ts", expect: "present" as const } },
        // OPTIONAL — advisory needs_human rows are legal as EXTRA, never the sole check.
        { position: 3, kind: "human" as const, description: "eyeball the funnel report",
          exec_kind: "needs_human" as const, params: null },
      ],
    },
  ],
  // OPTIONAL, non-blocking founder-facing advisory note (Phase 2 of
  // [[specs/every-spec-writer-authors-machine-runnable-verifications]]). NEVER read by
  // fold/promote/spec-check runner. Absence is the norm.
  human_review: "After ship, open /dashboard/x and confirm the layout reads right.",
...
**Structured is the ONLY sanctioned author path (retire-md-spec-writers-db-is-sole-spec Phase 4).** The retired MARKDOWN entrypoint — `authorSpecRowFromMarkdown` (and its `scripts/builder-worker.ts` wrapper `markNewSpecInReview`) — is CLOSED to new autonomous callers. Prose Verification bullets stamped `exec_kind='needs_human'` get rejected by `assertEveryPhaseHasMachineCheck`, parking the fix-spec at the CEO inbox — the exact failing state the whole spec exists to kill. A CI guard (`scripts/_check-no-markdown-spec-authoring.ts`, wired into `predeploy`) fails on any new caller of the retired chokepoint outside its pre-Phase-4 allow-list; a regression test (`src/lib/every-deterministic-body-producer.test.ts`) asserts every deterministic spec-body producer (coverage-register's `buildRegisterSpecBody` / `buildExemptSpecBody` · repair-agent's `buildRepairSpecInput` · author-spec's `buildStructuredSpecInputFromMarkdown`) yields a `StructuredSpecInput` whose every phase carries ≥1 machine-runnable check. A NEW autonomous spec-writer lane authors STRUCTURED from day one — see [[libraries/author-spec]] § "Structured is the ONLY sanctioned author path" for the canonical pattern.
...
**Structured intent + structured verification checks ([[specs/pm-structured-intent-and-refs]] + [[specs/every-spec-writer-authors-machine-runnable-verifications]]).** Every level of the PM tree (goals, goal_milestones, specs, spec_phases) carries plain-language `why` + `what` columns that LEAD the detail page — the same value humans and agents both read. Specs + phases are HARD-gated: [[libraries/author-spec]] `authorSpecRowStructured` throws `MissingIntentError` before the DB write when either is empty (or fails the plain-language lint that rejects code fences / `file:line` / `**Header:**` lines). The `## Verification` bullets are stored as structured rows in [[tables/spec_phase_checks]] (`{position, description, kind:'auto'|'human', exec_kind, params}`) with a machine-runnable payload from [[libraries/spec-phase-checks-executable]] that the deterministic spec-check runner executes; the same chokepoint gates ≥1 check per phase (`assertEveryPhaseHasChecks`) AND ≥1 MACHINE-RUNNABLE check per phase (`assertEveryPhaseHasMachineCheck` — Phase 1 of every-spec-writer-authors-machine-runnable-verifications; prose-only / needs_human-only verification is REJECTED with `MissingMachineCheckError`). Optional subjective "eyeball this after ship" prompts live on the spec-level `specs.human_review` column (Phase 2 — advisory, non-blocking, rendered on the spec card, never gates fold/promote). Brain refs move off the prose `**Brain refs:**` line into [[tables/spec_brain_refs]] rows (spec_id/phase_id → `kind/name` slug) — `scripts/_check-brain-refs.ts` validates every slug resolves to a real `docs/brain/{kind}/{name}.md`. The typed parent lives on `specs.parent_kind` (`function`/`mandate`/`milestone`) + `specs.parent_ref`. Legacy rows keep working (columns nullable); new authoring is gated. Existing prose-only rows are best-effort backfilled to typed by `scripts/backfill-spec-checks-to-typed.ts` (Phase 3 — dry-run/apply, safety-first classifier; unmappable prose stays needs_human).
