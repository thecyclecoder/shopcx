...
**Shipped (✅)** = the build pipeline ran and deployed the code — *automated*. **Folded** = the spec's machine spec-test passed, so its durable knowledge has been extracted into the permanent brain pages and its row flipped to `status='folded'` — also *automated*. **The fold trigger is the MACHINE spec-test pass, NOT a human click** (fold-on-spec-test-pass, task #29). The gap between Shipped and Folded should be a short, honest "live but not yet machine-tested-green" list, not a graveyard of done work waiting on an uncompletable human-QA backlog. Fold is **non-destructive**: the [[tables/specs]] / [[tables/spec_phases]] row is **preserved** (`status='folded'`) — the fold only extracts knowledge into the brain pages and appends a one-line [[archive]] pointer. **Nothing is ever lost.**
...
4. **Append to the archive index** — add one line to [[archive]]'s `## Index` list (newest-first): `- **{Title}** · verified {YYYY-MM-DD} · → [[lifecycles/{slug}]]` (the `· verified {date} ·` token is the format the board + brain-index generator parse — keep it literal). This is the browsable pointer the board's **Archived** section reads.
5. **Flip the row to `folded` (preserved, NOT deleted)** — the [[tables/specs]] row keeps its history with `status='folded'`; the PM flow reads the DB, so the spec stays visible/auditable. The fold-build PR also `git rm`s the on-disk `docs/brain/specs/{slug}.md` (content-only; the DB row is authoritative and the knowledge now lives in its permanent brain homes) — this is non-destructive: the row is preserved and the markdown is always `git show`-recoverable.
6. **One PR / commit** — fold + archive-index + status-flip + commit together. Don't leave the spec lingering "just in case." Git history is the immutable archive.
