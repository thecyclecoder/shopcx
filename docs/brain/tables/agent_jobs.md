...
| `kind` | `text` | `build` (default) ｜ `plan` ｜ `fold` ｜ `product-seed` ｜ `ticket-improve` ｜ `triage-escalations` — the job kind the worker branches on |
