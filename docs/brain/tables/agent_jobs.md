...
| `kind` | `text` | `build` (default) ｜ `plan` ｜ `fold` ｜ `product-seed` ｜ `ticket-improve` ｜ `spec-chat` — the job kind the worker branches on (free text, no CHECK; `claim_agent_job` filters by a dynamic `p_kinds` array) |
| `spec_slug` | `text` | for `build`: the `docs/brain/specs/{slug}.md` to build · for `plan`: the `docs/brain/goals/{slug}.md` to plan · for `product-seed`: the `product_id` · for `ticket-improve`: the improve-session id · for `spec-chat`: the [[roadmap_chats]] thread id (or `verify:{slug}`) |
