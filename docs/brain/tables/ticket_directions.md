...
## Predeploy drift guard

`scripts/_check-ticket-direction-path-drift.ts` (Phase 2 of [[../specs/ticket-direction-path-workflow-enum-drift]], wired into `npm run predeploy:static` as `check:ticket-direction-path-drift`) fails the build whenever the `TicketDirectionPath` TS union in [[../libraries/ticket-directions]] and the live `public.ticket_direction_path` enum disagree — a value in one but not the other. It parses the union out of the SDK source text (no runtime import, so an unrelated SDK compile error can't hide drift) and reads enum labels via `pg_enum join pg_type`. When the pooler is unreachable (a CI-lite context without creds) it SKIPS with a clear message and exits 0, matching [[../libraries/control-tower/migration-drift]] and the other DB-touching predeploy checks. The next path added to either side without a matching migration/union edit fails predeploy red — never a silent Sol-session kill in prod.
