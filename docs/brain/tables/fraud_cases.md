...
- **Linked cluster banning (2026-09):** When `status='confirmed_fraud'`, the confirm-fraud route's `ban_customer` step resolves the case's full linked cluster via [[../libraries/fraud-linked-customers]] (seed `customer_ids` unioned with all members of their [[customer_links]] groups) and sets `portal_banned=true` on every account. The investigation route and confirm-fraud route use the same resolver, ensuring the operator's screen and the actual ban cannot diverge. The response includes per-account results (banned / already-banned / error) and the full scope breakdown so the UI renders the true blast radius rather than assuming.
