...
| `adset_template` | `jsonb?` | The cloned adset spec (`optimizationGoal`/`billingEvent`/`bidStrategy`/`pixelId`/`customEventType`/`targeting`) applied to every per-test adset so only the CREATIVE varies. NULL for legacy cohorts. |
