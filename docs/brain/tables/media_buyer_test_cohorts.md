...
- **[[../specs/bianca-cold-test-recent-purchaser-exclusion]] Phase 3 (backfill):** `scripts/_backfill-cold-test-recent-purchaser-exclusion.ts` — one-shot idempotent backfill that, for every ACTIVE per-test cohort still missing `excluded_purchaser_audience_id`, find-or-creates the pixel-side last-180d purchasers audience via [[../libraries/meta-ads]] `getOrCreateRecentPurchaserAudience` (Phase 1 helper) and stamps both `excluded_purchaser_audience_id` AND `adset_template.targeting.excluded_custom_audiences = [{ id }]`. Compare-and-set on the no-exclusion shape — a hand-edited template is skipped, never overwritten. Dry-run by default; `--apply` writes. Auto-ledgered by [[../libraries/ship-time-backfill-detector]] via the `_backfill-*.ts` filename convention and drained on the box by [[../libraries/ship-time-backfill-executor]] `executeShipTimeBackfillsForSpec`.
