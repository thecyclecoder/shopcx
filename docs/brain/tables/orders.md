...
| `hold_status` | `text` | ✓ | Allergen/safety hold state — `'placed'` \| `'refused'` \| `null` (check-constrained). `'placed'` means an escalation raised a protective hold and the parcel MUST NOT ship; `'refused'` means the hold was ATTEMPTED but the parcel was already gone (`amplifier_shipped_at IS NOT NULL` or `fulfillment_status='fulfilled'`); `null` means no hold has ever been attempted. All three MUST render distinguishably on the ticket surface — a refused/absent hold on a live allergy ticket is an alert, not a field ([[../libraries/order-holds]] + spec `a-flagged-allergen-order-must-not-ship`). Written ONLY via [[../libraries/order-holds]] `attemptAllergenHold` (compare-and-set on `hold_status IS NULL`). |
| `hold_kind` | `text` | ✓ | Categorical reason the hold was raised (`'allergen'` for Phase 1). Bounded so the reconcile-sweep guard + ticket surface can key on it deterministically. |
| `hold_reason` | `text` | ✓ | Free-text reason as raised — mirrors the ticket's `escalation_reason` at the moment the hold was placed. |
| `hold_ticket_id` | `uuid` | ✓ | The ticket whose escalation triggered the hold. NOT an FK — a ticket-delete cascade would erase the audit trail on the order. |
| `hold_placed_at` | `timestamptz` | ✓ | When the hold was attempted. Stamped on BOTH the placed and refused paths — the attempt itself is what matters. |
| `hold_refused_reason` | `text` | ✓ | Set only when `hold_status='refused'` (archetype: `'already_shipped'`). The moment the remedy space collapses to a refund and the ticket handler needs to know. Partial index `orders_hold_status_placed_idx` covers the reconcile-sweep's `hold_status='placed'` lookup ([[../inngest/amplifier-import-reconcile]] `reconcileOne`). |
...
- `line_items` is JSONB. Variant ids inside, not a join. **Per-line shape** (webhook-write in `shopify-webhooks.ts` `handleOrderEvent`): `{ title, quantity, price_cents, sku, variant_id, variant_title, total_discount_cents, product_id }`. `variant_title` / `total_discount_cents` / `product_id` were added in Phase 2 of [[../specs/shopify-order-confirmation-emails]] so the [[../libraries/order-confirmation-data]] resolver renders without a Shopify GraphQL round-trip. `variant_id` remains the primary join key to `product_variants.shopify_variant_id`; `product_id` is a *secondary* Shopify-id lookup — images/UUID still come from the variant join.
- `shopify_order_id` is a numeric string. Internal joins should use `id` (UUID), not the Shopify id.
- `payment_details` is JSONB and gets **merged** by two writers, never clobbered. The webhook seeds the checkout breakdown (`subtotal_cents`, `tax_cents`, `shipping_cents`, `discount_cents` — all captured from `payload.subtotal_price` / `payload.total_tax` / `payload.shipping_lines[].price` (falling back to `payload.total_shipping_price_set`) / `payload.total_discounts`; Phase 2 of [[../specs/shopify-order-confirmation-emails]]). [[../libraries/fraud-detector]] later merges the card fingerprint (`gateway`, `card_bin`, `card_last4`, `card_company`, `card_name`, `card_exp`). Any writer touching this column MUST spread the existing value first (`{ ...existingPd, …new fields }`) — a bare object write erases the other side's fields. Subscription-renewal rows have this null (Appstle bypasses the Shopify checkout breakdown).
- `financial_status`: **mixed-case in production data** — both `"PAID"` (94% of rows, from Shopify webhook ingestion) and `"paid"` (6%, normalized) exist. Same for `"REFUNDED"`/`"refunded"`, `"PARTIALLY_REFUNDED"`/`"partially_refunded"`, `"PENDING"`/`"pending"`. Use `ILIKE` or `.in("financial_status", ["PAID","paid"])`. Don't use `.eq("financial_status", "paid")` — you'll miss 94% of rows. **Writers.** Two paths write this column: [[../libraries/shopify-sync]] + [[../libraries/shopify-webhooks]] `handleOrderEvent` (vendor-authoritative for Shopify orders) and [[../libraries/refund]] `refundOrder` (engine-independent backstop, derives the value from the [[../tables/order_refunds]] ledger post-refund and never regresses a stronger vendor-stamped state — see [[../libraries/refund]] § Writer contract for `orders.financial_status`). Before that backstop existed, in-house internal orders that were refunded still read as paid because there was no sync ([[../specs/a-refunded-order-must-say-it-was-refunded]]).
