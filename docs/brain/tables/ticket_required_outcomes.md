...
5. **`pending`/`failed` → `verified` (CS Director rescue reconcile)** — when a CS Director approve_remedy fires a rescue action that satisfies an earlier still-open or terminal-failed row, [[../libraries/honor-required-outcomes]] `reconcileRequiredOutcomesForFiredActions` re-runs `verifyActionInDB` against the row's own action shape. If the live predicate holds, `markOutcomeVerified` CAS from `pending` OR `failed` → `verified` and stamps `verified_at`. NEVER re-dispatches — the reconciler only READS live DB state, so a row whose predicate does not hold is left in its prior terminal state. Ground-truth: ticket cc78ad94 (Sol's `create_return` failed on the since-fixed order-level coupon ceiling; June's approve_remedy fired the same kind successfully; before this path, the failed row kept re-escalating the fully-resolved ticket on every completion-gate tick).
