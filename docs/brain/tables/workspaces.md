...
| `media_buyer_digest_enabled` | `bool` | — | default: `true`. When `false`, suppresses the media-buyer (Bianca) Slack digest Max posts to `#director-growth-max` after each media-buyer pass (`deliverMediaBuyerDigest` early-returns) — does NOT affect the media-buyer pass, meta_insights sync, or any other Max post. Set `false` for Superfoods (founder silenced the ~2-hourly digest, mb-digest-workspace-toggle). |
| `ads_supervisor_digest_enabled` | `bool` | — | default: `true`. Sibling of the above for the ads-supervisor drift digest (the 3h "N drift issue(s) … Dahlia's bin is thin" report Max posts to `#director-growth-max`). When `false`, `deliverAdsSupervisorDigest` early-returns — the pass / drift detection / fix-spec authoring still run, only the Slack post is skipped. Set `false` for Superfoods (founder silenced the thin-bin noise while Dahlia is held OFF, ads-supervisor-digest-toggle). |
