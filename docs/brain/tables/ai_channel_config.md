...
| `sol_playbook_selection_active` | `bool` | — | default: `false` · Phase 3 of [[../specs/sol-session-chosen-playbook-selection-retire-brittle-triggers]] — per-channel opt-in for the Sol-chosen playbook branch in [[../inngest/unified-ticket-handler]] `routeExec` § 2a. When **true**, Sol's live [[ticket_directions]] row names the playbook via `plan.playbook_slug` and `startPlaybook` is called directly (ledger `reasoning='sol:session-chose-playbook:{slug}'`). When **false** — the safe-rollout stance — the deterministic signal matcher at § 2b still owns the playbook start (ledger `reasoning='sol:matcher-chose-playbook:{slug}'`) EVEN IF `sol_first_touch_enabled=true`, so a workspace can have Sol author Directions without giving Sol dispatch authority. Rolls back per-channel with a single UPDATE. |
