...
| `status` | `text` | — | default: `'draft'` · live values: `draft` \| `ready` \| `archived` (probe 2026-07-13). `archived` is the retire state — set when an ad's landing URL is removed (retiring/URL-removed). The [[../libraries/ready-to-test]] reader `.neq('status','archived')`s + guards the row-loop so retired creatives never count toward Dahlia's bin depth, /director-training, or media-buyer replenish. Historical values `rendering` \| `failed` appear in older pipeline records but are no longer produced. |
| `created_by` | `uuid` | ✓ |  |
