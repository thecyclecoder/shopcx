...
- **Books don't finish closing until ~the 15th.** `lastClosedMonths` treats last month as "closed" the moment the calendar rolls over, but QuickBooks' month-end entries (inventory/COGS) don't fully land until ~the 15th of the following month. So a snapshot pulled **before the 15th** is premature. The refresh jobs run on the **16th** ([[../inngest/qb-snapshot-refresh]]) and **20th** ([[../inngest/investor-monthly-invite]]) precisely to be past that close.
- **Late entries mutate already-closed months — so re-grab, don't just append.** Entries post after the fact and quietly change historical months. The rule: **every refresh re-pulls the trailing 6 closed months** and upserts each in place (`backfillPnlSnapshots(ws, 6)`), so a month keeps getting corrected for ~6 months, then ages out. Never build a "snapshot only the newest month" appender — it would freeze stale numbers.
