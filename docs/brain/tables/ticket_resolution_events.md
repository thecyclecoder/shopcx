...
- **Selective-clarify rate tile** ([[../libraries/selective-clarify]] — Phase 2 of [[../specs/confidence-gated-problem-lockin-and-selective-clarify]]) — `/dashboard/tickets/analytics` reads a 7-day rolling window on `verified_outcome` via `/api/tickets/analytics/selective-clarify` and renders the `clarified / total` rate against the ~6% target the parent goal sets.
...
3. **Verify (`verified_at` + `verified_outcome`)** — one row-terminal stamp:
   - **`direct_action`** stamps its own verdict from `verifyActionInDB`: `confirmed` when every action verified (or self-heal retried cleanly); `drifted` when a claim couldn't be backed by a DB read. This stamp fires FIRST — before the return-time stamp — so it wins the idempotent-once compare-and-set on `verified_at IS NULL`. The selective-clarify gate ([[../libraries/selective-clarify]] — Phase 2 of [[../specs/confidence-gated-problem-lockin-and-selective-clarify]]) fires EARLIER still, at the top of the branch: on a low-confidence × irreversible plan it sends a scoped confirmation-turn instead of running any action and stamps `verified_outcome='clarified'`.
   ...
   - **The inline verify block — [[../libraries/sol-outcome-claim-guard]]** (Phase 3 of [[../specs/eliminate-false-promises-no-claim-ships-until-executed-and-verified]]) — sets `verified_outcome='unbacked'` when the response is rejected before it ships. The send guard's `assertClaimsBackedByOutcomes` compares each CLAIM the reply asserts against the ticket's [[ticket_required_outcomes]] rows; on any claim whose backing row isn't `status='verified'`, the reply is BLOCKED and `stampUnbackedOnLedger` compare-and-sets this stamp on `verified_at IS NULL` when the caller passes the turn's `resolution_event_id`. A Phase-0-blocked run leaves `shipped_at NULL` + `verified_outcome='unbacked'`.
