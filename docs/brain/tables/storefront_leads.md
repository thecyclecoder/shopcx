...
| `sms_status` | `text` | ✓ | Latest Twilio delivery status: queued/sent/delivered/undelivered/failed. Updated by [[../integrations/twilio]] marketing-status webhook — the popup send passes that route as an explicit `StatusCallback` (direct from the short code, no Messaging Service), without which this stays frozen at `queued` (ticket 8e9e325e). Legacy stuck rows reconciled by `scripts/backfill-popup-sms-status.ts`. |
