...
| `worker_kind` | `text` | — | the worker (= `agent_jobs.kind`, e.g. `build`/`repair`/`db_health`) — **denormalized** so the last-10 rollup is a single index scan, never a join |
...
**Indexes:** `(workspace_id, created_at desc)` (report ordering); `(workspace_id, worker_kind, created_at desc)` (the last-10 rollup + trend per worker — [[../libraries/worker-grader]] `computeWorkerRollup`).

## Foreign keys

**Out:** `workspace_id` → [[workspaces]].id · `agent_job_id` → [[agent_jobs]].id · `overridden_by` → `auth.users`.id.

**In:** [[worker_grader_prompts]].`derived_from_grade_id` → this · `derived_from_job_id` → [[agent_jobs]].id.
...
- **Per-worker rubric.** Each `worker_kind` is scored against its own rubric (the spec's locked config, in [[../libraries/worker-grader]] `WORKER_RUBRICS`) — a `build` on spec-phases-satisfied/`tsc`-clean/clean-merge, a `repair` on real-root-cause/fix-held, etc. Only rubric-backed kinds are graded (the Director's own + non-worker kinds are excluded).
- **Human-overridable.** An override sets `graded_by='human'` + `overridden_by`; the agent never re-writes a human grade. A large override gap proposes a [[worker_grader_prompts]] calibration rule.
- **Idempotent.** One grade per concluded job (the unique on `agent_job_id`) — a re-grade updates in place.
- **The grade only recommends.** The rollup drives a *coaching* trigger ([[../libraries/worker-grader]] `detectGradeDropCoaching` → [[../libraries/worker-instructions|coachWorker]]), not a self-promotion — the Director coaches; the leash changes only by CEO confirmation ([[../operational-rules]] § North star).

## RLS
Authenticated SELECT (the worker-profile report is owner-gated above the DB), service-role write — mirror [[worker_coaching_log]] / [[director_decision_grades]].

## Migration
`supabase/migrations/20260705120000_worker_action_grades.sql` (apply: `npx tsx scripts/apply-worker-action-grades-migration.ts`). Idempotent. Creates this + [[worker_grader_prompts]].

---

[[../README]] · [[worker_grader_prompts]] · [[agent_jobs]] · [[worker_coaching_log]] · [[director_decision_grades]] · [[../libraries/worker-grader]] · [[../specs/worker-grading-and-director-management]] · [[../goals/devops-director]] · [[../../CLAUDE]]
