...
| `user_id` | `uuid` | ✓ |  |
| `type` | `text` | — | CHECK-constrained. Valid: `macro_suggestion`, `pattern_review`, `knowledge_gap`, `system`, `fraud_alert`, `chargeback_alert`, `duplicate_order_alert`, `escalation_gap`, `agent_approval_request`, `agent_message`, `agent_daily_summary`, `return_request`, `mario_accuracy_alarm`, `refund_drift`. **Inserting an unlisted type → PostgREST 400 (23514)**; most inserts are fire-and-forget, so a bad type silently drops the notification. |
| `title` | `text` | — |  |
