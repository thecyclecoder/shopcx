...
- **Amazing Coffee — storefront supply tiers vs flat subscription pricing (+ 8-week interval cap) (category='rule', sort_order 25)** — durable ground truth so the orchestrator stops conflating the storefront's quantity-supply pricing (30/60/90-day tiers = 1/2/3 bags at 25%/31%/34% off) with the subscription surface's FLAT per-bag pricing (25% off, `$59.96`/bag; 3 bags = `$179.88`, not the storefront's `$158.30`), and stops offering an unpurchasable 90-day / quarterly cadence (8 weeks / every 2 months is the maximum). Also encodes the supply-size-vs-delivery-interval framing: bag count per shipment and cadence are separate levers, and a "3 bags every 90 days" ask fuses a one-time bulk-order price shape with a cadence we don't offer — resolve it honestly (verify the sub's actual per-line price, offer 3 bags @ 8-week cadence, or route the storefront bulk price as a ONE-TIME order). Derived from ticket `b28e7744-0451-46eb-a675-33836b96e491` (Juana Webman, `$2233` LTV) where the AI first denied tiered pricing existed and fabricated her account history, then over-corrected by confirming `$158.30` as a recurring subscriber rate. Seeded in `supabase/migrations/20261217120000_seed_amazing_coffee_supply_tier_pricing_prompt.sql`. **Tenant scope:** this content is Amazing-Coffee-merchant-specific, so the migration INSERTs it into ONLY the workspace that owns the source ticket — a CTE resolves `owner (workspace_id, ticket_id)` from `public.tickets WHERE id = b28e7744-…` and every column (`workspace_id`, `derived_from_ticket_id`, the `NOT EXISTS` idempotency scope) is derived from that one row; if the ticket isn't present (fresh env / other database) the CTE returns zero rows and the INSERT is a no-op. Never fan out across `workspaces` for a merchant-specific rule — `sonnet_prompts` is workspace-scoped at read time (both the orchestrator loader and the Settings → AI → Prompts surface filter by `workspace_id`), so an all-workspaces seed would surface a foreign merchant's rule as a tenant's own. See [[../lifecycles/ai-analysis]] and [[../orchestrator-tools]] § `get_product_knowledge`.
