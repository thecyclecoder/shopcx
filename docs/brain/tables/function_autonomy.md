...
**Seeded ALL-OFF** — today's reality: no director is automated, so every approval routes to the one CEO inbox (fail-safe: an unconfigured / partially-configured org never silently auto-approves). The workspace owner toggles a function on from the [[../dashboard/agents|Agents hub]] (`POST /api/developer/agents/autonomy`).
