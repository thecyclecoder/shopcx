...
**Logistics Director seat scaffolded 2026-07-12** ([[../specs/marco-logistics-director-seat]] Phase 3, via `scripts/apply-logistics-director-function-autonomy-seed-migration.ts` → migration `20261016130000_logistics_director_function_autonomy_seed.sql`). The `logistics` row lands at `(live=false, autonomous=false)` (`updated_by='marco-logistics-director-seat Phase 3 scaffold — read-only observer'`). Marco lands as a **read-only observer** — his `LEASH_CATEGORIES` is intentionally empty ([[../libraries/logistics-director]]) because the storefront-availability lever has no callable server-side helper and [[../functions/logistics]] § "Provenance / build model" flags the whole tooling as off-limits to Ada by founder directive 2026-07-10 (a deliberate bounded exception to "Ada is the sole builder"). The M3 dispatch escalates every logistics pending_action to the CEO. When [[../specs/marco-logistics-executor-surface]] opens the two callable executors, the CEO can flip `logistics` live+autonomous from the Agents hub and Marco (persona 📦 **Marco**) starts auto-deciding routed Logistics approvals within his (then-populated) leash. Compare-and-set on the audit stamps — never demotes an already-activated director.
