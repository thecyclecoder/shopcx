...
**Logistics Director seat scaffolded 2026-07-12** ([[../specs/marco-logistics-director-seat]] Phase 3, via `scripts/apply-logistics-director-function-autonomy-seed-migration.ts` → migration `20261016130000_logistics_director_function_autonomy_seed.sql`). The `logistics` row lands at `(live=false, autonomous=false)`. Marco landed as a **read-only observer** (empty `LEASH_CATEGORIES` per [[../libraries/logistics-director]]), then flipped to a **LIVE leash-bound director** with [[../specs/marco-logistics-executor-surface]] Phase 2 (2026-07-12) — his `LEASH_CATEGORIES` now carries `availability_toggle_within_crisis_lever` + `auto_readd_swapped_subscribers_within_crisis_cohort` (both crisis-cohort scoped, gated by the M3 dispatch's `requireCrisis()` guard). The row is still at `(live=false, autonomous=false)` — Marco's capability is open (the two categories are callable), but the CEO has not flipped the `live` switch yet, so all pending_actions still require explicit CEO approval before firing (no autonomous execution yet, only in-leash auto-approve). When the CEO flips `logistics` live+autonomous from the Agents hub, Marco (persona 📦 **Marco**) starts auto-deciding within his leash. Compare-and-set on the audit stamps — never demotes an already-activated director.
...
- `supabase/migrations/20260917120000_cs_director_function_autonomy_seed.sql` ([[../specs/cs-director-persona-and-org-placement]] Phase 2) — compare-and-set audit stamp for the `cs` row (dormant safest-leash landing).
- `supabase/migrations/20261016130000_logistics_director_function_autonomy_seed.sql` ([[../specs/marco-logistics-director-seat]] Phase 3) — inserts the dormant `logistics` row (`live=false, autonomous=false`) that the initial 2026-07-01 seed did not include; compare-and-set audit stamp; apply: `scripts/apply-logistics-director-function-autonomy-seed-migration.ts`.
