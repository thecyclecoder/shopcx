...
| `email_canonical` | `text` | ✓ | Gmail-canonical form of `email` — the shared identity key inbound-email ingest ([[../../../src/app/api/webhooks/email/route.ts]]) + [[../libraries/account-matching]] use to resolve dot/plus variants of the same real inbox to the same row (identity-gmail-canonicalization-and-dot-insensitive-matching Phase 2). Populated automatically by the `customers_email_canonical_trigger` BEFORE INSERT/UPDATE OF email trigger via the SQL `public.canonicalize_email(text)` function (mirrors `canonicalizeEmail` in [[../libraries/email-utils]]); NEVER written by app code (single source of truth is the trigger). Backed by partial composite index `idx_customers_email_canonical (workspace_id, email_canonical) WHERE email_canonical IS NOT NULL` — matches the (workspace_id, email) shape callers already use. NULL only when `email` itself is NULL. |
...
- **`comp_role` also gates the investors area.** The `investor` + `owner` roles double as the allowlist for the magic-link financial portal at `/investors` ([[../lifecycles/investors-area]]) — one flag, two uses (free product AND financial-portal access). Flip `comp_role` to null to revoke both. The monthly [[../inngest/investor-monthly-invite]] emails/texts exactly this set.
...
- **`email_canonical` is trigger-populated — never written from app code.** The `customers_email_canonical_trigger` (migration `20261104120000`) fires BEFORE INSERT and BEFORE UPDATE OF email, setting `NEW.email_canonical := public.canonicalize_email(NEW.email)`. That SQL function mirrors [[../libraries/email-utils]] `canonicalizeEmail` — Gmail dots and +tags collapse, `googlemail.com` normalizes to `gmail.com`, every other domain returns trimmed+lowered unchanged. Because every write goes through the trigger, no new customer-insert call site can drift out of sync — this is the defense-in-depth chosen over touching each of the ~8 insert sites (email webhook, widget, lead, checkout, checkout/identify, tickets, appstle, shopify-sync). Wedge: ticket `54f0f29e` (Julie Metz) spawned an empty shadow because inbound-email ingest looked up by exact string; identity-gmail-canonicalization Phase 2. Idempotent inline UPDATE in the migration + ship-time-backfill `scripts/_backfill-customers-email-canonical.ts` (auto-ledgered to [[data_op_runs]] by `detectAndEscalateShipTimeBackfills`) cover historical rows.
