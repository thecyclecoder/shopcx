...
- **Explore/exploit split on crown** ([[../specs/media-buyer-explore-exploit-split-on-crown]] Phase 1 + Phase 2 + Phase 3 — WIRED) — the exploit-tracking columns (`exploit_spawned` / `exploit_hits` / `exploit_exhausted` / `exploit_exhausted_at`) drive Dahlia's per-product 2-explore / 2-exploit shift after crown. Phase 2 (agent.ts replenish path) reads `listActiveWinnersForProduct` for the product, sorts best-CAC-first, and allocates the 2 exploit slots 1-per-winner via `amplifyWinner`; each spawn calls `incrementExploitSpawned(sourceWinner, N)` — the STRIKE counter (clones spawned since the last hit). Phase 3 (agent.ts `creditExploitHits`, called just before the pass heartbeat) attributes each exploit-origin test verdict back to its source winner: for every uncredited exploit `ad_campaigns` row ([[ad_campaigns]] `is_exploit=true` + `source_crowned_adset_id IS NOT NULL` + `exploit_hit_credited_at IS NULL`), resolve its live Meta adset via [[ad_publish_jobs]] and its lifetime CPA via `meta_insights_daily`; a [[../libraries/testing-results-sdk]] `tierForTest` verdict of `promising | crown` ⇒ compare-and-set `ad_campaigns.exploit_hit_credited_at = now()` (double-credit gate) AND `recordExploitHit(sourceCrownedAdsetId)` — the winner's `exploit_hits` bumps, `exploit_spawned` resets to 0, `exploit_exhausted` + `exploit_exhausted_at` clear. A `dud`/`testing` outcome adds no reset — the clone's spawn already counted a strike. Every hit writes one `media_buyer_exploit_hit_credited` [[director_activity]] row. At `exploit_spawned ≥ EXPLOIT_EXHAUST_STRIKES=4` with `exploit_hits = 0` (never hit), `incrementExploitSpawned` flips `exploit_exhausted=true` and stamps `exploit_exhausted_at`; the winner drops out of `listActiveWinnersForProduct` and Phase 2 stops allocating it exploit slots. When ALL a product's winners are exhausted the reader returns empty ⇒ Phase 2 reverts to 4-explore / 0-exploit. A NEW crown on a different test adset re-arms exploit fresh via its own row (own counters).
