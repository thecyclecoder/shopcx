...
| `phase_states` | `jsonb` | **DEPRECATED** (2026-06-24 — [[../specs/spec-readers-from-db-retire-parser]]): per-phase status is now **authoritative in `[[../tables/spec_phases]]` table** (`index`, `status`, `pr?`, `merge_sha?`). This column is retained for builds + one-shot specs (`flags.merged_pr` for one-shot). Multi-phase specs read status from `public.spec_phases` directly; `phase_states` here is no longer authoritative — kept only to avoid breaking legacy reads during retirement [[../specs/spec-fold-from-db-row]]. |
| `flags` | `jsonb` | board flags: `{ deploy_pending?, blocked?, critical?, deferred?, short_circuit?, short_circuit_reason?, merged_pr? }` · default `{}` · merge-patched on write. `critical` + `deferred` are spec-level state. `flags.deferred=true` wins over `status` for display. `flags.merged_pr` ([[../specs/spec-status-phase-pr-provenance]]) carries the card-level shipping PR for a **one-shot spec** (no phases). |
