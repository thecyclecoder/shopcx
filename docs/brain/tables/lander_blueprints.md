...
- **Target product is category-matched per teardown ([[../specs/cleo-blueprint-product-matching]] Phase 1).** [[../libraries/cleo-blueprint]] `matchProductToTeardown` scores each active product by head-noun + title/handle token overlap on the teardown's `brand` + `funnel_type` + `strategy` + `transferable_pattern` blob; `null` score → SKIP (we don't sell that category — never blueprint the wrong product). Replaces the pre-Phase-1 first-active-product punt.
- **AT MOST ONE blueprint per `(product_id, funnel_type)` — dedup gate.** [[../libraries/lander-blueprints]] `hasBlueprintForProductType(workspaceId, productId, funnelType)` is the DB-side gate the sweep checks alongside a within-sweep `Set` before every `createBlueprint`. There is no DB unique constraint (a blueprint can be `rejected` and re-authored) — the invariant is caller discipline in [[../libraries/cleo-blueprint]] `runCleoBlueprintSweep`. Two advertorial teardowns pointing at Amazing Coffee → ONE advertorial blueprint.
