...
- `supabase/migrations/20260728140000_specs_status_nullable_derived.sql` (`specs-status-override-only`) — makes `specs.status` NULLABLE + allows `NULL` in the CHECK so "purely derived" is expressible as `NULL`. The override-only invariant is enforced in the writers (`dualWriteSpecRow`/`setSpecStatus`/`upsertSpec` clear a derived destination to `NULL`); the one-off `scripts/cleanup-derived-spec-status.ts` nulled the 2 pre-existing bugged rows (`noop-pipeline-test-4`/`-5`) · apply: `scripts/apply-specs-status-nullable-migration.ts`
