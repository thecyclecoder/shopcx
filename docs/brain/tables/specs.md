...
| `status` | `text?` (NULLABLE) | `NULL ｜ in_review ｜ deferred ｜ folded` (the CHECK still ACCEPTS `planned`/`in_progress`/`shipped` for legacy rows, but the writers NEVER persist them). **OVERRIDE-ONLY** (`specs-status-override-only`): holds an EXPLICIT lifecycle override (`in_review` / `deferred` / `folded`) or **`NULL`** when status is PURELY DERIVED from `spec_phases`. A DERIVED state (`planned`/`in_progress`/`shipped`) must NEVER be stored — the writers clear it to `NULL` — see Derived status |
...
`specs.status` is no longer trigger-maintained — the `spec_phases_rollup` trigger on [[spec_phases]], the `specs_deferred_rollup` column trigger on this table, and `roll_up_spec_status` were all dropped in `derive-rollup-status` P3 (migration `20260725160000`). The stored column is **OVERRIDE-ONLY** (`specs-status-override-only`, migration `20260728140000`): it holds ONLY the explicit lifecycle overrides (`in_review` / `deferred` / `folded`) or `NULL`. A DERIVED state (`planned`/`in_progress`/`shipped`) is **NEVER persisted** — the writers actively clear it to `NULL` so the readers compute it from the phases. (The column was made NULLABLE in `20260728140000`; `NULL` is the canonical "no override → purely derived" value. The CHECK still ACCEPTS the legacy derived literals for backward-compat, but no writer emits them.) The planned/in_progress/shipped axis is DERIVED at read time by [[../libraries/brain-roadmap]] `deriveStatus` / `rollupPhaseStatus`:
...
Because the deriver always prefers the phase rollup (or, for a one-shot, the merge-provenance derivation), a stale `shipped`/`planned` on the stored column is never displayed — closing the [[../specs/spec-review-agent]] "shipped with 1 phase" class AND the one-shot "shipped-but-reads-planned" leak at READ time. `specs-status-override-only` closes it at the WRITE side too: the single dual-write choke point ([[../libraries/spec-card-state]] `dualWriteSpecRow`, via `isOverrideStatus`), `setSpecStatus`, and `upsertSpec` all map a derived destination to `NULL`, so a derived state can no longer be persisted at all. The disposition lane ([[../libraries/spec-card-state]] `applyAdaDisposition`) flips a spec OUT of `in_review` by CLEARING the override to `NULL` on `planned` (purely derived) and storing `'deferred'` only when the real `deferred` override is chosen. The bug this fixed: `noop-pipeline-test-4` carried a stored `status='planned'` (a derived state) because the old disposition wrote the literal instead of clearing the override.
