...
- [[../libraries/subscription-cycle-charge-claim]] `claimCycleCharge` (the SDK write path) and `readCycleCharge` (the 23505-branch lookup + diagnostic helper).
- [[../libraries/portal__order-now-guard]] `guardInternalOrderNow` — Phase 1 of [[../specs/a-subscription-is-never-more-than-one-cycle-behind]]. Refuses a portal order-now press when a row for the sub is unresolved OR was claimed within a 15-minute window. Ground truth 2026-09-23: three presses on one sub in twelve minutes billed three cycles ($351.71) — this guard reads the ledger to close that hole BEFORE the renewal event is fired.
...
## What the ledger does NOT catch — the multi-cycle catch-up burst

Phase 2 of [[../specs/a-subscription-is-never-more-than-one-cycle-behind]] fixes a class the per-cycle unique index cannot catch: when a sub falls several cycles behind, each successful charge USED to advance `next_billing_date` by exactly one cycle from the OLD stored date, so the daily cron re-picked the sub the next day and charged it again. Each catch-up charge carried a DIFFERENT `cycle_key`, so this ledger's uniqueness never collides — measured 2026-09-24: zero same-cycle duplicates across the whole ledger, and yet six multi-charge bursts on single subs totalling $1,232.70 (one customer was owed a $179.70 refund). The Phase 2 fix is the shared [[../libraries/subscription-billing-date]] `advanceToNextFutureBillingDate` helper used at all three advance sites in [[../inngest/internal-subscription-renewals]] — it skips whole cycles until strictly future, so a backlog is skipped rather than billed out one cycle per day. Skipped cycles are NOT written to this ledger anywhere; a cycle only earns a row when it actually charges.
