...
When a subscription falls several cycles behind, the billing-date advancement strategy prevents multi-cycle catch-up bursts: each successful charge advances `next_billing_date` to the next cycle in the **future** (not one cycle from the OLD stored date), so the daily cron skips the backlog rather than picking up the sub repeatedly. This breaks the pattern where each catch-up charge carried a DIFFERENT `cycle_key`, allowing this ledger's uniqueness to stay clean — measured 2026-09-24: zero same-cycle duplicates across the whole ledger, and yet six multi-charge bursts on single subs totalling $1,232.70 (one customer was owed a $179.70 refund) WITHOUT the fix. The helper [[../libraries/subscription-billing-date]] `advanceToNextFutureBillingDate` is used at all three advance sites in [[../inngest/internal-subscription-renewals]] — it skips whole cycles until strictly future, so a backlog is skipped rather than billed out one cycle per day. Skipped cycles are NOT written to this ledger anywhere; a cycle only earns a row when it actually charges.
