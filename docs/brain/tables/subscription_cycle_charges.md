...
3. **UPDATE → `failed`** — on Braintree decline, `resolve-cycle-claim-failed` stamps status + `transaction_id` + `resolved_at`. Dunning's `resetBillingDateAfterDunning` moves `next_billing_date` forward on decline, so its retry usually lands on a NEW `cycle_key`. But when dunning skips the order and the retry lands BACK on the same `cycle_key` (the wedge case that shipped [[../specs/failed-cycle-charge-claim-must-not-wedge-order-now-and-renewal-retries]] — ticket `cce7d76b`), a `failed` row is NOT a permanent block: the SDK's `claimCycleCharge` atomically resets a `failed` row back to `in_flight` under the new claimant on the 23505 conflict, so the same `cycle_key` is re-claimable. Only `in_flight` and `succeeded` rows refuse a new claimant.

The UPDATE branches to terminal statuses (`succeeded` / `failed`) are compare-and-set on `status='in_flight'` so a stale duplicate cannot overwrite a real outcome. The failed→in_flight RESET is compare-and-set on `status='failed'` so a concurrent thread that already reset it does not get double-reset, and any concurrent `succeeded` outcome that landed first blocks the reset (fall-through refusal).
