...
| `superseded_claims` | `jsonb` | — | default `[]` · newest-first array of prior claim snapshots on THIS row (`{ claimant, status, source, transaction_id, order_id, claimed_at, resolved_at, superseded_at, superseded_reason }`). Written by [[../libraries/subscription-cycle-charge-claim]] `claimCycleCharge` when a new claimant reclaims a prior `failed` OR stale `in_flight` row. Query `jsonb_array_length(superseded_claims) > N` to surface subs whose current cycle has been reclaimed many times. Phase 1 of [[../specs/a-declined-renewal-must-not-wedge-the-cycle-forever]] |
...
3. **UPDATE → `failed`** — on Braintree decline, `resolve-cycle-claim-failed` stamps status + `transaction_id` + `resolved_at`. Dunning's `resetBillingDateAfterDunning` moves `next_billing_date` forward on decline, so its retry usually lands on a NEW `cycle_key`. But when dunning skips the order and the retry lands BACK on the same `cycle_key` (the wedge case that shipped [[../archive.d/failed-cycle-charge-claim-must-not-wedge-order-now-and-renewal-retries]] — ticket `cce7d76b`), a `failed` row is NOT a permanent block: the SDK's `claimCycleCharge` atomically resets a `failed` row back to `in_flight` under the new claimant on the 23505 conflict, so the same `cycle_key` is re-claimable. A **stale `in_flight`** row — one older than `STALE_IN_FLIGHT_RECLAIM_MS` (10 min) — is reclaimable the same way (a crashed prior attempt would wedge the cycle indistinguishably from a `failed` one — [[../specs/a-declined-renewal-must-not-wedge-the-cycle-forever]] Phase 1). Only `succeeded` and **fresh** `in_flight` rows refuse a new claimant.

The UPDATE branches to terminal statuses (`succeeded` / `failed`) are compare-and-set on `status='in_flight'` so a stale duplicate cannot overwrite a real outcome. The reclaim reset is compare-and-set on the prior row's exact `status` AND `claimed_at` so a concurrent thread that already reset it does not get double-reset, and any concurrent outcome (a `succeeded` landing between the SELECT and the UPDATE, or a fresh in-flight attempt that just re-stamped `claimed_at`) blocks the reset (fall-through refusal). Every reset prepends a snapshot of the prior claim to `superseded_claims` so the reclaimed row still visibly carries its history — a repeatedly-declining sub does not silently look identical to a first-time attempt.
