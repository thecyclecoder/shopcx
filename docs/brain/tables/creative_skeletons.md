...
| `winner_tier` | `text` | ✓ | **winners-flow LANE A only.** AdLibrary's winner tier for this concept — `high_confidence_winner` \| `winner` \| `middle` \| `loser` \| `emerging`. Stamped by `ingestAd`'s `winner` arg from `scanWinners`. Null for LANE-B (domain-search) + legacy rows. Migration `20261022160000`. |
| `winner_score` | `numeric` | ✓ | **winners-flow LANE A only.** AdLibrary's composite score (0-1) for the concept. Ranking signal (best-first) when the scout caps how many statics to vision. Migration `20261022160000`. |
| `concept_tags` | `jsonb` | ✓ | **The unified concept breakdown** (winners-flow) — `{ angle, archetype, why_it_works, cialdini_lever, awareness_stage, format, variant_count? }`. LANE A fills it from AdLibrary's AI tags; LANE B + the backfill fill it from OUR vision (Phase 2c) using the SAME schema, so Dahlia researches + Max grades one shape across both collection lanes. Migration `20261022160000`. |
