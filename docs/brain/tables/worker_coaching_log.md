...
Surfaced on the worker's **profile page** (`/dashboard/agents/[role]` → "Coaching history") and (as a board `update`) on the [[../libraries/director-board|#directors board]]. Written via [[../libraries/worker-instructions]] `coachWorker`; orchestrated by [[../libraries/worker-coaching]] `runWorkerCoachingPass`.
...
| `worker_kind` | `text` | the worker the message was sent **to** ([[agent_jobs]] kind) |
...
| `instruction_id` | `uuid` | FK → [[worker_instructions]]`(id)` — the amendment this logged (null for a route/escalation) |
...
- `worker_coaching_log_worker_idx` on `(workspace_id, worker_kind, created_at desc)` — the profile-page history read.
- `worker_coaching_log_class_idx` on `(worker_kind, error_class, created_at desc)` — attempt counting + re-check.

## Common queries

### A worker's coaching history (the profile page)
```ts
const { data } = await admin.from("worker_coaching_log")
  .select("*").eq("workspace_id", workspaceId).eq("worker_kind", "regression")
...
[[../libraries/worker-instructions]] · [[../libraries/worker-coaching]] · [[worker_instructions]] · [[director_activity]] · [[director_messages]] · [[../specs/worker-coaching-loop]] · [[../specs/repair-agent]] · [[../goals/devops-director]]
