...
**Why it exists.** The Bianca M4 north-star is that the scaler ships with its OWN shadow→armed arming — a scaler with no arming rail is exactly the ungradable autonomous-spend surface the goal names as the one thing it must not create. Three preconditions must all clear before the executor may move budget: (1) SCALE-vocabulary GRADED PASS RATE ≥ 0.8 (`overall_grade >= 7`) on ≥ 20 [[media_buyer_action_grades]] rows over 14d scoped to `action_kind IN ('media_buyer_promoted_winner', 'media_buyer_replenished_test_cohort')` — see [[../specs/cold-scaler-arming-decides-on-evidence-not-absence]] Phase 3, (2) at least 7 consecutive `band='green'` [[media_buyer_sensor_trust]] snapshots, (3) cold-scaler [[../libraries/blended-cac-ltv]] `cacLtvRatio ≥ 3` (or under the caller's target). Any failing predicate lands `allowed=false` + a structured `reasons` payload naming the branch that refused, and the gate escalates to the CEO via [[../libraries/platform-director]] `escalateDiagnosisToCeo` + writes a Growth-owned [[director_activity]] row (`action_kind='cold_scaler_arming_denied'`).

**Distinct from [[media_buyer_arming_authorization]]** — the sibling table authorizes the TEST cohort's shadow→armed flip; this one authorizes the COLD SCALER cohort's flip. Two tables, two independent authorizations — the test rail's arming decision does NOT imply the scaler rail's, and vice versa. Sensor-trust + CAC:LTV preconditions match; Precondition #1 differs on purpose. The TEST gate reads shadow-vs-review agreement (a test cohort still runs in shadow, so the sample is producible). The SCALER gate reads Bianca's live SCALE-vocabulary grade pass rate (Bianca stopped running in shadow weeks ago — the graded-outcomes corpus is what actually exists on Dylan's account today).

**Distinct from the neighbouring signals.** Keep them straight:

- [[media_buyer_action_grades]] is the per-decision graded outcome — INPUT to the SCALE-vocabulary pass-rate precondition (filtered by the loader to `action_kind IN SCALE_ACTION_KINDS`; KILL grades excluded).
...
- **Precondition #1 reads GRADED SCALE-vocabulary rows, not shadow reviews.** [[../specs/cold-scaler-arming-decides-on-evidence-not-absence]] Phase 3 retired the shadow-review branch and its `metadata.surface='cold_scaler'` discriminator. The loader now filters [[media_buyer_action_grades]] to `action_kind IN ('media_buyer_promoted_winner', 'media_buyer_replenished_test_cohort')` and joins to `director_activity.metadata` only for the per-account scope. KILL grades (`media_buyer_paused_loser`) are filtered out at both the DB loader AND inside the pure gate so Bianca's ~97%-sound kill skill cannot vouch for her ~36%-sound promote skill.
...
[[workspaces]] · [[meta_ad_accounts]] · [[media_buyer_cold_scaler_cohorts]] · [[media_buyer_arming_authorization]] · [[media_buyer_action_grades]] · [[media_buyer_sensor_trust]] · [[director_activity]] · [[../libraries/media-buyer__cold-scaler-arming-gate]] · [[../libraries/media-buyer__arming-gate]] · [[../libraries/blended-cac-ltv]] · [[../specs/bianca-cold-scaler-arming-gate-shadow-to-armed]] · [[../specs/cold-scaler-arming-decides-on-evidence-not-absence]] · [[../goals/bianca-temperature-aware-campaign-structure]] · [[../functions/growth]] · [[../operational-rules]] (§ North star — supervisable autonomy)
