...
**Second incident — 2026-08-24 yvette SC126000 (spec: [[../specs/remedy-state-must-see-out-of-band-refunds]] Phase 3).** The `returns` policy row's authoritative rule `returns.refund_formula` says `net_refund = order_subtotal - label_cost` and explicitly excludes Shipping Protection, customer-paid shipping and the label cost — but the return-creation path (`computeReturnNetRefundCents`) had been reading the order TOTAL (tax + shipping folded in). yvette's MBG return was promised $55.86 while the policy sanctioned $50.54. **The code must match the rule, not the other way round.** Phase 3 renamed the compute input from the total-based name to `orderSubtotalCents` and pinned the derivation in one exported helper (`deriveOrderSubtotalCentsFromLines` in [[../libraries/shopify-returns]]) so a future refund path cannot silently reintroduce the total.
