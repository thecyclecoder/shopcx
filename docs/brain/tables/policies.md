...
- **Subscription pricing / 50%-MSRP floor:** the floor (a price the cleanup raised everyone to) is the rail [[../libraries/subscription-overcharge]] clamps an INFERRED baseline to — a baseline the detector CALCULATES from catalog rules when no direct evidence of what the customer paid exists. **CEO ruling 2026-08-01 — `pricing.historical_rate_beats_floor` (in the Subscription policy's `rules[]`):** a customer's DEMONSTRATED historical rate — a rate their own renewal history shows they were charged over a sustained window — is honoured over the floor, so a restore below the floor is in-policy when the demonstrated rate falls below it. Grounding: 57 subscriptions were restored below the floor on 2026-08-01, each with 12–29 renewals at exactly that rate; Vicki's four consecutive renewals at $24.95 (spec: [[../specs/june-restores-a-grandfathered-price-without-escalating]]) are the derived-from case. The rule is the machine-readable half; the prose sits alongside it in `internal_summary`. A raise attempt still refuses via `isRaiseAttempt` in `subscription-overcharge.ts:503` — lowering toward a demonstrated rate is in-policy, ANY write that would raise a customer's realized price is out.
