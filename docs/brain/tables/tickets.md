...
| `ai_handled_at` | `timestamptz` | ✓ | The UNIVERSAL "we responded to the customer" signal — stamped in [[../libraries/ticket-delivery]] `deliverTicketMessage` on every real (non-sandbox) customer-facing AI send, regardless of tier (Sonnet/Haiku orchestrator, Sol, journey, playbook all deliver through there). [[../inngest/ticket-analysis-cron]] `passesCoraSelectionGate` selects on `ai_handled_at IS NOT NULL OR sol_handled_at IS NOT NULL` so Cora grades the LOW-COST autonomous path too, not just Sol sessions (`sol_handled_at` is the Sol-specific sub-flag; the handling anchor is the later of the two). Backfilled for the trailing 7 days from the latest outbound AI/system-external message. An outbound-only ticket (a dunning email) may carry a stamp, but the grader also requires a customer message, so it stays excluded. See [[../specs/cora-grades-every-ai-handled-ticket-not-just-sol]]. |
| `sol_handled_at` | `timestamptz` | ✓ | The DETERMINISTIC "Sol handled this ticket" signal. Stamped by the WORKER (`scripts/builder-worker.ts` `runTicketHandleJob`) on the box session's terminal COMPLETED state via `createAdminClient()` — NOT by Sol's mid-session [[../libraries/ticket-directions]] `writeDirection` insert. Under a DB outage the in-session Direction insert could silently drop (observed on the first ~6-7 Sol-handled tickets), which hid "Sol responded" from Cora's feeder and starved [[../inngest/ticket-analysis-cron]] of tickets to grade. `sol_handled_at` is decoupled from that fallible per-turn insert. [[../inngest/ticket-analysis-cron]] `passesCoraSelectionGate` now selects on this column (`if (!ticket.sol_handled_at) return false`) and its `find-tickets` step filters `.not("sol_handled_at", "is", null)` at the source; per-cycle dedup compares `last_analyzed_at` vs `sol_handled_at`; the June-decided guard compares `director_activity.created_at` vs `sol_handled_at`. Idempotent-overwritable — a re-dispatched Sol turn advances the stamp to the latest handling. No default; historical rows without a Sol turn stay `null` (not selected). See [[../specs/cora-grades-on-deterministic-sol-handled-signal-not-brittle-direction-existence]]. |
