...
| `hard_gates` | `jsonb` | — | `{ no_fabrication:boolean, no_cold_offer:boolean, no_competitor_leak:boolean, render_ok:boolean }` — the per-check hard gates from the [[../../../.claude/skills/max-copy-qc/SKILL]] verdict. Open JSON (no CHECK) so a future gate lands without a migration; the .ts parser pins the required keys. |
...
| `declared_intent` | `jsonb` | ✓ | [[../specs/dahlia-researches-from-winners-flow-ad-library]] Phase 2 — the intent envelope Dahlia declared FIRST (Phase 1 `resolveResearchIntent`) and Max was graded AGAINST for this creative. Shape: `{ audience_temperature:'cold'\|'warm'\|'hot', purpose:'test-to-find-winner' }`. NULL for a legacy M1 verdict that never declared one. A PRESENT but malformed envelope fails closed. |
| `dahlia_rubric` | `jsonb` | ✓ | [[../specs/dahlia-researches-from-winners-flow-ad-library]] Phase 2 — Max's 5-axis rubric on the Dahlia creative, scored intent-aware. Shape: `{ competitor_selection:{score:int 1..10, reason:string}, temperature_selection:{...}, creative_quality:{...}, scroll_stopping:{...}, dr_consumer_psychology:{...} }`. **Advisory in Phase 2** — grade ledger only; Phase 3 wires the ready-to-bin threshold gate. NULL on a hard-gate fail (bounce is the signal — the rubric was not scored, same rule as `persuasion_rubric`) OR a legacy verdict. A PRESENT but malformed rubric (partial axis / out-of-range score / empty reason) fails closed with a per-axis reason. |
...
- **`dahlia_rubric` is INTENT-AWARE, not a Goodhart objective (Phase 2 rule).** Max is handed the same `declared_intent` Dahlia announced FIRST (via a TRUSTED CONTEXT block above the DATA fence — `renderMaxDahliaRubricTrustedContext` in [[../libraries/creative-qa]]), so a cold ad is graded as a cold ad, not blind. In Phase 2 the rubric is ADVISORY (grade ledger only); Phase 3 will wire the ready-to-bin threshold that reads this column to enforce a min composite / trigger a revise loop. Any Phase-2 code path reading this column MUST NOT gate campaign readiness on it — Phase 3 is the sanctioned reader.
