...
- **Reader:** [[../libraries/cold-scaler-cohort]] `getEffectiveMediaBuyerColdScalerCohort` (precedence), `getMediaBuyerColdScalerCohortById` (id lookup, scoped to workspace + active), and `listActiveColdScalerCohorts` (enumeration). The Bianca M4 follow-on specs — arming gate, CAC:LTV sensor, graduate-crowned-winners — all consume the SDK. **[[../specs/bianca-actually-graduates-crowned-winners-and-a-dead-meta-verb-cannot-fail-silently]] Phase 1** wires [[../libraries/media-buyer-agent]] `runGraduateForCrownedWinners` as the first real consumer of the mint → graduate → pause-source chain: for every crowned winner in a non-shadow pass whose product has an active cohort here, the runner calls `mintAndProvisionColdScalerCampaign` (idempotent) and `graduateCrownedWinnerToScaler` (4-gate) so the scaler-rail scaffolding actually fires instead of sitting silent.
