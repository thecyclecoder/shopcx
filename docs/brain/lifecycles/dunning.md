...
**⭐ Migration re-points OPEN dunning cycles (2026-09-21).** Every dunning step follows the `shopify_contract_id` recorded on the cycle. Before this fix, a migration that flipped a sub's contract left its open dunning cycles pointing at the pre-migration contract; each retry then reached a contract that isn't the sub's, found nothing to bill, and gave up **without writing any record** — so the sub silently dropped out of renewal selection for weeks. Five real customers were stranded 50-72 days; six of thirty-three open cycles were on wrong contracts on 2026-09-21. Fix: `repointOpenDunningCyclesForMigration` in [[../libraries/migrate-to-internal]] runs after each successful contract flip (both the billable path and the comp path), selects open cycles by **`subscription_id`** (the stable key across a flip, not the contract string), and rewrites `shopify_contract_id` via the [[../libraries/dunning]] `updateDunningCycle` SDK — never a raw `.from('dunning_cycles')` write. On a `(workspace_id, shopify_contract_id, cycle_number)` unique-index collision (4 of 7 repaired rows hit this — a cycle already exists on the target contract at the same cycle number), the orphan is CLOSED as `status='exhausted'`, `next_retry_at=null`, `closed_reason='orphaned_by_migration'` rather than mutating `cycle_number` (which would misrepresent the dunning ladder). A closed orphan carrying a `closed_reason` is correctly ignored by `dunning-new-card-recovery`, so it cannot resurrect on the next card add. Re-point failures are logged and non-fatal — the flip still succeeds.
