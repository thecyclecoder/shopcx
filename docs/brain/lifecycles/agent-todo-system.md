...
6. **Approve.** `POST /api/todos/[id]/approve` fires `agent-todo/execute` for any **Inngest-executable** type (the three customer-facing types **+ `ticket_analysis_rescore`**, gated by `isInngestExecutable()`). The worker drift-checks, dispatches (`sendTicketReply` / `directActionHandlers` / close / re-score), records the result, and auto-closes the ticket when the last customer-facing todo in the group executes (+ system note "Resolved via To-Do system. Approved by {role}…"). A rescore-only group does **not** auto-close.
