...
**Cheap triage tier (Haiku pre-filter before the box session).** Before a candidate that passes the trigger contract spawns the expensive Max box session, `enqueueTicketAnalyzeJob` runs a **single inline Haiku call** ([[../libraries/cora-triage-pass]] `runCheapTriagePass`) on the `auto_close` sweep. Most closed, Sol-handled tickets were handled fine; the cheap tier grades those itself — a lightweight `[cheap-pass]` [[../tables/ticket_analyses]] row + `last_analyzed_at` stamp — and only spawns the deep Cora session on a **terminal-state problem** (customer left unresolved/frustrated, an unkept promise, an out-of-policy offer, a false outcome claim). It judges the ENDING, not the messy middle (a recovered turn-1 stumble is not a trip), and it is **recall-biased** — any unparseable/contradictory verdict or hard failure fails OPEN to the deep session. Critically, the cheap tier **never escalates to June** — its only escalation is spawning the deep session, which is the sole path to `cs-director-call`. See [[../libraries/cora-triage-pass]] and [[../libraries/ticket-analyzer]] § Cheap triage tier.

**Cora dial-in (end-result primacy + coaching loop).** Cora is triggered by the cheap pass's `needs_review` flag (everyday path) OR by a **low CSAT** (`≤3` on a resolved ticket → `analyzeTicket(ticketId, "low_csat")` from [[../tables/ticket_csat]], bypassing the cheap pass). Her rubric grades the ENDING first: a severe issue that was RECOVERED before the close is a coaching note, not an escalation — she escalates to June ONLY when the ending is unsatisfactory (customer unresolved/threatening, or a severe issue that survived to shape the outcome). On a no-escalation verdict with a messy middle she emits a `sol_messy_turns` coaching signal ([[../libraries/sol-coaching-signal]]) — the same signal the cheap pass emits on a clean-but-messy close — which June's weekly digest ([[../libraries/cs-director-digest]]) aggregates into an `early_warning` + `add_rule` fix once a class recurs across ≥3 tickets. Cora always posts her verdict as a `[Cora Analysis]` internal note, and OVERRIDES a prior cheap-pass score in place (`applyAgentRescore`) so one authoritative grade stands per handling. See [[../libraries/ticket-analyzer]] § Cora dial-in.
...
| `src/lib/cora-triage-pass.ts` | Cheap Haiku triage tier gating the deep box session |
| `src/lib/sol-coaching-signal.ts` | Shared `sol_messy_turns` coaching signal both tiers emit for June |
| `src/lib/cs-director-digest.ts` | June aggregates `sol_messy_turns` → early_warning + add_rule fix |
| `src/app/api/csat/[ticketId]/route.ts` | ≤3 rating fires a `low_csat` Cora deep review |
