...
When a playbook IS active, the reply-vs-drift call happens in the [[../inngest/unified-ticket-handler]] `classify-playbook-msg` Haiku step BEFORE this Phase-3 orchestrator runs. That step now interpolates the pending step's question (resolved via `tickets.playbook_step` → [[../tables/playbook_steps]] + the most recent outbound external AI [[../tables/ticket_messages]] body) and applies an `ANSWERS THE PENDING QUESTION OVERRIDES EVERYTHING` rule that beats the lean-NEW_TOPIC default when the customer supplies what the playbook asked for (yes/no, confirmation, choice, address, order #). A message with substantive content bundled with thanks ("Thank you. Yes confirming this is the correct address.") is a confirmation, not gratitude. See [[../specs/playbook-drift-classifier-sees-the-pending-question]] — driven by ticket 8e2c87d6 (Suzanne Ross 2026-08-24, Replacement Order playbook dropped mid-flow after every reply was a literal answer).
