...
- **Copy variations DON'T require a Dynamic Creative ad set.** A single-video creative with multiple `titles[]`/`bodies[]` in `asset_feed_spec` is a plain **multi-text** ad — many can coexist in one regular ad set. What forces Dynamic-Creative semantics (and triggers "Dynamic Creative ads can only be created under Dynamic Creative Ad Sets") is including **`ad_formats`, `optimization_type`, or `asset_customization_rules`**. `createAdCreative` **omits all three** → non-dynamic, no `isDynamicAdSet` branch. (Confirmed from shopgrowth's working publisher; earlier we wrongly concluded variations needed a dynamic ad set.)
- **Link goes in `asset_feed_spec.link_urls = [{ website_url }]`** — NOT a top-level `link` and NOT in `call_to_action`, or Meta errors "The link field is required" (subcode 2061015). UTM tracking can't ride on `link_urls`, so it stays in the top-level **`url_tags`** field (shows up as the ad's "URL parameters").
- **Video ads require a thumbnail** — `asset_feed_spec.videos[].thumbnail_hash`. We pull Meta's auto-generated thumbnail (`getVideoThumbnail`) once the video is ready, re-upload it via `uploadAdImage` to get a hash, and attach it. (subcode 2490433 "Something went wrong" can mean a missing thumbnail or an unready video — `waitForVideoReady` covers the latter.)
