...
- **Phase 2 — Reimplement in shadow.** 🚧 Rebuild the mapping resolvers + the close computations in ShopCX (TS/Inngest), reading the same inputs, **posting nothing**. Two crown-jewel slices are ported + reconciled offline against the golden fixtures:
  ...
  - **Remaining (needs LIVE inputs):** the 3 zero-dollar SalesReceipts (COGS via `GroupLineDetail` BOM expansion) + the InventoryAdjustment (shrinkage, whole-unit `QtyDiff`) derive from live Amazon/3PL/Shopify unit counts + QB inventory snapshots. In progress next.
- **Phase 3 — Reconcile to zero.** 🚧 Diff shadow output vs `fixtures/.../qbo-entries` line-by-line. **JournalEntry: $0.00 reconciled (2026-06, full historical re-run).** Remaining artifacts (receipts, inventory adjustment) in progress. Then golden-master CI tests + CFO sign-off. The three reconcile harnesses (`_reconcile-je-processor-lines.ts`, `_reconcile-resolvers.ts`, `_reconcile-je-full.ts`) are the seed of the CI suite. **Method chosen (founder, 2026-07-10): historical re-run against frozen closed months — NOT a live parallel close.**
