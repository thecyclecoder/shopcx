...
- **Full Eve rebrand (founder-facing).** Every string the founder sees is now Eve, not "god mode": cockpit header + status ("Eve" / "Online" / "here till …"), the dashboard tab label ("Eve"), the CTA ("Chat with Eve"), the kill button ("Send home"), empty/loading/expired states, and standing-approvals copy. Internal identifiers are untouched (tables, `kind='god-mode'`, `/api/god-mode/*`, `/god/[token]`, the `godmode` sandbox).
  - **SMS bodies are the ONE exception to the flirt — deliberately clean.** All three texts (arm / 5-min nudge / done) are professional-warm with NO flirt and NO emoji (e.g. `"Eve here — I'm online and ready. Tap in:"`), because a text can surface on a lock screen. Eve's full flirty personality + emojis live INSIDE the app (cockpit + tab replies) only. Enforced by the comment in `sendGodModeSMS` ([[../libraries/god-mode]]).
