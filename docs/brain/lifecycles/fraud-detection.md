...
- Linked cluster resolver: [[../libraries/fraud-linked-customers]] (shared between investigate + confirm-fraud routes).
...
1. **Ban the customer + linked accounts** — resolve the full linked cluster via the shared resolver (same set the investigate route computes), then flip [[../tables/customers]].`portal_banned=true`, `portal_banned_at`, `portal_banned_by` on every account in the set. Return the list of accounts actually banned (id + email + whether already banned) so the operator sees the true scope, not an assumption. The confirmation dialog shows the exact accounts that will be banned BEFORE the operator clicks, sourced from the same resolver, and the result page confirms which were banned, already-banned, or failed per account (no silent partial success). The linked cluster resolver lives in `src/app/api/workspaces/[id]/fraud-cases/[caseId]/investigate/route.ts` and is called by both the investigate endpoint and the confirm-fraud route's `ban_customer` step, so the list the operator read on screen and the list that gets banned cannot drift. Ground truth: case 5182b1dd (2026-09-07) — operator confirmed fraud against 21 accounts; only 1 received `portal_banned`. Fixed in Phase 1 + Phase 2 to ban all linked accounts and show the blast radius before confirming.
...
[[ticket-lifecycle]] · [[chargeback-pipeline]] · [[ai-multi-turn]] · [[../integrations/shopify]] · [[../integrations/appstle]] · [[../libraries/fraud-linked-customers]] · [[../tables/fraud_cases]] · [[../tables/fraud_rules]] · [[../tables/known_resellers]] · [[../tables/amazon_asins]] · [[../tables/fraud_action_log]] · [[../inngest/fraud-detection]] · [[../inngest/reseller-discovery]] · [[../inngest/order-address-fallback]]
