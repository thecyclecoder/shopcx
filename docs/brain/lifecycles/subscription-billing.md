...
4. Update [[../tables/subscriptions]]:
   - `next_billing_date = advanceToNextFutureBillingDate(current_next, billing_interval, count, now)` — the shared pure helper in [[../libraries/subscription-billing-date]] steps by whole intervals until strictly in the future, so a sub several cycles behind SKIPS its backlog in ONE write rather than being billed out one cycle per day ([[../specs/a-subscription-is-never-more-than-one-cycle-behind]] Phase 2, CEO rule 2026-09-24: *one subscription can only ever be behind by a single cycle*). Whole-interval stepping preserves the customer's day-of-cycle anchor (month/year clamped to the target month's last day, not `setUTCMonth`-overflowed). Skipped cycles are NOT stamped as billed anywhere; the ledger only records the cycle that actually charged (this one). The same helper runs at all three advance sites in [[../inngest/internal-subscription-renewals]] (comp branch, zero-total branch, paid-renewal branch) so they cannot drift apart.
...
5. **Re-point OPEN dunning cycles** to the new `internal-*` contract ([[../libraries/dunning]] `updateDunningCycle`). Every step of dunning follows the `shopify_contract_id` recorded on the cycle, so a cycle left pointing at the pre-migration contract retries a contract that isn't the sub's, finds nothing to bill, and gives up **without writing any record** — the sub then drops out of the renewal selection silently. `repointOpenDunningCyclesForMigration` in [[migrate-to-internal]] selects open cycles by `subscription_id` (the stable key across a flip), and rewrites `shopify_contract_id` via the [[../libraries/dunning]] SDK — never a raw `.from('dunning_cycles')` write. The partial UNIQUE index `idx_dunning_cycles_contract` `(workspace_id, shopify_contract_id, cycle_number)` means a re-point can collide with a cycle already on the target contract at the same `cycle_number` (4 of 7 rows repaired on 2026-09-21 hit this); on collision the orphan is CLOSED as `status='exhausted'`, `next_retry_at=null`, `closed_reason='orphaned_by_migration'` rather than mutating `cycle_number` (which would misrepresent the ladder). `dunning-new-card-recovery` deliberately skips exhausted cycles carrying a `closed_reason`, so a closed orphan cannot resurrect on the next card add. Ground-truth: five subscriptions verified 2026-09-21 stranded 50-72 days because their open cycles pointed at the dead pre-migration contract; six of thirty-three open cycles were on wrong contracts, and repairing them recovered $480.22.
...
**Assisted-purchase create-subscription: contract id minted before insert + payment method persisted** ([[../specs/create-subscription-internal-branch-cannot-create-a-subscription]] Phase 1, derived from ticket `687b2e7a` — Susan Bellamy). `commerce/subscription.createSubscription`'s internal branch now generates `internal-<16 hex>` via `synthesizeInternalContractId()` UP FRONT and passes it into `buildCreateSubscriptionRow` via `opts.shopify_contract_id` BEFORE the insert. The prior code synthesized after the insert, but `subscriptions.shopify_contract_id` is NOT NULL — so every internal create failed on the constraint and the "repair" code was unreachable (four days of failures; the assisted-purchase playbook's last step never landed). `buildCreateSubscriptionRow` also now emits `payment_method_id`, which the interface had silently dropped.
