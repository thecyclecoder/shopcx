...
**⏳ Open:**
- Phase 3 (compositor typography + per-ratio safe zones + fit-to-box), Phase 4 (side-by-side QC gate against the source), Phase 5 (land copy at the angle `copy_pack`/`copyVariants` source + renders as `ad_videos` per-ratio rows). See `docs/brain/specs/dahlia-competitor-ad-adaptation-overlay-render.md`.
