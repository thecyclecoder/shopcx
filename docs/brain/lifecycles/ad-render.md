...
**✅ Overlay render path — Phase 2 (dahlia-competitor-ad-adaptation-overlay-render):** the verify-then-reword copy-adaptation instincts are wired into [[../libraries/creative-brief]] + Dahlia's SKILL. `buildCreativeBrief` now surfaces `brief.confirmedBenefits: [{ benefitName, softPhrasings, role }]` — the CATALOG of every benefit on OUR product where `product_benefit_selections.customer_confirmed=true` (role ∈ `lead`/`supporting`), each with the row's real `customer_phrases` for grounded rewording. A competitor-source brief also appends a `verify-then-reword` guardrail. The catalog is populated pure via [[../libraries/creative-imitation]] `selectConfirmedBenefits`; the same module exports `matchConfirmedBenefit(competitorClaim, confirmed)` (dimension-first classifier + phrase/token fallback), `hasDiverseBenefitStack` (no two beats on one dimension — Part 1 § "Diverse benefit stack"), and `enforceOfferFidelity(competitorCta)` with `OFFER_SUBSTITUTE_RISK_FREE = "Try It Risk-Free — 30-day money-back guarantee"` for offers we don't run. The [[../../../.claude/skills/dahlia-copy-author/SKILL.md]] IMITATE-DEBRANDED section carries the four positive-adaptation rules (VERIFY-THEN-REWORD · DIVERSE BENEFIT STACK · OFFER FIDELITY · ANCHOR TO CORE VALIDATED BENEFITS) so a competitor imitation KEEPS the winning device + applicable benefits (reworded from a real `customer_phrase`) and only SUBSTITUTES for a genuinely-lacked one. Pairs with the DON'T-fabricate firewall in [[../libraries/never-fabricate]] — the two sides of the imitate-then-innovate loop.

**✅ Overlay render path — Phase 3 (dahlia-competitor-ad-adaptation-overlay-render):** compositor typography + per-ratio layout wired into [[../libraries/creative-overlay]]. New `SAFE_ZONES: Record<NanoBananaAspect, {topPct, bottomPct, sidesPct}>` carries the Meta unified 2026 numbers verbatim (9:16 = 14/20/6, 4:5 = 14/14/6, 1:1 = 10/10/6); `fitFontToBox({ text, maxWidthPx, maxHeightPx, minPx, maxPx, weight, italic?, lineHeightRatio? })` is the area-first font-fit (largest font in `[minPx, maxPx]` that fits the box, greedy word-wrap via `wrapTextToBox` + `glyphWidthRatio` per-weight, no-orphan rule via `fixOrphans`); `planLayout(ratio, w, h, clearZone?)` returns a per-ratio + scene-aware layout plan — every text/CTA box lives INSIDE the safe rect, the CTA sits in the bottom safe band (never on the hero), and the layout follows the scene's actual clear zone (`OverlayClearZone` = `top_band` full-width above a centered-low product OR `side_column_left`/`right` column layout with the product on the opposite side). Column layouts use asymmetric gutters — tight to the product side, generous at the frame edge (Part 3). `buildOverlaySVG` refactored to consume the planner + fit engine instead of eyeballed pixel offsets; Part-3 type treatment enforced by construction (headline heavy 900, sub-headline / payoff light 300, benefit stack bold italic 700, CTA 700 in a white pill).

**✅ Overlay render path — Phase 4 (dahlia-competitor-ad-adaptation-overlay-render):** the side-by-side QC gate is wired into the overlay branch. New [[../libraries/creative-side-by-side]] owns the deterministic surface — `buildSideBySide(competitorImage, adaptedImage, opts?)` produces a pure sharp `[competitor | divider | ours]` composite (both halves `fit: "contain"` letterboxed, never cropped, to the per-half canvas so the vision judge sees equal halves), the `SIDE_BY_SIDE_QC_STRUCTURE = ["hook", "regret", "benefit_stack", "social_proof_payoff", "risk_reversal"]` enum names the 5 preserved-structure beats verbatim, and `sideBySideGate(verdict)` is a pure predicate that fails on either axis (`energyMatchOrSurpass` or `structurePreserved`) or a critical `issues` line — mirroring the prime directive "ship only when ours ties or wins on visual energy AND preserved structure." [[../libraries/creative-generate]] `generateCreative`'s overlay branch now returns an optional `sideBySide?: { buffer, mimeType }` on `GeneratedCreative`: when a `designReferenceUrl` (signed competitor skeleton via [[../libraries/creative-skeleton]] `signCreativeShot`) is provided, it fetches the bytes via `fetchDesignReferenceBytes` and builds the composite alongside the adapted render. A fetch failure is non-fatal (`console.warn` + proceed) — the side-by-side is an audit + gate, not a blocker of the overlay path. [[../libraries/creative-agent]]'s outer MAX_QA_ATTEMPTS regen loop is the consumer: hand the buffer to the vision judge, run `sideBySideGate` on the verdict, revise in-session on fail (mirrors the existing vision-QC bounce). The gate's reason string NAMES the missing structure beat(s) so the revise turn is concrete.

**✅ Overlay render path — Phase 5 (dahlia-competitor-ad-adaptation-overlay-render):** the landing targets are wired into [[../libraries/creative-overlay-landing]]. New pure `OVERLAY_LANDING_TARGETS` constant enumerates the three write surfaces the detail page reads (`ad_creative_copy_variants` primary, `product_ad_angles.metadata.copy_pack` fallback, `ad_videos` per-ratio rows re-signed from `meta.storage_path`); `overlayFinalsStoragePath(ws, videoId, ext)` is the canonical `finals/{ws}/{videoId}.{ext}` path pattern; `buildAngleCopyPackUpdateBody(existingMetadata, copyPack)` splat-preserves the angle's prior metadata (provenance / concept_tag) while writing `copy_pack`; and `landOverlayCreativePack(admin, opts)` orchestrates the deterministic sequence — angle-copy-pack update (compare-and-set on `id + workspace_id + .select('id')`) → canonical `ad_videos` insert → `uploadBuffer(finals/{ws}/{videoId}.jpg)` → sign URL → flip to `ready` + stamp `meta.storage_path` (compare-and-set on the update) → sibling `ad_videos` with `format_variant_of_id = canonicalId` (same-psychology invariant) → optional `writeCopyVariants` for the temperature-banded pack. Every mutation carries the confirming predicate the read implied (coaching #11-12) so a cross-workspace / stale-row bleed on a landing write is impossible. Composed from [[../libraries/creative-pack]] `planCreativePackInserts` (existing pure planner) + [[../libraries/ad-copy-variants]] `writeCopyVariants` (existing SDK chokepoint) + [[../libraries/ad-storage]] `uploadBuffer` / `signedUrl` (default storage seam; tests inject a fake).
...
| `src/lib/ads/creative-imitation.ts` | Phase-2 verify-then-reword surface: `ConfirmedBenefit`, `selectConfirmedBenefits`, `matchConfirmedBenefit`, `benefitDimensionOf`, `hasDiverseBenefitStack`, `enforceOfferFidelity` |
| `src/lib/ads/creative-side-by-side.ts` | Phase-4 side-by-side QC gate: `buildSideBySide`, `SIDE_BY_SIDE_QC_STRUCTURE`, `SideBySideVerdict`, `sideBySideGate` (pure) |
| `src/lib/ads/creative-overlay-landing.ts` | Phase-5 landing targets: `OVERLAY_LANDING_TARGETS`, `overlayFinalsStoragePath`, `buildAngleCopyPackUpdateBody`, `landOverlayCreativePack` (compare-and-set writes to `product_ad_angles.metadata.copy_pack` + `ad_videos` + `ad_creative_copy_variants`) |
...
[[../reference/competitor-ad-adaptation]] (how to adapt a proven competitor ad → ours: copy rules, the 3-layer text-overlay render, side-by-side judgment) · [[demographic-enrichment]] · [[product-intelligence]] · [[../tables/product_ad_angles]] · [[../tables/ad_campaigns]] · [[../tables/ad_videos]] · [[../tables/ad_jobs]] · [[../tables/ad_avatars]] · [[../tables/ad_avatar_proposals]] · [[../tables/ad_avatar_candidates]] · [[../integrations/higgsfield]] · [[../integrations/openai]] · [[../libraries/ad-angles]] · [[../libraries/ad-validator]] · [[../libraries/ad-render]] · [[../inngest/ad-tool]] · [[../dashboard/marketing__ads]] · [[../dashboard/marketing__ads__new]] · [[../dashboard/marketing__ads__avatars]] · [[../dashboard/settings__ad-tool]]
