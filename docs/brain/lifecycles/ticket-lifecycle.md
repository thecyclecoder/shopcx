...
| Sol returns `escalate_to_june` (or a guard/honor block on her reply) | `escalated` (branch-stamped) | the box worker escalates the ticket to June's lane for EVERY rail-hit (`escalateSolRailHit` sets `escalated_at`/`escalated_to=null`) → the triage-escalations cron enqueues a `cs-director-call`; there is no "needs human" — June is the CS final call |
...
**The close contract every close writer must honor** — the analyzer's gate is a two-field predicate (`status='closed' AND closed_at IS NOT NULL`), so a writer that flips status without also stamping `closed_at` (and `updated_at`, so the row's mtime advances) leaves the ticket permanently invisible even though the Control Tower and dashboards see it as closed. Every close path in the codebase — `closeTicketOnResolvingReply` ([[../libraries/ticket-directions]]), `closeTicket` ([[../libraries/tickets-mutate]]), the [[../inngest/unified-ticket-handler]] `message_sent` block, [[../inngest/journey-outcomes]] (the async fan-out on `journey/session.completed`), and the synchronous journey-completion route `src/app/api/journey/[token]/complete/route.ts` (cancel / saved / declined outcomes) — stamps all three fields together. A regression pin lives at `src/lib/journey-complete-close-contract.regression.test.ts` on the journey completion route so a refactor that reintroduces a bare `status='closed' + resolved_at` write (as the origin bug did — `resolved_at` alone is not what Cora selects on) red-lights loudly instead of shipping a ticket that looks closed to humans but is invisible to the analyzer.
