...
### Orders
- **Order detail** ([[../libraries/portal__handlers__order-detail]]) — full order view (order number, placed date, honest status badge, line items with images, totals, tracking link, shipping address). Read-only. Returns 404 on orders outside the customer's link group. In-house portal only — Shopify extension routes orders via subscription-detail.
