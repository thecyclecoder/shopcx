...
- **Sub-detail payment card** — for **internal** subs the detail screen shows the card the renewal will charge (its **pinned** `subscriptions.payment_method_id` if set, else the customer default), and offers **"Change card for this subscription"** — a picker of the customer's vaulted Braintree cards (`route=setSubscriptionPaymentMethod`) plus "+ Add a new card" → `/payment-methods`. Appstle subs read the last Shopify order's transaction and only get the "Manage payment methods" link.
