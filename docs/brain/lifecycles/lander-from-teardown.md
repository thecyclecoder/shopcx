...
Cleo's deterministic sweep ([[../libraries/cleo-blueprint]] `runCleoBlueprintSweep`) reads [[../libraries/research-urls]] `listNewTeardowns`, picks a target product ([[../libraries/cleo-blueprint]] `pickTargetProduct` → `listActiveProducts` — **gated by [[../libraries/advertised-products]] `listAdvertisedProductIds`, hero-product-advertising-gate Phase 2:** the enumeration only returns products with `products.is_advertised=true`, so an attachment SKU can NEVER be a teardown target. This closes the DR-content leak that produced parked Tumbler + Sleep-Gummies blueprints CEO-dismissed 2026-07-11), loads the product's existing [[../tables/storefront_experiments]] `lander_type` set, and calls `decideBlueprintForTeardown` per teardown:
