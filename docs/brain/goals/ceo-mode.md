...
**Role agents own the objective; their tools optimize a proxy.** A director's *specs are tools* — controllers with a setpoint (the [[../specs/storefront-iteration-engine|storefront iteration engine]] optimizes "ROAS above target"). Tools optimize that proxy locally and can drive themselves into a **degenerate state** that satisfies the proxy while destroying the objective (no winning ads → the engine scales budget to ~0 → ROAS "fixed," revenue gone — Goodhart's law). The **role agent owns the real objective** (profitable growth, CAC≤LTV, a revenue floor) and supervises its tools against it, acting as three things the tool isn't: (1) a **circuit-breaker** that overrides/re-tunes the setpoint (hold a budget floor, never fully pause a proven SKU); (2) a **root-cause diagnostician** that asks the question the tool can't (*why* is nothing winning — creative, targeting, landers, offer?); (3) a **work-spawner** whose diagnosis becomes the two lanes below — operational guardrails + new specs (better creative → [[../specs/winning-static-creative-finder]], a landing-page experiment harness, a targeting test). **A tool hitting a rail is not an outcome — it's the trigger for the agent's highest-value work.** This requires tools to *surface their reasoning + degenerate states* (not silently zero out) and *respect agent-set guardrails*, and requires objectives to be **explicit + hierarchical** (CEO sets objectives → role agent holds + supervises → tools execute) so divergence is detectable. Three nested control loops, each slower and wiser than the one below.
