...
**Model:** a spec is **operational state** (like a ticket / order / agent_job) → it lives in the DB while in-flight. The **brain stays the source of truth for finished, durable knowledge** — the FOLDED record of a shipped spec/goal. This sharpens the brain-is-source-of-truth principle (today the brain awkwardly holds half-built in-flight scratchpads).

**The hierarchy (all DB rows, parent FK, status DB-driven + rolled-up at every tier; one-to-many everywhere):**

```
goals        (id, parent_goal_id? , status, title, body…)   ← parent_goal_id set ⇒ this goal is a "SUBGOAL"
  └ milestones (id, goal_id      , position, status, title, body…)
      └ specs    (id, milestone_id , status, intended_status, title, body, owner, parent, blocked_by…)
          └ spec_phases (id, spec_id , position, status, pr, merge_sha, title, body, verification…)
```

A **SubGoal is just a Goal with a `parent_goal_id`** — NOT a separate table. The goal-decomposition engine always operates on the same four tiers (Goal → Milestones → Specs → Phases) and is indifferent to whether a goal has a parent; the parent link is purely a relationship the board uses to render a goal nested under another (CEO Mode ▸ Fully Autonomous CTO ▸ M1…M5 ▸ specs ▸ phases). Most goals have no parent; a goal CAN be (re)assigned under another goal at any time (a single `UPDATE goals SET parent_goal_id=…`), the same move-ability we want for phases.
...
- **M5 — Goals + milestones are data too (the top two tiers).** A **`goals`** table (id, `parent_goal_id` nullable self-ref → a SubGoal is just a goal with a parent, NOT a separate table; status `proposed｜greenlit｜complete`; title; body) + a **`milestones`** table (id, `goal_id` FK, position, status, title, body). Specs gain a `milestone_id` FK (M1). Status at every tier is a DB column that ROLLS UP from children — never the `**Status:**` line or `⏳/✅` emojis. **Greenlight becomes a one-click DB flag with a UI button** (the surface the CEO was missing — it's why the db-driven-specs goal had to be hand-committed). `parseGoal` / `setGoalStatusLine` / the goal-status markdown machinery retire; the goal-decomposition engine reads greenlit goals from the DB and decomposes Goal → Milestones → Specs → Phases (indifferent to parent_goal_id). Fold writes the goal's brain record on completion. Backfill `docs/brain/goals/*.md` into the rows.
