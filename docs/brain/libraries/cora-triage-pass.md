...
## Pre-purchase-still-blocked override (deterministic)

The cheap classifier cannot always be trusted to catch a specific storefront-block failure mode: on ticket 3dd271be (mdengberg2, Mixed Berry 60-day sub) the customer's LAST message still said she couldn't buy, the AGENT had looped 3× on cache-clear + invented a "30/60/90 Day Supply card" UI, and Haiku still returned `needs_review=false, score=9`. To prevent that class of false-resolve, `runCheapTriagePass` runs `detectPrePurchaseStillBlocked` on the same transcript AFTER `parseTriageResult`. When both halves hit — (1) the LAST inbound customer message still describes being blocked from buying / ordering / checking out / selecting / subscribing / adding to cart, AND (2) some AGENT reply suggested `clear (your|the) cache / cookies`, `try incognito`, `try a different browser`, or `hard refresh` — the override forces `needsReview=true` and prepends `customer_unresolved` to `signals`, so the ticket routes to the deep session exactly like any terminal-state trip. The summary is prefixed `[pre-purchase-still-blocked]` for auditability.

The AGENT-side dead-end patterns mirror the [[assisted-purchase-direction]] `assertSolFastDefaultToConcierge` cache-loop patterns — the cache-clear / hard-refresh regexes REQUIRE a prescriptive prefix (`please`, `could you`, sentence-start imperative), so an agent's ACKNOWLEDGMENT of what the customer already tried ("I saw you tried clearing your cache") does NOT trip the override.
