...
- The override builds `claude/*` ONLY. Incidental branches (no `claude/` prefix, not production) still skip — this is preserved by the literal command string + the Phase 3 grep gate at `scripts/_check-vercel-ignore-step-rails.ts` (chained into `predeploy:static` and enforced pre-commit by the build lane, see [[../operational-rules#Predeploy static guards — enforced in the build lane, not by npm's `predeploy` hook]]). The gate asserts the constant still carries the `^claude/` anchor, the `production` env check, the `exit 1` build branch, and the `exit 0` skip branch; the build lane fails red if a code change ever weakens any of them.

> **A note for "what runs on a deploy?"** Vercel's own build runs `next build` (and its TypeScript typecheck) on every deploy — that's the TSC gate. The `~30 scripts/_check-*.ts` guards ("static scanners" — RLS, table-refs-have-migrations, PM-SDK compliance, node-registry drift, etc.) are NOT gated by Vercel; they are gated pre-commit inside the build lane at `scripts/builder-worker.ts:27412` via `npm run predeploy:static`. The `predeploy` npm script itself is inert on its own (npm fires a `predeploy` pre-hook only before a script literally named `deploy`, and there is none). See [[../operational-rules#Predeploy static guards — enforced in the build lane, not by npm's `predeploy` hook]] for the full mechanism + why a `prebuild` hook is a trap.
