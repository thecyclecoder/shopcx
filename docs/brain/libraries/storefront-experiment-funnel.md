...
**File:** `src/lib/storefront/experiment-funnel.ts` · Reads [[../tables/storefront_experiment_variants]] (persisted rollups) + [[../tables/storefront_sessions]] (`experiment_assignments` spine) + [[../tables/storefront_events]] · Uses `winProbabilityVsControl` from [[storefront-bandit]]. Read-only — never writes (the bandit's [[storefront-experiment-attribution]] owns the rollup columns).
...
2. **Funnel rates (event-derived):** `engagement_rate`, `atc_rate`, `lead_rate` are computed fresh from `storefront_events`, keyed on the **same session-stamp spine** the attribution lib uses (experiment-session-stamped-attribution) — a session's arm comes from `storefront_sessions.experiment_assignments` (paged via the `.contains` jsonb filter, `is_internal=false, is_bot=false`), so a session counts toward an arm's engagement/ATC/lead iff it's stamped to that arm. Internal/bot are excluded at this report layer.
...
## Gotchas
- **Session-stamp-keyed, sticky.** A session belongs to at most one arm of an experiment (sticky stamp), so `session_id → variant_id` is unambiguous.
- **Bounded fetch.** ATC/lead/engagement events are pulled `.in("session_id", …)` over the stamped-session set (chunked 200), not the whole event log.
- **No stamped sessions yet → all zeroes, not an error** (the detail page renders zeroes for a fresh experiment).
