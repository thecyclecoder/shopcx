...
- `readRecentScaleRailDeferralsForAdsets(admin, workspaceId, adsetIds, nowMs)` → `Set<string>` — reads `director_activity` where `action_kind='media_buyer_scale_rail_deferred'` in the last `RAIL_DEFERRAL_LOOKBACK_MS` (24h), scoped to the workspace + `metadata.target_object_id ∈ adsetIds`. Returns a set of adset IDs that had a recent scale-rail deferral (per-object cooldown or per-account daily budget ceiling); an adset in this set counts as legitimate crown coverage (no false-positive fix-spec). Exported so tests pin argv-level filters and set semantics; best-effort on read error (returns empty set, degrades to pre-Phase-3 behavior).

### 1. Crown/kill drift (Bianca) — coverage requires `status='executed'` OR a legitimate scale-rail deferral

Calls [[testing-results-sdk]] `getTestingResults(admin, workspaceId)` — which itself calls `resolveTestThresholds` against the workspace's live [[../tables/iteration_policies]] row (crown ≥ 8 purch + CAC ≤ $150 + spend ≥ $450; early-trim ≥ $300 with 0 sales; deadline $1,200 without hold band). Any `active` row in the returned `products[].rows[]` classified `crown` or `dud` becomes a candidate. For each candidate, `readExecutedIterationActionsForAdsets` reads `iteration_actions.action_type ∈ ('scale_up','pause') AND status='executed'` scoped to the workspace + the adset ids. **Crown coverage (Phase 3, 2026-08-24):** also calls `readRecentScaleRailDeferralsForAdsets` — if an adset has a recent scale-rail deferral (per-object cooldown or per-account daily budget ceiling arm) within the 24h window, it counts as legitimate crown coverage (Bianca evaluated the crown and correctly chose to defer per policy, writing a `media_buyer_scale_rail_deferred` `director_activity` row instead of an `iteration_actions` row). A crown with **no executed scale_up AND no recent rail deferral** is a should-crown miss. **Kill coverage (unchanged):** a dud with **no executed pause** row is a should-kill miss (rail deferrals do not apply to pause operations). Each miss authors a `bianca_missed_crown` / `bianca_missed_kill` finding.

**Coverage contract ([[../specs/media-buyer-decided-kills-must-execute-on-meta-not-just-be-recorded]] Phase 2):** the read requires `status='executed'`. Pre-Phase-2, the check counted BOTH `decided` and `executed` rows ("a queued-but-unfired action still counts as covered"), so a decided-but-unfired kill was HIDDEN from the very watcher meant to catch it — the exact hole that let four Superfood duds keep bleeding at ROAS 0.00 while the ledger CLAIMED a pause it never made. Post-Phase-2 a `decided`-only row (Bianca hasn't fired yet, or the executor is stalled) OR a `failed` row (the Meta call threw — Phase 1's inline execute stamped it `failed` + raised an escalation card via `escalateMediaBuyerExecuteFailure`) leaves the adset ABSENT from the coverage map, so `!acted?.hasPause` reads true and the `bianca_missed_kill` finding fires — the gap becomes visible per the no-false-promises principle ([[../specs/eliminate-false-promises-no-claim-ships-until-executed-and-verified]]). Pinned in `src/lib/ads-supervisor.coverage-executed.test.ts` (argv-level `.eq("status", "executed")` pin + mapping semantics + grep guard against the pre-Phase-2 shape).

**Ledger persist failures now escalate immediately** ([[../specs/ads-supervisor-fix-fdc11e10-bianca-kill-120253353767090184]] Phase 1): a third class of missed-kill was silent-skip on ledger persistence — when the `iteration_actions.upsert()` errored or returned no rowId, the runner's `if (!rowId) continue;` silently abandoned the decided pause, and the dud kept burning spend for up to 3h before the supervisor's next pass. The fix flips that class from silent into immediate escalation: `escalateMediaBuyerExecuteFailure` with `media_buyer_kill_persist_failed` (and its promote/reactivate siblings) raises a per-adset per-day CEO card on the first pass where the ledger fails, instead of waiting 3h. The supervisor's coverage read still fires `bianca_missed_kill` if the 3-hour check finds no executed row — but now every missing row has either a fresh persist_failed card (ledger error) or a fresh execute_failed card (Meta error) already surfaced on the same 2h cadence, giving the CEO a much shorter visibility window than before. Idempotent per-day deduping prevents 2h pass spam while keeping the issues visible daily until fixed.

**Scale-rail deferral coverage (Phase 3, 2026-08-24):** the per-object cooldown rail was armed in [[../specs/fix(media-buyer)-arm-the-per-adset-cooldown-rail-one-budget-change-per-adset-per-day]] (one budget change per adset per day). When Bianca evaluates a crowned adset but defers the promote due to the cooldown, it writes a `media_buyer_scale_rail_deferred` `director_activity` row (action_kind, metadata.target_object_id, metadata.rail) instead of an `iteration_actions` row. Without the deferral read, the crown would fire `bianca_missed_crown` every 3h until the 24h cooldown expired, filling #director-growth-max with false positives. The deferral coverage check (`readRecentScaleRailDeferralsForAdsets`, 24h lookback = `RAIL_DEFERRAL_LOOKBACK_MS`) reads these rows and treats a recent deferral as proof of legitimate policy enforcement. A deferral older than the cooldown itself can't still be blocking a promote on this tick, so it does NOT count as coverage — it falls off, the supervisor re-fires `bianca_missed_crown` if Bianca still hasn't promoted, and Bianca either succeeds (cooldown cleared) or hits the rail again (fresh deferral row). Pinned in `src/lib/ads-supervisor.rail-defer-coverage.test.ts` (argv-level filter assertions + source grep guard against pre-Phase-3 shape).
