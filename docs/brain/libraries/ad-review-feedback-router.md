...
- **The receiving lanes don't yet narrow on `revise_reason`.** Phase 2's contract is the router + the queued job. `runAdCreativeJob` / `runAdCreativeCopyAuthorJob` / `runAdCreativeCopyQcJob` currently ignore the `revise_reason` + target fields; consuming them to run a targeted re-drive (rather than a full pass) is the follow-up. The dispatch chain is intact — the CEO's packet is no longer inert.
