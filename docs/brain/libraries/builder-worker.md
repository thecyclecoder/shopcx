...
**One bounded re-ask for the JSON envelope before parking** ([[../specs/storefront-optimizer-re-asks-once-before-parking]] Phase 1). The lane used to park a job the moment `extractJson(resultText)` returned no recognizable status, throwing away eleven turns of real reasoning on a formatting slip (the same lander parked twice in 24h with a sound lever already picked, just wrapped in prose). Now, on the terminal fall-through the runner consults the pure helper `shouldReAskForJsonEnvelope` in `src/lib/storefront-optimizer-reask.ts` and — when the result is unparseable, the session did NOT stream `isError`, and no re-ask has yet fired — resumes the SAME box session ONCE with `storefrontOptimizerReAskPrompt()` asking for only the JSON envelope (no re-analysis; the model has its decision in memory). A successful re-parse falls through into the existing `idle` / `needs_input` / `needs_build` / `propose` branches unchanged. A second unparseable attempt parks exactly as before, with the log text now saying a re-ask was attempted so the next reader knows this is a genuine formatting failure. Mirrors the same-session parse-repair the [[deploy-guardian]] and [[mario]] lanes already use. The two neighbouring branches are untouched: `isError && !parsed` still fails the job (a broken session can't self-recover on a `--resume`), and a parsed-but-unrecognized status still parks (a semantic failure, not a formatting slip). Bounded in-scope by a local `alreadyReAsked` boolean; no DB column.
...
## The deploy build gate — extracted for reuse ([[../specs/a-red-main-is-a-first-class-pipeline-alarm]] Phase 2)

The pre-merge `next build` gate the box lane has always run — before extraction it lived inline in `runNextBuildGate` (scripts/builder-worker.ts:27724). Phase 2 lifted its pure logic + runner into [[deploy-build-gate]] (`src/lib/deploy-build-gate.ts`) so the auto-merge chokepoint ([[github-pr-resolve]] `autoMergeReadyPrs`) can call the SAME gate — one truth, no drift on what counts as a merge-blocking error. The box lane's `runNextBuildGate` is now a thin wrapper that delegates to the extracted `runNextBuildGate(wt, { runShellSync: sh, runShellAsync: shAsync })` so its exact shell behavior (64 MB output cap, 15-minute timeout) is preserved verbatim.

- `BUILD_GATE_BLOCK_RE` matches the four cacheComponents / prerender phrases tsc structurally cannot see (`Uncached data was accessed outside of <Suspense>` · `Error occurred prerendering page` · `Export encountered an error` · `Route segment config … not compatible`). `BUILD_GATE_PATH_RE` matches `src/app/` / `src/components/` / `next.config` / `middleware.` for path-scoping (the ~4-minute build isn't paid on docs-only / lib-only PRs).
- `classifyBuildGateOutput(code, out)` is the PURE asymmetric classifier — block ONLY on the block phrases; a compile / module / binary failure is treated as infra noise (`pass:true`) because it is NOT the author's code (bouncing back would loop forever). That asymmetry is deliberate and load-bearing; pinned in `src/lib/deploy-build-gate.test.ts` (`npm run test:deploy-build-gate`).
- **Auto-merge chokepoint wiring:** the box's standing pass now passes a `runDeployBuildGate` callback into `autoMergeReadyPrs({ admin, runDeployBuildGate })` — the callback checks out the PR head in a fresh worktree via `git fetch origin <branch>` + `git worktree add --detach` and calls `runNextBuildGate(wtDir)`, tearing down the worktree in a `finally`. The chokepoint refuses a merge on a real block; a preflight fetch/worktree-add failure returns `pass:true` (infra noise, same asymmetry). The webhook path passes no callback — a build-affecting PR without a callback is DEFERRED (safer than merging unchecked); the box's next tick supplies the callback and runs the real gate.
...
## Pre-commit self-verify gate — `preCommitSelfVerify`
...
- **The check.** `preCommitSelfVerify({ workspaceId, slug, repoRoot })` in `scripts/builder-worker.ts` dynamically imports [[spec-check-runner]] and calls `runSpecChecks({ deps: { loadChecks: defaultLoadChecks, executors: defaultExecutors, packageScripts, repoRoot: <wt> } })` — the SAME deterministic runner the post-hoc `spec-test` lane invokes (Vera is retired — the lane is `runSpecChecks` alone now; see [[spec-check-runner]] § graduate-vera). So a check that fails here also fails post-hoc; the point is catching it a build cycle earlier.
...
Each pool config dir holds a `.credentials.json` whose `claudeAiOauth.expiresAt` is the authoritative local signal for its OAuth token's lifetime. When it's already ≤ now, the next `claude -p` on that account is guaranteed to 401 and drop whatever ticket lands there. So before picking an account, the box sweeps the pool and ejects any account whose stored token is already expired via the SAME `markAccountAuthExpired` machinery (long cap window + dedupe'd CEO card naming the exact `CLAUDE_CONFIG_DIR=<dir> claude auth login` command) as the Phase 2 reactive path — a dead account never receives a session in the first place. The Phase 2 backstop still catches the rarer case where the file looks fresh but the server rejects the token anyway.

- **`readCredentialsExpiresAt(configDir)`** at `scripts/builder-worker.ts` reads `$configDir/.credentials.json` and returns `claudeAiOauth.expiresAt` (ms). Returns `0` on any error (missing file / malformed JSON / missing field), so a malformed credential can NEVER cause a silent proactive eject — the reactive Phase 2 path stays in charge for those.
- **`cachedCredentialsExpiresAt(configDir, now)`** wraps the stat in a 30-second per-account cache (`credentialsCache: Map<string, {expiresAt, checkedAt}>`, TTL `CRED_EXPIRY_CACHE_MS = 30_000`) so the pick hot-path doesn't stat on every call. A stale entry is harmless — a re-login rewrites the file, the cache expires within seconds, and the reactive backstop covers the interim.
- **`sweepExpiredCredentials(now)`** iterates `accounts`; for each account whose cached `expiresAt > 0 && expiresAt <= now`, calls `markAccountAuthExpired(dir, now, "credentials.json expiresAt=… ≤ now — proactive eject before dispatch")`. `markAccountAuthExpired` is dedupe'd on `capEventLogged` so a re-sweep of an already-ejected account is a no-op — one CEO card per account per window.
- **Wired into `pickNewSessionAccount(now)`** at the top of the pick, before `healthyAccounts(now)` runs. When ALL accounts are unavailable after the sweep (usage-capped or OAuth-expired), the null-return branch logs `every pool account is unavailable (usage-capped or OAuth-expired) — awaiting soonest reset or CEO re-login` so the ops signal is visible rather than a silent dispatch failure. The dispatcher's normal `blocked_on_usage` park handles the block downstream — the ticket is not dropped.

North star: same shape as every other bounded proxy — the sweep proposes the eject; `markAccountAuthExpired` + the CEO card are the deterministic supervisor artefact per [[../operational-rules]] § North star. A silently-expired half of the pool can never again quietly eat sessions.

## Auth-expiry account ejection + CEO re-login alert ([[../specs/box-session-retry-on-oauth-token-expired-401-sol-never-drops-ticket]] Phase 2)

When a session's account rejects its OAuth token as expired (the `isTransientAuthError` signature — the same `401 + OAuth access token has expired` shape the in-session retry loop above and the build-lane requeue at L26243 already classify), the box pulls that pool account from rotation and raises a CEO card so the login happens promptly instead of after a pile-up of dropped tickets.

- **`markAccountAuthExpired(dir, now, detail)`** at `scripts/builder-worker.ts` (near `markAccountCapped`) sets `a.cappedUntil = now + WEEKLY_CAP_MAX_MS` (one full weekly window so a persistently-dead token doesn't re-probe + re-alert every ~20 min while the CEO is asleep) and records a new `auth_expired` `AccountEvent` type so the box-health surface distinguishes it from a usage-wall `cap`. Fires-and-forgets `emitCeoAuthExpiryAlertBestEffort(dir)` — a `dashboard_notifications` insert of `type='box_account_auth_expired'`, deduped by `metadata->>dedupe_key = 'box_account_auth_expired:<config_dir>'` so one open card per account at a time — with a body that names the config dir + the exact `CLAUDE_CONFIG_DIR=<dir> claude auth login` command. On the next boot the `restoreAccountCapsOnBoot` weekly-window clamp preserves the ejection.
- **`withAccountFailover` at scripts/builder-worker.ts** — a new branch parallel to the usage-cap hop (right after the `isUsageCapError` check): when the run's error is `isTransientAuthError`, call `markAccountAuthExpired` and hop to the next healthy account with a `failover` event tagged `auth-expired`. `isTransientAuthError` explicitly rules out `isUsageCapError` and the usage-cap branch returns before this check, so a genuine wall still takes the wall/hop path. Every non-build lane (ticket-handle, ticket-improve, triage, spec-test, migration-fix, repair, regression, security, storefront, product-seed, pr-resolve, …) routes through this helper, so a dead account is ejected + the ticket re-routes to a healthy account in the SAME turn — never dropped as "errored with no direction".
- **`runTicketHandleJob` isError backstop** — reaching this branch with a residual auth-expired signature means every pool account was unavailable this turn (all capped + all expired) OR the failover ran out. Instead of failing the ticket (the customer isn't at fault for an account-auth problem), the branch idempotently re-invokes `markAccountAuthExpired` and requeues the job via a compare-and-set `.update({status:'queued', claimed_at:null, claude_session_id:null, claude_session_config_dir:null, error:'TRANSIENT_AUTH_REQUEUE: …'}).eq('id', job.id).eq('workspace_id', job.workspace_id).eq('status','in_progress').select('id')` so a concurrent transition can't overwrite a resolved row. A ticket that hits the backstop stays in queue and reruns on a fresh worker once the CEO re-logs in a pool account — never a dropped ticket for an ops signal.
...
## Predeploy static gate — repair only what the branch owns

The build lane runs the full `npm run predeploy:static` guard chain as a blocking pre-commit step. Historically, on a non-zero exit the lane RESUMED Bo's session to repair the violation — with no check on whether the offending file was one this build touched. When two concurrent builds hit the same PRE-EXISTING violation on main, each authored its own wording of the same fix in files its spec had no business editing, and whichever branch merged first turned every sibling into a reconcile conflict the box refuses to auto-resolve. Ground truth: **2026-08-31** — the `cold-scaler-arming-decides-on-evidence-not-absence` and `creative-scout-job-stamps-spec-slug` builds sat parked at `needs_attention` / `reconcile_conflict` for **6 days** on the SAME three `_kcups` files because each independently made the same repair while a separate PR landed that identical fix on main. Neither spec was about kcups.

The fix teaches the gate the difference between a violation the branch CAUSED and one it INHERITED from main, and repairs only the former.

- **Classifier.** [[predeploy-guard-extract]] `classifyPredeployViolationScope({ out, changedPaths })` parses the guard output into repo-relative paths (the `  • {file}:{line}  →  {snippet}` and `  [VIOLATION] {file}:{line}  {snippet}` shapes our guards emit) and splits them against the branch's `git diff --name-only origin/main...HEAD`. Both sides normalized (leading `./` stripped, backslashes to `/`) before comparison. Pure, no I/O, unit-tested — see `predeploy-guard-extract.test.ts`.
- **Fail-closed default (load-bearing).** `allInherited` is TRUE **only** when at least one path was extracted AND none of them is in `changedPaths`. An EMPTY extraction (guard output we cannot parse) yields `allInherited: false` so the loop falls back to today's repair-it behavior. Silently skipping a real violation would be strictly worse than a redundant repair, so an unparseable guard NEVER masquerades as "nothing this branch owns."
- **Owned or mixed → today's behavior, unchanged.** `allInherited === false` (some path is owned, or unparseable) → resume Bo's session and repair, bounded by `PREDEPLOY_REPAIR_MAX`. Mixed deliberately still repairs — the branch genuinely owns part of it.
- **Fully inherited → skip + record + route to ONE owner.** `allInherited === true` → BREAK the repair loop and let the commit proceed. The lane records ONE [[director-activity]] row (`director_function: 'platform'`, `action_kind: 'predeploy_inherited_violation_skipped'`, `metadata: { job_id, spec_slug, guards, inherited_paths }`) so Ada's feed reads consistently with the sibling `build_predeploy_guard_repaired` row, then enqueues ONE deduped fix spec via `authorSpecRowStructured` keyed on `predeploy-violation-{guard}`. The deterministic slug is the dedupe property: the second, third and Nth build that hits the same inherited violation re-author the SAME row idempotently ([[author-spec]] `reopenIfReauthoredAndChanged`) instead of opening N specs. Parent is the platform `Infra & DevOps / reliability` mandate (`parentKind:'mandate'`, `parentRef:'platform#infra-devops-reliability'`).
- **Non-fatal author.** A failed fix-spec author path is logged but never blocks the build — the SKIP decision is what unblocks THIS commit; the durable dedup fix spec is the follow-up, and the next dispatch that hits the same inherited violation re-tries the author.
