...
The re-drive path in `runBuildJob` treats the two classes distinctly: a fail against real code CONSUMES a `BUILDER_DEFERRED_REDRIVE_MAX` re-drive; an unevaluable outcome DOES NOT (it blocks the ship AND spares the build, and demands a human or a repair lane fix the check — never a phantom-ship pass, which would be the exact hole this spec closes in a new costume). The ground-truth incidents this vocabulary exists to prevent: `cancelled-subs-stop-reporting-a-future-billing-date` (2026-08-21, 3 re-drives burned on a `(?i)` pattern `git grep` refused; nothing on main mentions `cancelled_at`) and `playbook-drift-classifier-sees-the-pending-question` (2026-08-24, same shape).
