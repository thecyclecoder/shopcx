...
### `getLinkedShopifyCustomerIds` — function

```ts
async function getLinkedShopifyCustomerIds(workspaceId: string, customerId: string,) : Promise<string[]>
```

Returns every distinct `customers.shopify_customer_id` in the same link group as `customerId` — the set of Shopify identities a minted loyalty coupon's `customerSelection.customers.add` must cover so the whole linked-account person can redeem it at checkout regardless of which profile they check out under. `redeem_points` (in [[action-executor]]), the `/api/loyalty/redeem` dashboard route, and the portal `loyalty-redeem` handler all pass this list into `customerSelection` instead of the single-profile `member.shopify_customer_id`. Empty array when no customer in the group has a `shopify_customer_id` on file; callers merge in the checking-out / member profile's own id as a last-resort fallback. Precedent: ticket e4d34bba (Jennifer Rogers — 16,147 points earned on itsjenrogers@yahoo.com but every LOYALTY-* coupon was pinned to the linked jmartwick@yahoo.com Shopify id and unusable at checkout). Spec: loyalty-redeem-and-coupon-usability-span-linked-accounts Phase 1.
...
- **Redemption + minting span the whole link group.** `redeem_points` (in [[../libraries/action-executor]]), the `/api/loyalty/redeem` dashboard route, and the portal `loyalty-redeem` handler each (a) resolve the loyalty member via `getMemberByCustomerId` — the link-group-aware chokepoint that aggregates sibling rows — instead of the direct-profile `.eq('customer_id', …)` fast-path, and (b) pass EVERY id returned by `getLinkedShopifyCustomerIds` into the minted code's `customerSelection.customers.add`. Points earned on any profile in the group are redeemable from any profile, and the resulting coupon works at checkout under any linked profile — not just the profile the loyalty_members row happens to live on (which is what pinned Jennifer Rogers' 16,147 points to an unusable code minted for her sibling jmartwick@yahoo.com Shopify id, ticket e4d34bba). Codes minted for MANY linked profiles is distinct from the subscription-apply regen path, which mints for the ONE contract-owning profile — a checkout coupon must reach any linked profile; a contract-attached coupon must match the profile that owns the contract. Spec: loyalty-redeem-and-coupon-usability-span-linked-accounts Phase 1.
