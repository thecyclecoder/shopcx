...
- **store** — `{ brandProofPoints[] }` from `workspaces.social_brand_proof_points` (workspace-wide "overall selling points" the product tables DON'T hold — 30-day guarantee, 700k customers, 15k reviews, family-owned, Austin TX). The blind spot the SDK closes. Same for every product.
- **offer** — computed from the product's active `pricing_rules`: `headline` ("Up to 34% off + free shipping (25% SnS + up to 12% for 3+)"), `maxCompoundDiscountPct` (SnS × volume, **multiplicative** — 25% × 12% = 34%), plus the **price-on-static** inputs: `msrpCents` (never show bare — HARD NO), `discountedUnitCents` (the strikethrough target), `perServingCents` (`$1.76/cup` vs a $4–8 latte), `disclaimer` ("3+ units on Subscribe & Save"). Display rule in [[meta-scaling-methodology|../reference/meta-scaling-methodology]] § Price-on-static.
