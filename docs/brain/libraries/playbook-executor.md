...
### `matchCancelledSubForStatedProduct` — function

```ts
function matchCancelledSubForStatedProduct(input: {
  customerMsg: string;
  cancelledSubs: CancelledSubProjection[];
}): MatchedCancelledSubIntent | null
```

Pure — no DB, no network. Returns the UNAMBIGUOUS cancelled sub whose item titles the customer named in this turn's message, projected onto the shape [[#`resolveAssistedSubscriptionIntentFromCancelledSubs`]] needs to build a `RawAssistedPurchaseIntent`. The match is case- and punctuation-insensitive substring (title / variant_title normalized identically to the message) — deliberately NOT fuzzy: "Cacao French Roast" typed for "Cocoa French Roast" is a miss, because auto-resolving a mistyped product to a different sub is a real risk. Returns null on ambiguity (two cancelled subs whose titles both appear), on zero matches, or on a matched sub missing `billing_interval` / `billing_interval_count`. The ambiguity bail-out is deliberate: Sol's Direction boundary is the right place to disambiguate, not the routing boundary.

Spec: [[../specs/assisted-subscription-purchase-resolves-variant-when-routing-omits-params]]. Ground-truth case is ticket `e5dedbf8` — Deborah Kacprowicz asked to restart her cancelled Cocoa French Roast sub (2 bags every 2 months) matching contract `28425617581`; the orchestrator routed to `assisted-subscription-purchase` but its decision carried no `create_subscription` action with a `variant_id`, so the routing boundary originally escalated. This matcher lets the boundary resolve the target from her cancelled sub instead. Pinned by `playbook-executor.cancelled-sub-fallback.test.ts`.

### `resolveAssistedSubscriptionIntentFromCancelledSubs` — function

```ts
async function resolveAssistedSubscriptionIntentFromCancelledSubs(input: {
  workspaceId: string;
  customerId: string;
  customerMsg: string;
  today: string;
...
DB-backed sibling of [[#`matchCancelledSubForStatedProduct`]]. Fetches the customer's subs via [[commerce-subscription]] `listSubscriptionsByCustomer`, projects the `status='cancelled'` rows onto the pure matcher's shape, and returns a `RawAssistedPurchaseIntent` (`actionType='create_subscription'`, variant + cadence from the matched sub, `nextBillingDate=today` for an immediate first bill) when the match resolves. Trust boundary is preserved: the returned intent still flows through [[#`resolveAssistedPurchaseIntentToParams`]] so `findVariant` re-verifies the id and the workspace-scoped `product_variants.price_cents` is what stamps `unit_cents` — nothing on the customer's message or the cancelled sub reaches the outgoing price or vendor.
...
- **The live orchestrator's `handlePlaybook` mirrors the Sol Direction routing boundary for the two assisted-purchase slugs** ([[../specs/live-orchestrator-assisted-purchase-carries-picked-item]]). When Sonnet picks `action_type:'playbook'` targeting `assisted-order-purchase` or `assisted-subscription-purchase`, [[action-executor]] `handlePlaybook` calls [[#`extractAssistedPurchaseIntentFromDecision`]] on the decision's `actions[]`, hands the result to [[#`resolveAssistedPurchaseIntentToParams`]] for the DB-backed variant → internal-UUID resolution, and passes the resolved params through `startPlaybook`'s `seed_context.assisted_purchase_params` — EXACTLY the shape `handleAssistedCreate` reads at the terminal step. If the intent cannot be resolved (no `variant_id` on any action, or `findVariant` returns null for a cross-workspace / bogus reference), `handlePlaybook` escalates with `reason='assisted_purchase_orchestrator_missing_intent:<slug>'` INSTEAD of calling `startPlaybook` with an empty context. Rationale: starting with empty params only trips the sibling `assistedCreateMissingItemsGuard` on every subsequent turn, and the orchestrator emits its canned "which product and flavor..." reply in an infinite loop (ticket `083201b5` — Maria D James, confirmed 3× Cocoa French Roast on a vaulted card, looped ~4 times before escalating `no_progress_context_cap`). The escalation hands the ticket back to Sol so `resolvePlaybookForDirection`'s DB-backed resolver runs against a fresh Sol-authored Direction with the customer still in the conversation. Trust boundary is preserved: `unit_cents` + `vendor` are NEVER sourced from the decision — see the sec:real-vuln note on `resolveAssistedPurchaseIntentToParams`.
  ...
  - **Cancelled-sub reactivation fallback** ([[../specs/assisted-subscription-purchase-resolves-variant-when-routing-omits-params]]): before the escalate branch, `handlePlaybook` also tries [[#`resolveAssistedSubscriptionIntentFromCancelledSubs`]] when (a) the slug is `assisted-subscription-purchase`, (b) `extractAssistedPurchaseIntentFromDecision` returned null, and (c) `ctx.customerId` is set. The sibling projects the customer's `status='cancelled'` subs (via [[commerce-subscription]] `listSubscriptionsByCustomer`) onto the pure [[#`matchCancelledSubForStatedProduct`]]; if the customer named exactly one cancelled sub's product in this turn's message, the returned `RawAssistedPurchaseIntent` still flows through `resolveAssistedPurchaseIntentToParams` (same variant re-verification, same server-side price + vendor). Only `assisted-subscription-purchase` has a cancelled-sub anchor — the order slug stays strict. Ground-truth case: ticket `e5dedbf8` — Deborah Kacprowicz asked to restart her cancelled Cocoa French Roast subscription '2 bags every 2 months' (contract `28425617581`); the original refusal escalated even though her cancelled sub carried the target variant + cadence unambiguously. On the fallback path, `sysNote` records `resolved from cancelled_sub_reactivation` so the routing decision is auditable.
