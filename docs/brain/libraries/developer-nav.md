...
- **`DEVELOPER_GROUPS: DeveloperNavGroup[]`** — the developer surfaces **grouped + in sidebar order**: **Org** (Message Board · Org Chart · Directors · Agents — moved here off the main tree) · **Development** (Goals · Pipeline · Build box · Control Tower · Approvals · Taxonomy map · Message Center · Spec Tests · Human QA · Regressions · Security tests · Branches) · **Resources** (Brain). Each group is `{ heading, items }`; each item: `href` · `label` · `icon` (heroicon path) · `desc` (Overview card copy) · optional `badge`. Excludes the Overview itself (the portal home).
