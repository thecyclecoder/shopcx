...
### `subscriptionLoyaltyCouponCodes` — function

```ts
function subscriptionLoyaltyCouponCodes(appliedDiscounts: unknown): string[]
```

Same-vs-different-code splitter behind the `apply_loyalty_coupon` stacking guard (spec: apply-loyalty-coupon-same-code-reapply-is-idempotent-success-not-stacking). Returns the LOYALTY-* / SMILE-* codes currently attached to a subscription's `applied_discounts`, uppercased for comparison. Same tolerant shape family as `subscriptionHasLoyaltyCoupon` (bare string · `{title}` · `{code}`). The handler uses it to distinguish a SAME-CODE re-apply (return success — the desired terminal state is already the case, an idempotent no-op) from a DIFFERENT-CODE stacking attempt (refuse — the real single-`LOYALTY_REMEDY_MAX_CENTS` ceiling the loyalty-remedy-hard-cap spec exists to protect). The pre-fix predicate returned true for ANY LOYALTY-*, so the first apply landing the coupon caused every subsequent retry to hit the coarse "already has a loyalty coupon" refusal — the required-outcome gate then blocked auto-close and re-escalated (ticket be3d6ab7-7a33-40c7-ab13-faee84dfb0b1: coupon LOYALTY-15-8M8HKP landed on internal-f2a92aa057f94a2e on the first apply, three retries all failed the guard → false escalation to June + "applied!" vs "team is working on this" contradiction to the customer). Case-insensitive on the LOYALTY- / SMILE- prefix. Tests: `src/lib/action-executor.loyalty-order-stacking-guards.test.ts`.

### `hasDiscountCode` / `stillHasDiscountCode` — function

```ts
function hasDiscountCode(appliedDiscounts: unknown, code: string): boolean
const  stillHasDiscountCode = hasDiscountCode  // semantic alias
```

Shared shape-tolerant matcher used by BOTH sides of `verifyActionInDB`'s coupon read-back. True when `subscriptions.applied_discounts` carries `code` under ANY of the stored shapes the codebase writes (bare string · `{title}` · `{code}` · `{id}`), case-insensitive because `resolveCoupon` returns the caller's casing. Exported under two names so the call sites read symmetrically — `hasDiscountCode(...)` on the apply side, `!stillHasDiscountCode(...)` on the remove side ("the code is truly gone"). Load-bearing on both sides:

- **`apply_coupon` / `apply_loyalty_coupon` verify** — the pre-fix predicate was `discounts.some(d => d.title === action.code)`, matching only the `{title}` shape. Internal subscriptions write the `{code}` shape (that `computeAppliedDiscountCents` honors), so a coupon that was correctly applied verified as failed, the outcome-completion gate blocked auto-close, and the resolved ticket false-escalated to a human (Janelle Heath 2026-08, ticket `715658b5-934e-4ec8-937f-6ea749e8d3ea` — `$10` LOYALTY-10-Z2XRVP genuinely applied to K-Cups sub `internal-e392298b48834705` yet escalated as `apply_loyalty_coupon[failed]`). The fix routes the apply-side verify through `hasDiscountCode`, so a coupon present under any shape verifies true. Tests: `src/lib/action-executor.verify-apply-coupon.test.ts`.
- **`remove_coupon` verify** — pre-fix, `verifyActionInDB` had no `remove_coupon` case and the switch hit `console.warn(uncovered action type — assuming OK: remove_coupon)`, letting a silent no-op remover ship an "I removed it" message onward (Randi Stier 2026-08-10, ticket `c2bc8bd8-2aca-4eeb-968b-dd968a3d0dbc`). The remove-side case re-reads `applied_discounts` and returns `verified=false` when `stillHasDiscountCode` is still true, so the same class of false success cannot pass even if a future variant of the shape mismatch re-emerges. Tests: `src/lib/action-executor.verify-remove-coupon.test.ts`.
...
- **`apply_loyalty_coupon` stacking guard is code-aware — a SAME-code re-apply is idempotent success, not a "no stacking" failure.** The upfront stacking guard (spec: apply-loyalty-coupon-same-code-reapply-is-idempotent-success-not-stacking, ticket `be3d6ab7-7a33-40c7-ab13-faee84dfb0b1`) reads `subscriptions.applied_discounts` and splits on which LOYALTY-* codes are present via `subscriptionLoyaltyCouponCodes` (exported): if the ONLY loyalty code on the sub is the SAME code being applied (case-insensitive), return `{ success: true, summary: 'Loyalty coupon <code> already applied … (idempotent no-op)', couponCode: code }` — the desired terminal state is already the case, the required-outcome gate clears, and the escalation self-retires; if a DIFFERENT loyalty code is present (or the same-code hit sits alongside a different-code hit), refuse with the single-`LOYALTY_REMEDY_MAX_CENTS`-ceiling error the loyalty-remedy-hard-cap spec intended. Pre-fix, the guard used the coarse `subscriptionHasLoyaltyCoupon` predicate which returned true for ANY LOYALTY-* — so the first apply landing LOYALTY-15-8M8HKP on `internal-f2a92aa057f94a2e` caused three retries to all fail the guard, false-escalating to June with the "applied!" / "team is working on this" contradiction to the customer. **Ordering invariant (Fix 1 — pre-merge security review):** `refuseLoyaltyCouponIfCustomerMismatch` runs BEFORE the same-code idempotent-success return, so an attacker-controlled state where `applied_discounts` already carries the incoming code but the code belongs to a DIFFERENT Shopify customer than the contract owner refuses with the mismatch error — never stamps a successful authorization for a coupon this contract owner does not own. The cross-customer/mismatch (`refuseLoyaltyCouponIfCustomerMismatch`) and single-ceiling (`LOYALTY_REMEDY_MAX_CENTS`) rails remain intact. Tests: `src/lib/action-executor.loyalty-order-stacking-guards.test.ts` (25 tests, including the handler-level ordering regression).
...
- **`create_subscription` / `create_order` failure inside an assisted purchase escalates the ticket + mints a CEO card** ([[../specs/create-subscription-internal-branch-cannot-create-a-subscription]] Phase 2, derived from ticket `687b2e7a` — Susan Bellamy). When the terminal step of an assisted-purchase flow fails — either the [[playbook-executor]] `handleAssistedCreate` branch OR the cs-director `handleApproveRemedy` branch when the escalated batch included one of these actions — the shared side-effect in [[assisted-purchase-failure-escalate]] `escalateAndCardOnAssistedPurchaseFailure` fires: (1) `tickets` gets `status='open'` + `escalated_at=now()` + `escalation_reason=<why>` (compare-and-set on workspace_id + id) so the ticket does NOT sit `open` + `escalated_to = null`; (2) a CEO `dashboard_notifications` `agent_approval_request` card is inserted via [[assisted-purchase-failure-card]] `buildAssistedPurchaseFailureCard` naming the customer + the exact plan agreed to + the concrete failure + the origin (`playbook` vs `director_remedy`). Reuses the `agent_approval_request` surface `buildApprovalsFeed` reads — the founder sees it in the same escalated-set as [[cs-director-escalate-founder-card]] + [[cs-director-author-spec-failure-card]]. Best-effort per side-effect (a failed escalation write does NOT swallow the customer-visible response the interpreter returned). Susan Bellamy's ticket sat open + unowned across TWO silent failures (2026-08-07 playbook, 2026-08-09 director remedy) precisely because this rail was missing; the 2-week / 15-message customer chase is what surfaced it.

- **`create_order` / `create_subscription` open with an unconditional vaulted-PM guard** ([[../specs/assisted-purchase-playbook]] Phase 1). The guard reads [[../tables/customer_payment_methods]] across the customer's link group via [[customer-links]] `linkGroupIds` — a card vaulted on a linked sibling counts (payment-method-lookups-must-span-linked-accounts spec Phase 1). It keeps only `status='active'` rows via `pickChargeableVaultedPm`, and — on a miss — launches the [[../journeys/add-payment-method]] journey via [[journey-delivery]] `launchJourneyForTicket`, writes an internal `[System] {action} deferred — no vaulted payment method` note on the ticket, and returns `{ success: false, error: 'no_vaulted_payment_method', summary: '{action} deferred …' }` WITHOUT calling the commerce effector. Fail-closed and unconditional (no flag bypass) — a missing PM can never reach `createOrder` / `createSubscription`. Pinned by `action-executor.vaulted-pm-guard.test.ts`.
  - **Deliberate: only the Braintree vault (`customer_payment_methods`) is consulted.** A Shopify-vaulted card on the same person does not satisfy this guard, and the assisted-purchase journey fires even when one exists. The downstream effectors (`create_order` / `create_subscription`) drop through to the Braintree rail whenever possible, so a Shopify-only card is not what the guard is set up to charge. Revisit if/when a Shopify-vaulted card becomes reachable from the same effector.
...
- **`update_line_item_price` and `partial_refund` no longer accept the agent's arithmetic as a sanctioned source for a price correction — the computed rate is the ONLY sanctioned source, and agents may TRIGGER a correction but not AUTHOR the number.** `update_line_item_price` resolves the subscription for `p.contract_id`, calls [[subscription-overcharge]] `detectOvercharge`, and hands the (signal + agentBase + projector) to `deriveRestoreBase` from the same module. When a signal names the target variant its `restore_base_cents` wins and `p.base_price_cents` is only logged; when no signal exists the handler refuses a raise, refuses an immaterial change (>= $1 AND >= 2%, the floor `detectForSubscription` uses at detection — the 2026-08-02 $0.01 catalog-drift failure mode), and confirms the projected realized via [[pricing]] `resolveSubscriptionPricing` so an internal sub's stacked quantity break can't overshoot the established rate. `partial_refund` **no longer silently clamps** (spec [[../specs/a-clamped-refund-must-never-report-success]] Phase 1): when a signal exists and the agent over-asks, the handler returns `{ success: false, error: "... exceeds the signal-computed overchargeDelta ..." }` so no customer-facing message claiming the original amount can ship on the back of a downward-corrected refund. The failure disables the reply path, so the ticket escalates instead of falsely promising the customer the requested amount (SC137733: $52.91 requested, $5.01 refunded, customer told $52.91). Under-asks are still allowed as partial goodwill, logged for audit. `full_order_refund` refunds the order's collected `total_cents` verbatim (no agent-supplied amount) as a founder-authorised alternative to agent-invented figures. Both apply the same materiality floor whenever claimed as overcharge remediation (signal present OR reason mentions "overchar"). Every successful line-price write emits a `customer_events` row (event_type=`subscription.line_price_changed`, exactly as the spec names it) carrying the previous realized per-unit, the new one, the source (`overcharge_signal` vs `agent_supplied`), the ticket id, and the raw signal + agent bases — the audit trail whose absence made the r.aycock $54.78 overwrite of a verified $44.96 restore invisible for a day. Every refused raise (either the no-signal `raise_no_signal` predicate or the signal path's `exceeds_established` guard) emits an `agent_message` [[../tables/dashboard_notifications]] row for the CEO — a price increase on an existing subscriber is a decision an objective-owner should see, per the CLAUDE.md north-star rule: hitting a rail escalates, it does not execute. Tests: `src/lib/subscription-overcharge.derive-restore-base.test.ts` pins the three verification scenarios (signal overrides agent · raise-no-signal refused · lower-no-signal allowed) purely — no live DB, the projector is a fixture. Spec: [[../specs/a-price-correction-must-use-the-computed-rate-not-an-agents-arithmetic]].

- **`full_order_refund` is non-autonomous — it REQUIRES founder approval via `june_remedy` + `executeParkedRemedy`.** The handler refunds `orders.total_cents` verbatim (no agent-supplied amount negotiation), but it is guarded by `if (!ctx._founderApprovedFullOrderRefund) return { success: false, error: ... }`. This flag is set ONLY on the ActionContext inside `executeParkedRemedy` when a `june_remedy` card has been explicitly approved by the founder; any other caller (Sol cheap-execution, a raw Sonnet `direct_action`, a journey/playbook/workflow) reaches the handler with the flag unset and receives a refusal. The refusal routes the response through the escalate path (no customer-facing message ships), so an unapproved full refund can never be promised to the customer. This is the HANDLER-LEVEL GUARD (second rail after [[june-remedy-approval]] `MONEY_ACTION_TYPES` gate + [[agent-action-queue]] deny-list + [[required-outcomes-validator]] removal) that enforces the north-star rule: money moves only on an objective-owner's say-so. Spec: [[../specs/a-clamped-refund-must-never-report-success]] Phase 2 & 3. A founder-authorised full refund becomes a SANCTIONED route (spec phrase "Give that a sanctioned route rather than leaving the only options 'clamped' or 'bypass the guard'") when an agent sees the clamp refuse a downward-corrected `partial_refund` and needs an alternative that doesn't invent a number. **ALSO — customer binding prevents same-workspace cross-customer refunds (Phase 1 of [[../specs/full-order-refund-must-bind-ticket-customer]], widened by `create-return-order-lookup-must-span-linked-accounts`):** the handler bails upfront when `ctx.customerId` is missing and filters the order lookup by `.in("customer_id", linkGroupIds(...))` in addition to workspace_id, so a founder-approved parked remedy on ticket T (customer A) cannot refund an order owned by an unrelated customer B — even if the payload names a valid same-workspace order number. Linked accounts are the same person (see [[customer-links]]), so a linked-sibling order is accepted; an unrelated customer's order still returns zero rows. The refusal happens BEFORE `@/lib/refund` is loaded, so no network side effect fires on a cross-customer refusal. Tests: `src/lib/action-executor.full-order-refund-customer-binding.test.ts` (5 cases: cross-customer by shopify_order_id, cross-customer by order_number, empty customerId, unapproved-founder still refuses first, same-customer still succeeds).

- **`create_return` order-ownership lookup spans the customer's link group** (spec: `create-return-order-lookup-must-span-linked-accounts`, ticket bafa0f7a-447b-4cdf-a44b-f4fca0decce6). The `create_return` handler used `.eq("customer_id", ctx.customerId)` on its orders lookup, so an order sitting on a linked-sibling profile — same person, separate customer row — was invisible and `create_return` returned "Order not found" on a genuinely-owned order (ground truth: SC138373 owned by linked peer 270c88ba while the ticket's `ctx.customerId` was 827b7894). Widened to `.in("customer_id", linkGroupIds(admin, workspaceId, ctx.customerId))` via [[customer-links]]. The ownership property is preserved — unrelated workspace customers still return zero rows because `linkGroupIds` only expands to the true peers — while the linked-sibling case that #2816 fixed for payment methods is now fixed for order-ownership lookups too. The same widening lands on every sibling `.eq("customer_id", ctx.customerId)` filter in the executor: `update_line_item_price`'s crisis lookup, `partial_refund`'s loyalty-member reconcile, `redeem_points_as_refund`'s member lookup, `crisis_set_auto_readd`'s crisis lookup, and `verifyActionInDB`'s `scopeSub` / `scopeOrder` verifier helpers. Test: `src/lib/action-executor.create-return-linked-accounts.test.ts`.
...
- **`remove_item` short-circuits to the local items path on internal subs** (ticket `c13fbad1`, Kristy Teague's bundled split). Variant-driven removal fetches the live contract via [[commerce__subscription]] `subscriptionGetLiveContract`, which returns `{ internal: true, lines: { nodes: [] } }` for a sub with `is_internal=true` (internal subs manage lines on `subscriptions.items` by variant_id, not vendor line gids). When `contract.internal` is true the handler delegates straight to `subRemoveItem(workspaceId, contract_id, { variantId })` — the same wrapper the direct-lineId branch uses — which routes through [[internal-subscription]] `internalSubRemoveItem` and filters the local items JSON. Without this branch the empty `lines.nodes` array proxied as "No lines matching variant … on contract" even when `subscriptions.items` literally carried the variant, blocking every internal sub's `remove_item` and leaving Kristy's Vanilla Creamer line on bundle `21683c68` after June's approved split. The multi-line Appstle loop (line_gid resolution across duplicate lines) is unchanged.
