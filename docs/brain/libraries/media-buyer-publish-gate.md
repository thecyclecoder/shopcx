...
- [[../inngest/ad-tool]] `adToolPublishToMeta` — RE-CHECKS the gate on the loaded job just before `createAd`. On refusal it DOWNGRADES `publish_active=false` (writes the change back to `ad_publish_jobs`) and escalates. This catches a route-bypass (an ad-hoc script fires `ad-tool/publish-to-meta` directly) or a cohort retired between insert and publish. **It MUST pass `createAdsetSpec: perTestSpec` (the loaded `ad_publish_jobs.create_adset_spec`) into the gate** — the exclusion rail reads the proposed targeting from `createAdsetSpec?.targeting ?? targeting ?? null`, so an omitted spec makes the rail see `null` and refuse `missing_purchaser_exclusion`/`missing_customer_exclusion` on EVERY per-test publish. Guarded by `ad-tool.publish-gate-spec.test.ts` (a source-contract test) so the caller can never again hand the rail a null spec.

> **Incident 2026-07-21 — fleet-wide silent test-launch freeze.** The `adToolPublishToMeta` re-check called the gate WITHOUT `createAdsetSpec`. Harmless until the cohorts got their exclusion-audience ids populated — from that moment every per-test publish across all 6 hero products read `null` targeting, refused `missing_purchaser_exclusion`, and posted PAUSED. The CEO escalation deduped/never surfaced, so the only signal was a paused adset spotted on Meta. Fixed by passing `createAdsetSpec: perTestSpec`; the gate's own exclusion logic was correct and already tested. Lesson: a rail gated on a config field (`excluded_purchaser_audience_id`) that flips from null → set can arm a check the caller silently can't satisfy — cover the caller contract, not just the gate.
