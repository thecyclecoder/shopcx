...
- **AGENT-KIND BARE-SLUG CONVENIENCE (ad-creative-box-session-only-retire-deterministic-path Phase 3 — 2026-07-19).** At an agent-kind cursor the walk ALSO honors the bare agent-kind slug alongside the canonical `agent:<slug>` / `agent-kind:<slug>` id — mirrors the department-key convention above. Closes the 2026-07-19 gap where a `kill_switches.node_id='ad-creative'` row (the bare slug — the form the CEO cockpit surfaces) failed to suppress an `agent:ad-creative` claim through the resolver, letting the daily cadence produce ~2 queued+claimed jobs while the switch was frozen. Pinned by the two `agent-kind bare-slug` cases in [[../../../src/lib/control-tower/kill-switch-resolver.test.ts]] (bare slug resolves both canonical and slug lookups OFF; sibling agent-kinds stay ON — no over-broad match). The DB-side mirror ([[node-ancestry-sync]] `withKindAlias` → [[../tables/node_ancestry]] `ancestors[]`) reflects the same alias so `public.claim_agent_job`'s ancestor join matches identically.
