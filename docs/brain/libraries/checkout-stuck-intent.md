...
Pure predicate that recognizes a CHECKOUT-STUCK customer message as a first-class intent — distinct from the coarse `account` / `general` / `outreach` buckets the [[unified-ticket-handler]] classify-bucket step returns. Phase 1 of [[../specs/checkout-stuck-defaults-to-assisted-purchase-concierge-sonnet-and-sol]]. A customer who "can't check out", whose OTP / verification code isn't arriving, who is "stuck at the payment or authentication screen", or who asks "how do I finish my order" is a candidate for the assisted-purchase concierge flow — not a stateless "try another card" dead-end reply from the cheap orchestrator.
...
The Phase-1 predicate recognizes the intent as its own thing so Phases 2-5 can special-case it:
- **Phase 2** — [[model-picker]] keeps checkout-stuck on Sonnet even when `recentMergesCount > 0`, and the drift/re-session router flags Sol back in.
- **Phase 3** — Sol's Direction launches the `add-payment-method` [[../journeys/add-payment-method]] then confirms items, then asks one-time vs S&S.
- **Phase 4** — re-enables the `assisted-order-purchase` / `assisted-subscription-purchase` playbooks behind Sol's session-chosen selection.
- **Phase 5** — analytics slice + brain updates for the concierge flow.
