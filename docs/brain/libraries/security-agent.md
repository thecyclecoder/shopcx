...
3. **`diff` mode** — `git fetch origin main` + `git show {sha}` read-only on Max → ONE verdict:
   ...
   - **Unparseable/unrecognized verdict** ([[../specs/needs-attention-triage-and-verdict-robustness]] Phase 2 + [[../specs/iteration-ingest-async-reports-fix-tooling-69594a]]) → the shared `resolveReviewVerdict` helper (shared with [[repair-agent]] + [[regression-agent]]) first attempts a **same-session JSON parse-repair re-prompt** (`securityReviewRepairPrompt`) — the model already spent the budget doing real review work, so re-emitting ONLY the verdict envelope from the existing context recovers a verdict far cheaper + far more often than a fresh re-investigation. If the parse-repair also fails to produce a recognized verdict (or no prior session exists), it fail-safes to an **actionable** `needs_attention` reason — `"security review produced no parseable verdict after 2 attempts — re-run or review manually: <excerpt>"` — **never** a bare "ended without a recognizable verdict", never an auto-pass. The director's `reconcileNeedsAttention` ([[platform-director]] Phase 1) then triages that parked item.
