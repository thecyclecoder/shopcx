...
- Returns `products` (forest), `unattributedEntry` (non-product, non-blog landings e.g. `/checkout` — surfaced separately but **included in `grandTotal`** so it reconciles), `blogEntry` (the synthetic **"Blog"** top-level bucket for `/blog` first-touch sessions that don't resolve to a product handle — same shape as `unattributedEntry`, also **included in `grandTotal`**), and `grandTotal` (all sessions combined). Each node carries `FunnelNodeMetrics`: the 5 step counts + `add_to_cart`, plus `engagement_rate` (engaged/visit), `conversion_rate` (order/visit), `atc_rate`.

**Blog as a top-level entry bucket:** since the `blog-pixel-tracking` spec (phase 1, in `public.specs`) the storefront blog fires `blog_view` + `blog_engaged`. A first-touch `/blog` landing (path contains a `blog` segment and resolves to NO product handle) routes to the synthetic top-level "Blog" node instead of "Unattributed entry" — so **blog→bounce vs blog→product is legible** in Max's funnel. The blog-events are intentionally NOT in `STEP_OF_EVENT`, so a blog session contributes only a `visit` on the Blog node and can NEVER inflate any product's `visit` / `pdp_view` / `engaged` / etc. Product-slice hides the Blog node the same way it hides `unattributedEntry` (blog isn't a product). Blog→product session flow ALSO surfaces via the composing **"Blog" referrer slice** on product nodes (a session that clicked blog→product still lands on the product node, with `referrer=Blog`). First-touch attribution stands: a blog reader who opens a PDP later in the same session stays on the Blog node.
