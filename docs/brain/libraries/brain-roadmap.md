...
- **`GoalCard.accumulation`** (`GoalBranchAccumulation`) + **`deriveGoalAccumulation(memberSpecs, exempt)`** ([[../specs/spec-goal-branch-pm-flow]] M6) — the goal-branch accumulation surface: `{ onGoalBranch, totalSpecs, allOnGoalBranch, exempt, exemptReason }`. Derived (pure) from the goal's deduped member SpecCards' `onGoalBranch` flags + the parent-goal exemption (`deriveGoalExemption(row, allRows, linkedSpecCount)`, which mirrors `goals-table.isGoalParentExempt` — `is_parent` OR has-sub-goals OR no-buildable-specs — from already-loaded rows, no extra DB reads). `allOnGoalBranch` ⇒ the "⬆ ready to promote" badge; `exempt` ⇒ the parent-goal "sub-goals promote independently" note. Consumed by `GoalAccumulation` on the goals board + detail.
