...
`scripts/builder-worker.ts` `runSpecTestJob` invokes `runSpecChecks` in-process and writes the runner's verdicts straight to [[spec-test-runs]] — post-ship AND pre-merge (see § graduate-vera in the header — the Vera Max session is retired, so nothing gets "spawned" here anymore, and Vault's security review is its own solo `security-review` job enqueued on completion, not a fused verdict). A `needs_human` bullet the runner can't execute stays surfaced; a runner exception is a HARNESS error → a re-runnable `error` run, never a false `fail`. Every invocation beats the Control Tower loop `DETERMINISTIC_SPEC_CHECK_RUNNER_LOOP_ID` (ok:true when the runner returned verdicts; ok:false when it threw — a threw run is re-tried, no LLM fallback exists) — the "monitored, not graded" liveness assertion the [[agent-grader]] carve-out on `spec-test` + null `claude_session_id` pairs with.
...
## The reconcile-before-defer self-heal — [[check-reconciliation]]

A phase's `expect: 'present'` grep check pins an EXACT literal the spec author guessed BEFORE the code existed. When the builder ships functionally-correct code under a different literal (a renamed symbol, `IS NULL` vs `is null`, a reworded token), a git-grep of the stale literal finds no match, `verifyPhaseAccumulatedOnBranch` marks the phase unverified, and `isSpecAccumulationComplete` returns `complete=false` — the completed build then parks with a DEFERRED PR and burns the redrive budget. A false negative.

`finalizeBuiltPhase` in [[builder-worker]] now calls [[check-reconciliation]] `reconcileFailingGrepChecksForSpec` on `acc.complete === false` BEFORE the defer. Two ordered steps per failing check:

1. **Normalized re-match** — a case-insensitive, whitespace-tolerant ripgrep against the same path. A hit repoints the pattern to the actually-present casing.
2. **Bounded intent judge** — a Claude Sonnet judge reads the check's `description` (INTENT) + the phase's branch DIFF for the check's path. If the intent is satisfied under a different literal that's actually IN the diff, it returns that literal; a fabricated one is caught by the FINAL deterministic ripgrep of the proposal.

The runner itself is unchanged — the reconciler wraps around it: after a successful repoint via [[spec-phase-checks-table]] `upsertPhaseChecks`, the runner's own `grep` executor is what re-verifies. The FINAL gate is always a real deterministic grep of the corrected pattern (no bypass, no phantom-ship). `expect: 'absent'` misses are NEVER reconciled — an absent-should-be-absent-but-is-present check is a real signal.
...
[[spec-phase-checks-executable]] · [[spec-phase-checks-table]] · [[spec-check-db-probes]] · [[spec-test-runs]] · [[spec-test-harness-classifier]] · [[builder-worker]] · [[check-reconciliation]] · [[agent-jobs]] · [[../specs/machine-declared-verification-and-deterministic-spec-test-runner]] · [[../specs/spec-test-agent]] · [[../specs/merge-gate-verifies-real-phase-checks-not-status-flags]]
