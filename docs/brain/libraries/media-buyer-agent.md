...
- `kill` — a decision-tree loser from [[media-buyer__meta-cpa-signal]] `detectMetaCpaLosers` triggers a `pause` (via `iteration_actions`). The kill fires ONLY on the crown/kill decision-tree: leading-signal trim (cost-per-ATC / CPM / clicks-no-ATC, with the converter guard so an adset converting at/under `crown_max_cpa_cents` is never trimmed early), max_test_spend deadline (spend ≥ `max_test_spend_cents` without reaching the hold band), and 0-purchase backstop (spend ≥ `crown_min_spend_cents` with no purchases). The legacy ROAS-floor / `pause_min_spend_cents` kill trigger is RETIRED ([[../specs/media-buyer-kill-on-decision-tree-retire-roas-floor]] Phase 1) — a test with sales, under the deadline, within/near the hold band is NEVER killed. The action cites the source `meta_ad_id` so the audit trail names the creative, not just the wrapper.
