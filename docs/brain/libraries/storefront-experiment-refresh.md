...
4. **Phase 4 decision** (`decideExperiment`) for non-rolled-back experiments → promote/kill/hold; persists status + `last_decision` snapshot. A `kill` on an offer experiment ALSO sweeps its linked offers via `rollbackRenewalOffersForExperiment` (same un-touch contract as rollback).
