...
- **`GoalRow`** — `{ id, workspace_id, slug, title, body, outcome, success_metric, owner, proposer_function, parent_goal_id, is_parent, status, created_at, updated_at, milestones: GoalMilestoneRow[] }`. `is_parent` (spec-goal-branch-pm-flow M5) marks a PARENT goal — exempt from the atomic goal→main promotion.
...
- **`setGoalIsParent(workspaceId, slug, isParent)`** (spec-goal-branch-pm-flow M5) — the sanctioned writer for `goals.is_parent`. Marks/clears a goal a PARENT (contains sub-goals, no direct buildable specs) — a parent is EXEMPT from the atomic goal→main promotion. A single slug-resolved UPDATE.
- **`goalHasChildGoals(goalId)`** → `boolean` (spec-goal-branch-pm-flow M5) — does ≥1 other goal name this one as `parent_goal_id`? One of the structural fallbacks `isGoalParentExempt` ORs (CEO Mode is a parent TODAY via this, before anyone sets the flag). Fails CLOSED (treat-as-parent) on a read error.
- **`isGoalParentExempt(workspaceId, slug)`** → `{ exempt, reason }` (spec-goal-branch-pm-flow M5 — the parent-goal exemption) — is this goal EXEMPT from the atomic goal→main promotion? Exempt iff ANY of: (a) `is_parent === true` (explicit override), (b) it HAS child goals (`goalHasChildGoals`), or (c) it has NO buildable member specs (none linked through any milestone). A parent has no `goal/{slug}` branch to merge; its children promote independently. Read-only through the goals/specs SDK; fails CLOSED (returns exempt) on a read error — an unknown goal must never be force-promoted. Called by [[agent-jobs]] `promoteCompleteGoalsToMain`.
