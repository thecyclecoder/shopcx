...
- `## Decomposition` → one `### M{position} — {milestone.title}  _(status)_` per milestone (status DERIVED from the milestone's child specs via `deriveMilestoneStatus` — `goal_milestones.status` was dropped in `derive-rollup-status` P3), then a `_milestone_id: {uuid}_` line (the stable `goal_milestones.id`), the milestone body, then a `- [[../specs/{slug}]] — {title} _(status)_` bullet per child spec. The `M{position}` handle + the `milestone_id` line are what the **goal planner** reads to bind each proposed spec to a real milestone (`specs.milestone_id`) — the goal→milestone→spec link is DB-driven, resolved by `runPlanJob` from this render.
