...
| `decideApproval(admin, { approvalId, decision, questionText? })` | Terminal-flip one approval to `approved` / `denied` / `asked`. Idempotent (no-op if already terminal). `ask` requires `questionText`. **`asked` is a re-ask block** ([[../specs/one-open-escalation-per-thing-and-a-founder-answer-stops-the-asking]] Phase 3) — [[june-remedy-approval]] `raiseJuneRemedyApproval` / `raiseFounderApproval` consult `readReAskState` before minting, and an open `asked` card for the same (ticket, category) subject BLOCKS the new mint + returns `via:'blocked_by_asked'` with the `questionText` so the caller feeds it into the next cs-director-call run. Bounded — after `JUNE_REASK_CEILING` (5) mints in `JUNE_REASK_WINDOW_MS` (24h) for the same subject, ONE `escalation_kind='asked_ceiling'` `dashboard_notifications` card fires instead (deduped by Phase 1's DB unique open-card index). |
...
| `sendGodModeSMS(admin, { workspaceId, kind, cockpitToken?, context? })` | Phase-5 emit — best-effort, never throws. `kind: 'arm' \| 'approval' \| 'done'` plus the director-cockpit variants `'director-arm' \| 'director-approval' \| 'director-done'` (M4 of [[../lifecycles/director-cockpits]]) that interpolate `context.personaName` so a leash-bound director cockpit's text reads in that director's name, never Eve's flirt. Called from `armSession` (arm), `nudgeStalePendingApprovals` (approval — the 5-min reminder, batched via `context.count`), `disarmSession` + `reapGodModeSessions` (done), and [[director-coach-threads]] `armDirectorCockpit` / `nudgeStaleDirectorApprovals` (the director-* kinds). Sends the persistent cockpit URL in arm + approval; done omits URL. Silent no-op when no founder phone or no workspace `twilio_phone_number`. |
...
The `sendGodModeSMS` inside this SDK stays reserved for cockpit lifecycle texts only — the god-mode session (arm / 5-min approval nudge / done) plus the director-cockpit variants ([[director-coach-threads]], M4 of [[../lifecycles/director-cockpits]]). External founder alerts get their own module so their idempotency ledger, kind vocabulary, and best-effort discipline don't leak into the cockpit approval-nudge state machine.
