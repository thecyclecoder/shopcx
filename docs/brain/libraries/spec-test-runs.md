...
- `getAutoFoldEligibleSlugs(workspaceId)` / `getFoldRefusalsBySlug(workspaceId)` — the fold gate's **two rails from ONE evaluation** (both call the internal `evaluateAutoFoldGate`). `getAutoFoldEligibleSlugs` returns `string[]` (the ready-to-fold slugs, unchanged shape/callers). `getFoldRefusalsBySlug` returns `Map<string,string>` — the per-slug refusal reason the gate computed but would otherwise discard, populated for every shipped-and-un-archived spec the gate REJECTED this pass (an eligible spec has no entry). The [[pipeline-doctor]]'s `shipped-not-folded` classifier ([[../specs/a-shipped-spec-that-cannot-fold-is-stuck]] Phase 1) reads this so the board reports the fold gate's OWN reason instead of a generic "investigate" — same single-definition rule `isCleanMachinePassRun` already enforces for spec-test greenness. The eligibility criteria below are what BOTH derive from:
