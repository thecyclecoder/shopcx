...
Phase 3 — the deterministic publish-time gate. Returns `{ ready: true }` when the campaign carries all 3 placement statics (canonical feed_4x5 + 9:16 sibling + right_column_1x1 sibling, all `media_kind='static'` AND `status='ready'`) AND the resolved copy surface's `metadata.copy_pack` (the angle's, else the campaign's — see `resolvePackCopySource`) holds ≥4 headlines + ≥4 primary texts. Otherwise returns `{ ready: false, reason, detail }` with one of seven stable `CreativePackIncompleteReason` values so Bianca's publish gate can branch on the machine-readable reason (never free-text).
...
- **Copy pack lives on the angle — OR on the campaign:** The 4×4 headlines + primary texts are persisted on `product_ad_angles.metadata.copy_pack` (backward-compatible JSONB, no schema change), not on a new column. The [[creative-agent]] `insertReadyCreative` stamps this via `planCreativePackInserts.angleMetadataCopyPack`. **But the manual lane has no angle at all** — [[ads__manual-creative]] exists precisely for creatives produced outside Dahlia's autonomous lane, and it writes the identical pack to `ad_campaigns.metadata.copy_pack` with `angle_id` left NULL. `resolvePackCopySource(angleMetadata, campaignMetadata)` (pure) is what the caller uses to pick between them — angle first, campaign second.
- **⚠️ The angle-only read was a silent publish block (2026-09-23).** `isCreativePackComplete` read the angle's metadata only, so EVERY manual-lane static refused at the publish gate with `copy_pack_missing` while a complete 4×4 pack sat on the campaign row. Ground truth: the two Amazing Coffee K-Cups statics (`58a2fe1a` / `394bf6cf`) — 3/3 placement statics `ready`, 5 headlines + 5 primary texts on `ad_campaigns.metadata.copy_pack`, `angle_id: null` → both refused. A probe found **zero** angles anywhere carrying `metadata.copy_pack`, and the only writer ([[creative-overlay-landing]]) updates that field ONLY when the campaign already has an `angle_id` — so minting an angle would not have populated it either. Fixed by `resolvePackCopySource` at the [[../inngest/ad-tool]] caller; the predicate itself stays pure.
