...
| `setCompetitorMetaResolution(id, { meta_page_id, meta_resolved_name, meta_likes, meta_resolved_via }, { workspaceId? })` | **winners-flow Phase 1** — persist a competitor's resolved Meta advertiser identity (the `meta_page_id` the winners scan targets). Stamps `meta_resolved_at`. Populated by `scripts/resolve-competitor-advertisers.ts` calling [[adlibrary-winners]] `resolveAdvertiser`. `meta_page_id=null` ⇒ unresolved = a **reliable bad seed** (unlike the old 0-ads flag). See [[../integrations/adlibrary]] (`/api/advertisers/search` → `/api/winners/advertiser/{pageId}`). |
