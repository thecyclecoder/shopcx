...
## Cold-scaler graduate heartbeat (Bianca — Phase 3)

`deliverMediaBuyerDigest` accepts an optional 4th arg `cohortHeartbeats: readonly CohortGraduateHeartbeat[]` (see [[cold-scaler-graduate-heartbeat]]). When non-empty, `composeDigest` appends a `Cold-scaler graduates:` section under the promote/kill lines — one line per active cold-scaler cohort showing `cohort {id8} — last graduated {age or "never"}, {N eligible winners}[, last skip: {reason}]`. The caller ([[builder-worker]] media-buyer lane) computes the heartbeats via `runColdScalerGraduateStallCheck` right before the digest post — the same call ALSO raises a deduped CEO stall card per (workspace, cohort, UTC day) for any cohort whose stall predicate holds (`eligibleWinnerCount > 0 && !lastGraduatedAt`). Zero-eligible cohorts do not escalate — that's the healthy quiet rail, and alerting on it would train the CEO to ignore the signal (the exact anti-pattern the 2026-07-27 incident exposed).
