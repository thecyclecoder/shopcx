...
`ApprovalFeedItem` carries: `source` (`pending｜decision`) · `status` · `actionable` (escalated-to-CEO **and** has inline actions) · `kind`/`typeLabel` · `raisedBy`/`routedTo`/`decidedByLabel`/`autonomous` · `spec`/`goal`/`milestone`/`phase` · `title`/`summary` · and the pending affordances `jobId`/`actions`/`deepLink`/`escalatedBy`.
