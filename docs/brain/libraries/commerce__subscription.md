...
- `latest_order` — `{ id, order_number, financial_status, delivery_status, total_cents, created_at, delivered_at }` from the most recent [[../tables/orders]] row keyed on `subscription_id`. `null` when the sub has never billed. Full `OrderView` arrives in Phase 2 (`commerce/order.ts`).
