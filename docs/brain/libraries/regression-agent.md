...
4. **Review** (Max, read-only, web search) → ONE verdict:
   ...
   - **Unparseable/unrecognized verdict** ([[../specs/needs-attention-triage-and-verdict-robustness]] Phase 2) → the shared `resolveReviewVerdict` helper (shared with [[repair-agent]] + [[security-agent]]) RE-RUNS the review **once** with a JSON-only repair hint appended to the prompt ([[../specs/ada-director-spec-status-cards-fix-tooling-fa6848]] Phase 1), then fail-safes to an **actionable** `needs_attention` reason — `"regression review produced no parseable verdict after 2 attempts — re-run or review manually: <excerpt>"` — never a bare "ended without a recognizable verdict", never assume-resolved. The shared `extractJson` ranks LAST-balanced-JSON last-wins so prose-around-envelope no longer breaks parsing on attempt 1. The director's `reconcileNeedsAttention` ([[platform-director]] Phase 1) then triages that parked item.
