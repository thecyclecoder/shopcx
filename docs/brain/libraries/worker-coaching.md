# libraries/worker-coaching

The DevOps Director's **coaching brain** — the supervisory pass that detects a worker's repeated mistakes and acts on them ([[../specs/worker-coaching-loop]], Phase 1). Reads [[../tables/director_activity]], writes via [[worker-instructions]] `coachWorker`, posts to [[director-board]], routes code bugs to [[../specs/repair-agent|Repair]], and escalates to the CEO. Pairs with [[worker-instructions]] (the store) — this is the **detect → route → coach/route/escalate → re-check** logic on top.

**File:** `src/lib/agents/worker-coaching.ts` (server-only). **Runner:** `scripts/run-worker-coaching-pass.ts` (dry-run by default).

## Detection

- **`detectRepeatedErrors(admin, workspaceId, opts?): Promise<RepeatedErrorCandidate[]>`** — group recent [[../tables/director_activity]] by `(worker, disposition class)`. Resolves the worker kind via `metadata.worker_kind` → `metadata.job_id` ([[../tables/agent_jobs]]`.kind`) → the `action_kind`. Returns a candidate for every disposition a worker applied **≥ `REPEAT_ERROR_THRESHOLD`** times in the window, surfacing `recurredSignatures` — **the concrete wrongness signal we trust without the grading loop:** a signature the worker dismissed that **came back** (a correct disposition makes a problem stop). Grades from [[../specs/director-loop-grading]] become an additional input once it ships.
...
- **`runWorkerCoachingPass(admin, workspaceId, { apply?, coachAll?, directorFunction? }): Promise<CoachingPassResult>`** — **dry-run by default** (`apply:true` writes). For each **coachable** candidate (one with a recurred-signature signal, unless `coachAll`):
  ...
  - else **coach**: `coachWorker` (amend the instructions + log the message), post the board `update` ("🛠️ Ada coached 🔴 Remi: …"), link the post, write a `coached_worker` activity row.
...
[[worker-instructions]] · [[../tables/worker_instructions]] · [[../tables/worker_coaching_log]] · [[../tables/director_activity]] · [[director-board]] · [[repair-agent]] · [[../specs/worker-coaching-loop]] · [[../specs/platform-director-agent]] · [[../specs/director-loop-grading]] · [[../goals/devops-director]]
