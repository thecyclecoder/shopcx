...
⚠️ **…and the escalation has to actually reach someone (fixed 2026-08-11).** This branch is where the autonomous fix loop **admits defeat** — the one moment it is guaranteed to need a human. Until 2026-08-11 it wrote *only* the activity row above, with **no `dedupe_key`** and **no CEO card**. `reconcileSwallowedEscalations` ([[platform-director]]) — the backstop that re-emits missing notifications — filters the ledger to rows that *carry* a `dedupe_key`, so this escalation was invisible on **both** surfaces: it reached nobody. It stayed harmless only because nothing routed into this lane on an *undiagnosed* park.

The **PLATFORM-OWNER RUNG** ([[needs-attention-route]]) made this lane the destination for undiagnosed build parks, which made this branch the rung's terminal failure mode — so it now mints the card itself: `escalateDiagnosisToCeo` with `escalationKind='premerge_fix_loop_guard'` and `dedupeKey='premerge-fix-loopguard:{originSlug}'`, and the activity row carries the same `dedupe_key` so the reconciler can re-emit if the mint fails. Minting here (rather than leaning on the reconciler) also means the founder sees it on the FIRST pass. The kind is enrolled in the build-stuck family in [[approval-inbox]], so the card dedupes against sibling build-stuck cards and auto-clears when the spec folds or a build lands. A silent rail is the degenerate state of the north star, not an implementation detail.
...
A pre-merge **Vault** security review (a standalone solo `security-review` session — the fused spec-test+security session was retired 2026-07-17; see [[spec-check-runner]] § graduate-vera and [[security-agent]] § branch) that returns `real-vuln` on an **in-flight branch** routes through `spawnPreMergeFix` — the SAME fixes-as-phases path — instead of authoring a standalone security fix spec. Vault's findings are mapped to failing checks via `buildSecurityFailingChecks`, each with a STABLE `check_key` (`sec:<check>[:<location>]`) so the loop-guard + per-key dedup fire (no endless Fix N). The branch's security-review row stays NOT-green (`verdict='real-vuln'`), so `isSecurityGreenForBranch` holds the PR until the fix phase ships and a FRESH solo Vault re-review of the origin's branch clears it.
...
- **The re-test path.** The fix phase ships on the origin's own `claude/build-{slug}` branch, so its re-test is a pre-merge run (`isPreMerge = !!previewOrigin && !!branch`) — deterministic `runSpecChecks` re-verifies the spec's own checks and, on completion, a fresh standalone branch-mode Vault review re-verifies the security finding. Both must go green for the M4 promote gate to open.
