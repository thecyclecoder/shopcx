...
**Dual-write to `public.specs`** ([[../specs/spec-authoring-writes-db-and-worker-materialize]] Phase 3): every `upsertCardState` patch ALSO writes the corresponding typed column on the future-canonical [[../tables/specs]] row (status, deferred, priority, intended_status) via the inline `dualWriteSpecRow` helper. The mirror stays the read-path until [[../specs/spec-readers-from-db-retire-parser]] (M3) cuts readers over; the dual-write keeps the row honest with the mirror so callers reading either source see the same answer. Field mapping: `patch.status → specs.status`, `flags.deferred → specs.deferred` (the `specs_deferred_rollup_trigger` rolls status to `'deferred'` when set, restores phase rollup when unset), `flags.critical → specs.priority` (`'critical'`/null), `flags.intended_status → specs.intended_status` (cleared on Ada's disposition). A flag key with value `undefined` is treated as a CLEAR (writes null/false) — matching the writers' convention of consuming a flag by setting it `undefined`. A 0-row UPDATE (no specs row yet for this slug) is silently fine; the row catches up on the next author/edit pass via [[specs-table]] `upsertSpec`. Per-phase PR provenance dual-writes via [[agent-jobs]] `applyMergedBuildEffects` — the typed [[../tables/spec_phases]] row is the canonical surface for phase progress; `spec_card_state.phase_states[i].{pr,merge_sha}` mirrors it.
