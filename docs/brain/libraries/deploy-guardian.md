...
**Loop blast-radius filter (`isExcludedFromDeployRegressionLoop`).** The loop-side twin of the above — a NEW red `loop_alerts` row is dropped from `newRedLoops` when it's a `kpi_drift:<metric>:<cadence>` alert for an **audit-skipped** metric: the `liveSpecSetDependent` weekly-aggregate / live-spec-set meta-metrics (`specs_per_week`, `regression_coverage_pct`) and the `currentState` point-reads (`lane_utilization`, …). These reflect **PM volume / a moving-population membership delta** (how many specs shipped this week + their regression coverage), NOT the deployed code — a no-op spec's deploy cannot move them. The predicate is `[[platform-scorecard]].isAuditSkippedKpiDriftLoop`, which parses the loop_id and consults the **same `liveSpecSetDependent`/`currentState` registry flags** that [[kpi-review]] `auditAllKpis` skips ([[../specs/kpi-audit-skip-live-spec-set-dependent-metrics]] / #848) — **single source of truth**, so the audit skip and the deploy-attribution gate can't drift apart. A genuine windowed-aggregate `kpi_drift`, an error-rate loop, or a test/regression loop is NOT excluded and still trips `regressed`.
...
>
> The loop filter + diff-plausibility gate closed the **`noop-pipeline-test-6`** false-revert: Reva auto-reverted a NO-OP spec (one unused exported const + one unused nullable column — incapable of any runtime regression) because two **weekly-aggregate** `kpi_drift` loops (`specs_per_week`, `regression_coverage_pct`) flipped red in its canary window from a high-volume PM night. Those loops reflect PM volume, not the deploy. A deploy may now be auto-rolled-back ONLY for a regression in a metric that genuinely reflects the deployed code (errors, latency, a real test/regression loop) — never a PM-volume aggregate.
