...
**In-flight-only filter (vault-security-review-loop-fix).** The backstop SKIPS any branch whose spec is **shipped/folded** (DB `specs.status` via `getSpec`) or **archived** (slug in `listArchivedSlugs`) — checked BEFORE both the READY-scan and the no-preview recovery. Pre-merge gating is an in-flight-only concern: once a spec has merged or folded its `claude/build-*` branch may still carry a stale READY preview, but re-running spec-test/security on it every pass is pure waste. This was half of the **Vault re-review loop** — a folded spec's stale READY branch got a fresh `security-review` enqueued each standing pass forever (observed: `spec-test-request-fix-inline-author-and-approve`, `in-testing-board-and-lifecycle-timeline` re-reviewed every ~5 min, always clean). The other half is fixed in [[security-agent]] `enqueueSecurityReviewJob` branch mode (a CLEAN review now also blocks re-enqueue until the branch advances). Net: a shipped/folded spec gets NO new security-review; an in-flight branch gets exactly ONE per branch state.
