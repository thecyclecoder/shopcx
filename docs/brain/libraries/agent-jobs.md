...
### `maybeEnqueuePreMergeSecurityOnAccumulation` — function  *(fused into the pre-merge spec-test session — consolidate-premerge-checks-one-session Phase 1)*
...
**Retired as a standalone pre-merge enqueue — now inert.** Originally the pre-merge SECURITY TRIGGER: the security twin of `maybeEnqueuePreMergeSpecTestOnAccumulation`, calling [[security-agent]] `enqueueSecurityReviewJob` in `branch` mode from the preview-ready hook + `backstopPreMergeChecks`. It ran a SEPARATE branch-mode Max session per branch (paying a full cold context re-hydration on top of the spec-test session that just did the same thing).

Under **`consolidate-premerge-checks-one-session` Phase 1** the pre-merge security review is FUSED into the pre-merge spec-test session (`runSpecTestJob` in `scripts/builder-worker.ts`): the same session reads the branch diff + preview ONCE and emits BOTH verdicts in one JSON envelope (`{...spec_test, security: {...}}`). The worker writes the spec-test verdict to `spec_test_runs` FIRST (partial-safety — a crash after the spec-test half still records it), then INSERTS a synthetic `security-review` agent_jobs row for the branch (status='claimed', mode='branch', preview_origin) and applies the security verdict via the shared `applySecurityVerdictToJob` helper (the SAME sinks the standalone lane uses: agent_jobs terminal update + `director_activity` + authored fix-spec + routed build queue via [[approval-router]]).

This function is retained for its callers (the preview-ready hook + `backstopPreMergeChecks`) but now always short-circuits with `enqueued: false, reason: "fused into pre-merge spec-test session"`. The standalone [[security-agent]] `enqueueSecurityReviewJob` remains available for **post-merge `diff` mode** (per-merged-diff review) and as a **safety-net fallback** the fused session invokes when its own security envelope is missing/malformed. The `isSecurityGreenForBranch` signal still reads the SAME agent_jobs rows — the synthetic row the fused session inserts satisfies it.
...
**consolidate-premerge-checks-one-session Phase 1 — fused security.** The security half of the backstop is now a no-op (`maybeEnqueuePreMergeSecurityOnAccumulation` short-circuits). Its work moved INTO the pre-merge spec-test session (`runSpecTestJob`), which emits BOTH verdicts off the same loaded diff and inserts a synthetic security-review row for the branch. The backstop still calls the security trigger for symmetry, but the actual security signal is produced by the fused spec-test lane — so the backstop's spec-test re-fire IS the security re-fire.
