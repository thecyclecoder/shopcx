...
### `maybeEnqueuePreMergeSecurityOnAccumulation` — function  *(deprecated for branch mode as of fused-premerge-security-authoritative-drop-standalone Phase 2)*
...
**As of [[../specs/fused-premerge-security-authoritative-drop-standalone]] Phase 2, the fused spec-test session is AUTHORITATIVE for pre-merge branch mode.** The `runSpecTestJob` box loop emits a structured, evidence-backed security envelope alongside the spec-test verdict in ONE Max session; `applyFusedSecurityAsBranchVerdict` ([[../../../scripts/builder-worker]]) INSERTS a `security-review` `agent_jobs` row for the branch (pre-claimed with status='claimed') and applies the terminal verdict via `applySecurityVerdictToJob` — that row is INDISTINGUISHABLE to `isSecurityGreenForBranch` from a standalone-produced row. The Phase-1 pure validator ([[../src/lib/security-envelope]] `classifyFusedSecurityEnvelope`) downgrades a bare/self-declared `clean` to `needs_human`, so a rubber-stamp CANNOT satisfy the M4 promote gate.

**Historical context.** Under **`consolidate-premerge-checks-one-session` Phase 1** the pre-merge security review was FUSED into `runSpecTestJob` with a bare `security.status="clean"` envelope, which a rubber-stamp could satisfy. Under **`isolate-premerge-security-verdict` Phase 1** the fusion was REVERSED: this function was restored as the standalone enqueue and `runSpecTestJob` discarded its security envelope. Phase 2 of `fused-premerge-security-authoritative-drop-standalone` restores fusion with the Phase-1 evidence contract enforced — the fused envelope must carry structured per-check entries `{check, verdict, evidence}` (all 5 required checks, `clean` with evidence, `finding` with file:line + severity) or the validator downgrades it.

Phase 3 (planned) will retire this standalone enqueue (fused row is the sole pre-merge producer). Until then this function still runs and its output is what `getSecurityStateForBranch` also reads — the fused row is added on top and both are reconciled by the same `RUNNING`/`SURFACED`/`completedClean` rollup. Best-effort + never throws.
...
**fused-premerge-security-authoritative-drop-standalone Phase 2 — fused session is authoritative.** The backstop's spec-test re-fire remains active + IT is what produces the authoritative pre-merge security verdict (via the fused envelope + `applyFusedSecurityAsBranchVerdict`). The standalone `maybeEnqueuePreMergeSecurityOnAccumulation` re-fire still runs (Phase 3 will retire it) but its verdict is redundant with the fused row — both feed `getSecurityStateForBranch` and are reconciled by the same rollup.
