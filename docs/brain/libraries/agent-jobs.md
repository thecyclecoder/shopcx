...
The **M3 pre-merge spec-test TRIGGER**. Under the branch-accumulation model a spec's phases build one-by-one onto ONE persistent `claude/build-{slug}` branch (each push fires a per-build Vercel preview — [[preview-capture]]). The spec-test must run ONCE against the WHOLE built spec, not per phase. So the trigger fires iff the spec is **fully accumulated on its branch** ([[specs-table]] `isSpecAccumulationComplete` — every phase carries a `build_sha` / is terminal) AND **a preview URL exists**. Idempotent — the underlying `enqueuePreMergeSpecTest` dedupes per `(workspace, slug, branch)`. Two callers drive it now:
1. **⚡ `enqueuePreMergeFromDeploymentReady`** (below) — the EVENT-DRIVEN path: the GitHub `deployment_status` webhook fires this the moment a preview goes READY (preview-ready-event-trigger). This REPLACED the retired in-worker `pollCapturePreviewUrl` READY callback (which used to fire this from `runBuildJob` + `finalizeBuiltPhase` — both removed 2026-07-17).
2. **`backstopPreMergeChecks`** — the standing-pass heartbeat (below), the safety net that recovers a dropped webhook / a preview that was never captured.
...
```ts
async function enqueuePreMergeFromDeploymentReady(args: {
  sha: string | null; previewUrl: string | null; environment?: string | null;
}): Promise<{ enqueued: boolean; reason?: string; branch?: string; slug?: string }>
```

The **event-driven pre-merge trigger**, called from [[../integrations/github-webhook]] **Gate D** on a `deployment_status` success. GitHub identifies a Vercel preview deploy by commit SHA (not branch), so this maps SHA → its `claude/build-*` branch ([[github-pr-resolve]] `resolveBuildBranchForSha` via GitHub `branches-where-head`), finds the branch's build job, persists `preview_url`+`preview_state='READY'` (advance-only), and calls `maybeEnqueuePreMergeSpecTestOnAccumulation` — firing the fused Vera/Vault session within SECONDS of READY instead of waiting for the box's next standing pass. Skips (with a reason) on a non-preview env, a missing SHA/URL, a SHA that no longer heads a build branch (superseded/merged deploy), or no build job for the branch. Cheap guards (env/sha/url) return before any network. Best-effort + never throws. Pure-guard tests: `agent-jobs.premerge-deployment-ready.test.ts`.
...
### `promoteEligibleSpecsToGoalBranch` — function  *(spec-goal-branch-pm-flow M4 — the spec→goal merge · parallel-build-serialized-merge-and-deadlock-autobreak Phase 3)*

```ts
interface GoalBranchPromoteResult { 
  promoted: string[]; 
  conflicts: string[]; 
  escalated: string[]; 
...
The **spec→goal-branch promotion poll** — the M4 integration. For every GOAL-BOUND spec that is `isSpecPromoteEligible` (M3 seam — accumulation ∧ spec-test-green ∧ security-green on its `claude/build-{slug}` branch) and not yet on its goal branch (`goal_branch_sha` unset), it merges that branch into **`goal/{goal-slug}`** (created from `origin/main` by the FIRST spec of the goal — that spec SEEDS it) and stamps `specs.goal_branch_sha` with the merge commit (via [[specs-table]] `stampSpecGoalBranchSha` — the M5 seam). Uses the GitHub `/merges` API ([[github-pr-resolve]] `mergeSpecBranchIntoGoalBranch`) — **no local checkout** — so it runs identically from the box worker standing pass AND the [[../integrations/github]] webhook (mirroring `autoMergeReadyPrs`). **Merges are sequenced by `blocked_by`** (`sequencePromoteCandidates`, a Kahn topo-sort over goal-mate edges) so a dependency lands on the goal branch before its dependent — which then BUILDS off the goal branch (`runBuildJob` Part 2), seeing the dependency's code. **CRITICAL: `blocked_by` entries MUST be plain kebab slugs** (e.g. `sol-ticket-direction-artifact`), never namespaced (e.g. `goalSlug:specSlug`). The `sequencePromoteCandidates` topo-sort looks each entry up by exact slug match and does NOT split on `:` — a namespaced entry resolves to no spec and lets the dependent build out-of-order ([[goal-member-blocked-by]] Phase 2 validates/repairs drift; [[goal-proposals]] Phase 1 normalizes Pia's decomposition write-path). ONE merge per spec (idempotent — already-stamped specs skip; the `/merges` 204 path is idempotent). **Does NOT push the goal branch to main** (that's M5's atomic goal→main promotion).

**Phase 3 — Serialized rebase-merge guard ([[../specs/parallel-build-serialized-merge-and-deadlock-autobreak]] Phase 3).** `mergeSpecBranchIntoGoalBranch` now wraps its merge in a per-goal **serialization mutex** so two concurrent invocations (standing pass + webhook) queue rather than race the GitHub merge endpoint. BEFORE the merge, it COMPARES goal-branch vs spec-branch via [[github-pr-resolve]] `compareGoalBranchToSpecBranch` and invokes [[github-pr-resolve]] `decideGoalBranchRebaseMerge`. If the branches are `identical` or `behind` (spec branch fully integrated already) → `skip` (no-op). If `ahead` (direct fast-forward) → `merge` directly. If `diverged` (a sibling has advanced the goal branch beneath us) → `rebase-then-merge`: merge goal branch INTO spec branch first (bring sibling advances in), then re-compare and loop. Rebase runs up to `GOAL_BRANCH_REBASE_MAX_ATTEMPTS=2` times; on rebase conflict (two goal-mates genuinely touched the same lines) or attempt exhaustion → `escalate` (never force-merge — this is the safety gate that lets parallel builds coexist). On escalate, `promoteEligibleSpecsToGoalBranch` records ONE `director_activity` row (`actor='serialized-rebase-merge-guard'`, `action_kind='goal_branch_merge_escalated'` — see [[director-activity]]), pushes to `escalated[]`, and skips the spec (no stamp). The standing pr-resolve flow or a human resolves the overlap. Pre-existing `conflict:true` outcome (direct merge 409, less common now) is preserved unchanged. **Conflicts are surfaced** (`conflicts[]`), **escalations are surfaced** (`escalated[]`), never silently dropped. Best-effort per spec; never throws.
...
### `decideGoalMemberDeadlockAutoBreak` / `autoBreakGoalMemberDeadlockIfDue` — functions  *(parallel-build-serialized-merge-and-deadlock-autobreak Phase 1)*

```ts
export const GOAL_MEMBER_DEADLOCK_AUTOBREAK_COOLDOWN_MS: number;
function decideGoalMemberDeadlockAutoBreak(input: {
  members: GoalMemberDispatchState[];
  inflight: GoalMemberInflightRow[];
}): GoalMemberDeadlockAutoBreakDecision
async function autoBreakGoalMemberDeadlockIfDue(workspaceId: string, ejectedSlug: string): Promise<GoalMemberDeadlockAutoBreakResult>
```
...
The **atomic goal→main promotion poll** — M5, one hop past M4. For every GREENLIT (non-`proposed`/non-`folded`) goal in the build-console workspace it gates in order: **(1) parent-goal exemption** — skip a parent via [[goals-table]] `isGoalParentExempt` (`is_parent` flag OR has child goals OR no buildable specs — a parent has no goal branch; its children promote independently); **(2) goal-complete** — require [[specs-table]] `goalBranchState(goalSlug).allOnGoalBranch` (every member spec integrated on the goal branch — M4's seam); **(3) GREEN** (option b, combination-verified without extra preview deploys) — require EVERY member spec individually promote-eligible: an **on-branch member** (`memberAccumulationSatisfiedOnGoalBranch(member)` = `member.onGoalBranch === true`, i.e. `goal_branch_sha` set) takes the **Phase-1 fast-path** — SKIP the stale per-member `verifyPhaseAccumulatedOnBranch` accumulation re-check (already gated at spec→goal merge by `promoteEligibleSpecsToGoalBranch`) and gate only on the cheap DB-read green legs [[spec-test-runs]] `isSpecTestGreenForBranch` + [[security-agent]] `isSecurityGreenForBranch`; a member NOT on the goal branch keeps the full `isSpecPromoteEligible` gate (accumulation ∧ spec-test-green ∧ security-green). The atomic merge itself is the final combination check (each dependent built OFF the goal branch, so the integrated whole was compiled together; a clean land confirms no `main` drift). **Why the fast-path:** re-running `verifyPhaseAccumulatedOnBranch` against a member's now-stale `claude/build-{slug}` at M5 catches over-precise / drifted grep patterns as FALSE-NEGATIVES even when the goal-branch artifact is deploy-clean — live: v3-ad-creative-engine held on 8/10 members' stale-grep false-negatives despite the goal branch passing tsc + every predeploy gate and a 0-conflict merge; a human hand-merged and that manual path skipped `finalizePromotedGoal`, stranding all 10 member folds. Phantom-protection stays at spec→goal merge (where a member FIRST accumulates); the goal-branch ARTIFACT is what actually ships. **(3b) POSITIVE-VERIFICATION CI gate on the artifact** *(Phase 2, added because removing the per-member re-check must not lower the bar)* — require the `goal/{slug}` branch HEAD to be CI-green via [[github-pr-resolve]] `goalBranchCiGreen(goalSlug)` (mirrors the check-runs shape the existing lanes read: resolve `refs/heads/goal/{slug}` → GET `/commits/{headSha}/check-runs` → pass through `classifyCheckRunsForCiGreen`, the pure predicate). Fails CLOSED — a read error, an empty check-run list, an in-flight run, or ANY red conclusion HOLDS the promote via `notReady[]` (surfaced with the reason, never a silent skip). This is the artifact-side positive verification that replaces the per-member stale greps. **(4) promote** — [[github-pr-resolve]] `mergeGoalBranchIntoMain(goalSlug)` merges `goal/{slug}` → main in ONE merge, then the SINGLE non-skippable post-merge entrypoint `finalizeGoalMainPromotion({workspaceId, goalId, goalSlug, mergeSha, adminClient})` runs the FIVE atomic effects in one call (`stampGoalPromotedToMain` → Reva atomic `openDeployWatch` → `applyGoalPromotionEffects` stamps shipped + reactive-folds → `autoQueueUnblockedByGoal` outside-dependent release → `finalizePromotedGoal` retires the goal), so NO caller (this seam or a future ad-hoc/manual promoter) can land the merge without finalizing — the exact miss that stranded v3-ad-creative-engine's 10 member folds when a human hand-merged. A **409 conflict** HOLDS the goal (`conflicts[]`, nothing stamped). Runs from the SAME seams M4 uses (box worker standing pass + the Gate-C github webhook); GitHub `/merges` API (no checkout). Idempotent (a goal already on main merges as a 204 + re-stamps inertly), best-effort per goal, never throws. **One-off (no-goal) specs do NOT promote here** — they ship via the Gate A auto-merge of their `claude/build-{slug}` branch (see `applyMergedBuildEffects` whole-spec stamping below).

### `memberAccumulationSatisfiedOnGoalBranch` — function  *(goal-main-promote-gates-on-artifact-not-stale-per-member-rechecks Phase 1 — the on-branch fast-path predicate)*

```ts
function memberAccumulationSatisfiedOnGoalBranch(member: { onGoalBranch: boolean }): boolean
```

**Pure predicate** the M5 member-eligibility loop reads to decide whether a member takes the on-branch **fast-path** at goal→main (SKIP the stale per-member accumulation re-check). Returns `member.onGoalBranch === true`. A member with `onGoalBranch=true` (its `specs.goal_branch_sha` is set) already passed [[spec-check-runner]] `verifyPhaseAccumulatedOnBranch` at spec→goal merge time — M4's `promoteEligibleSpecsToGoalBranch` gates on the SAME `isSpecAccumulationComplete` predicate. Re-running that grep against the now-stale `claude/build-{slug}` branch at M5 catches over-precise / drifted patterns as FALSE-NEGATIVES (v3-ad-creative-engine incident). The fast-path only skips the accumulation LEG; spec-test-green + security-green legs stay (cheap DB reads on the branch preview). `isSpecPromoteEligible` is UNCHANGED — it keeps its accumulation gate for the spec→goal promote (where a not-yet-merged member MUST be verified); only the goal→main member loop takes this fast-path. Testable without DB or GitHub.
...
**Retire a goal that just promoted to main** — the gap-closer so a promoted goal doesn't linger `greenlit`. Two sanctioned steps: **(1)** [[goals-table]] `setGoalStatus(goalId, 'complete')` — the explicit `greenlit → complete` lifecycle override (the board *derives* `complete`, but the stored column stays `greenlit` after M5, and the goal-fold lane's guard reads the *stored* status); skipped if already `complete`/`folded`. **(2)** enqueue ONE `kind='goal-fold'` `agent_jobs` row (deduped on an in-flight goal-fold for the slug; skipped if the goal is already `folded`) — the goal-fold lane folds the goal's durable knowledge into the permanent brain pages and flips `goals.status='folded'`. Now called through `finalizeGoalMainPromotion` (below), which chains the four upstream effects so no caller can land the merge without running finalize. `agent_jobs` is not a PM table, so the goal-fold enqueue is a plain insert; the goal-status write is the only PM write, via the SDK. Best-effort + idempotent; never throws.

### `finalizeGoalMainPromotion` — function  *(goal-main-promote-gates-on-artifact-not-stale-per-member-rechecks Phase 3 — the single non-skippable post-merge entrypoint)*

```ts
interface FinalizeGoalMainPromotionResult { effects: GoalPromotionEffects; outsideQueued: string[]; finalize: GoalFinalizeResult; }
async function finalizeGoalMainPromotion(input: { workspaceId; goalId; goalSlug; mergeSha; adminClient? }): Promise<FinalizeGoalMainPromotionResult>
```

**The one entrypoint every caller runs the moment `mergeGoalBranchIntoMain` returns a real `mergeSha`.** Folds the four sanctioned post-merge effects + the finalize into ONE call in this exact order: **(1)** [[goals-table]] `stampGoalPromotedToMain(goalId, mergeSha, actor)` — persist the atomic merge SHA on the goal row FIRST so any downstream reader (spec-fold gate, control-tower heartbeat) sees the promoted state and the `main_merge_sha`-set predicate clears the HELD backstop; **(2)** [[deploy-guardian]] `openDeployWatch({branch: goal/{slug}, isAtomic: true, ...})` — snapshot Reva's pre-deploy baseline close to the merge (the highest-blast-radius deploy in the system, previously UNWATCHED by Gate A); **(3)** `applyGoalPromotionEffects(workspace, goalSlug, mergeSha)` — the ONLY shipped-writer (flip every member spec's phases → `shipped` tagged with mergeSha, stamp card-level `last_merge_sha`, enqueue spec-tests, invoke reactive fold); **(4)** `autoQueueUnblockedByGoal(workspace, goalSlug)` — release OUTSIDE dependents whose blocker is `kind:"goal"` (the per-member `autoQueueUnblockedBy` calls inside `applyGoalPromotionEffects` cover only `kind:"spec"` goal-mates); **(5)** `finalizePromotedGoal(workspace, goalSlug, admin)` — the NON-SKIPPABLE tail (stored greenlit → complete + goal-fold enqueue). **Why this matters:** before Phase 3 the sequence was INLINE in `promoteCompleteGoalsToMain`, so a hand-merged / ad-hoc promoter that landed the merge outside this seam would skip the tail and strand every member fold at the fold gate's Rail-3 goal-bound-defer — the live v3-ad-creative-engine incident where all 10 member folds sat stranded until Dylan hand-backfilled. Best-effort per step (never throws): each step's failure is logged and the sequence continues so a transient step-1 failure doesn't skip the effects, and a transient step-3 failure doesn't skip the finalize. Idempotent: `stampGoalPromotedToMain` compare-and-sets on id, `finalizePromotedGoal` no-ops an already-folded goal + dedupes an in-flight goal-fold. `[[reconcileCompletedGoalsToFolded]]` is the belt-and-suspenders reconciler that catches any goal still stranded despite this entrypoint (`main_merge_sha` set, stored status ∉ {complete, folded}).
...
**Self-archive a COMPLETE non-parent goal** — the FORWARD fix so a 100% goal never sits stranded on the active board awaiting a manual backfill. `finalizePromotedGoal` (above) retires ONLY a goal that shipped THROUGH a goal branch; a **legacy goal whose member specs shipped one-off** (no `goal/{slug}` branch → the M5 promoter never evaluated it) reaches a 100% rollup but never gets the greenlit→complete + goal-fold enqueue — it lingers FOREVER as `greenlit`/`complete` (the 8 goals Dylan hand-folded). This standing reconciler re-evaluates the FULL active goal set each pass in TWO passes: **Pass 1 (derived-rollup)** — via [[brain-roadmap]] `getGoals` (folded goals are off that read = never a candidate), for each goal: **(1a) ROLLUP 100%** — require the DERIVED card `status === 'complete'` (every milestone rolls up complete = every member spec shipped|folded) AND `linkedSpecCount >= 1` (never fold an empty 0-spec goal); **(1b) PARENT EXEMPTION** — skip via [[goals-table]] `isGoalParentExempt` (`is_parent` flag **OR** has child goals — `goalHasChildGoals` counts EVERY child row incl. already-folded children, the structural signal that exempts `ceo-mode` — **OR** no buildable specs); a parent stays active at 100% awaiting its sub-goals and NEVER auto-folds; **(1c) FOLD** — reuse `finalizePromotedGoal` (greenlit/complete → complete + goal-fold enqueue). **Pass 2 (stranded-shape belt-and-suspenders — Phase 3)** — via [[goals-table]] `listGoals` (raw rows, so the STORED status is authoritative — NOT the derived card status), for each raw row NOT already processed by pass 1: catch any goal where `main_merge_sha` IS SET but STORED `status` ∈ {`proposed`,`greenlit`} — the pre-Phase-3 stranded shape a hand-merge / ad-hoc script leaves behind when it lands the goal branch on main without running finalize; parent-exempt goals with a stranded `main_merge_sha` are recorded as `kept:"stranded-parent-exempt"` (never re-retire a family); the rest run `finalizePromotedGoal` and record `reconciled_completed_goal_folded` with `actor:"reconciler:stranded-promoted-goal"`. This is the belt-and-suspenders for any goal already stranded BEFORE Phase 3's `finalizeGoalMainPromotion` chokepoint went live. Wired into [[../lifecycles/roadmap-build-console]]'s `runPlatformDirectorStandingPass` right after the M5 goal→main promote block. Idempotent per-slug (`processed` set dedupes across the two passes; `finalizePromotedGoal` no-ops an already-folded goal + dedupes an in-flight goal-fold), bounded (the active set), and LOGGED — one `reconciled_completed_goal_folded` [[../tables/director_activity]] row per goal folded (never silent, with the `actor:` field distinguishing the two passes). Best-effort per goal; never throws.
...
### `reconcileMergedSpecPhases` — function  *(ship-all-phases-on-squash-merge — standing-pass recovery · [[../specs/merge-gate-verifies-real-phase-checks-not-status-flags]] P2)*

```ts
async function reconcileMergedSpecPhases(admin?, verifyOnBranchOverride?): Promise<{ reconciled: string[]; phasesStamped: number }>
```

The **re-runnable recovery** for the post-merge-ships-only-one-phase bug. `applyMergedBuildEffects` now stamps EVERY phase of a fully-accumulated squash-merge — but it only runs on the **completed→merged transition**, so a spec that ALREADY merged under the old (one-phase-only) hook is STUCK with un-shipped phases (`noop-pipeline-test-4` / #837: P1 `in_progress`, `merge_sha=NULL`, while P2 shipped). This standing-pass function recovers them: it enumerates `merged` `kind='build'` jobs (one per slug), and for any whose spec still has a NON-terminal phase, recovers the merge SHA from an already-shipped sibling phase (the squash-merge commit), then **VERIFIES each un-shipped phase's real grep checks against that merged code** via [[specs-table]] `verifyPhaseAccumulatedOnBranch(workspaceId, slug, position, mergeSha)`. Only phases whose verifier reports `accumulated:true` are stamped `shipped` with the sibling's `merge_sha` + `pr`; a phase whose verifier reports NOT accumulated is **LEFT un-shipped** (Phase 3's phantom-ship detector surfaces it) — never blanket-stamped. This closed one of the three exploit triggers the P2 spec cites (v3 factor-rollup-sdk-with-significance-gate phantom-shipped P2/P3 by blanket-stamp with no code check). If NO phase is shipped yet (no SHA to copy) it LEAVES the spec for the audit path (`audit-spec-shipped-state`). After stamping any verified phases it fires the post-ship hooks (`enqueueSpecTestIfDue('shipped')` + `autoQueueUnblockedBy`); if EVERY un-shipped phase failed verification the spec is skipped entirely (no partial recovery, hooks skipped — the whole spec is a phantom). **Strictly idempotent** — a fully-shipped spec is skipped, and `stampPhaseShipped` on an already-shipped phase is inert. **The pure partition** `partitionPhasesByVerifyVerdict(unshipped, verdicts)` is the guard seam (exported for unit-testing the reconciler without mocking Supabase). `verifyOnBranchOverride` is a `VerifyPhaseDeps` DI point so tests can drive the guard with fixture verdicts. Called from the [[../../../scripts/builder-worker]] platform-director **standing pass** (right after the Gate-A auto-merge backstop). Best-effort per spec; never throws.

**⚠️ FAIL-CLOSED (merge-gate-verifies-real-phase-checks Phase 2, 2026-07-22):** the old blanket back-fill — copy a shipped sibling's `merge_sha` onto EVERY unshipped phase — assumed the squash-merge was *fully accumulated*, which stamped phases shipped whose code was never in the merge (**the phantom-ship**: v3 `factor-rollup-sdk-with-significance-gate` P2/P3 marked shipped with `getFactorRollup` never written). Now the function takes `deps.verifyPhaseAccumulated` ([[spec-check-runner]] `verifyPhaseAccumulatedOnBranch` via a box-side worktree at the merge SHA) and **only back-fills when the merged code passes the spec's machine checks**. No verifier, or a failing verdict ⇒ the back-fill is REFUSED (surfaced in `result.skipped`), never a blanket phantom-stamp. The builder-worker caller injects the verifier; a caller without one fails closed.
...
### `retestOriginBranchSecurityIfFixMerged` — function  *(security-real-vuln-deadlock-breaker Phase 3)*

```ts
async function retestOriginBranchSecurityIfFixMerged(workspaceId, fixSlug, adminClient?): Promise<string | null>
```

The **security sibling** of `retestOriginIfFixMerged` above, and the piece that makes a `real-vuln` finding survivable rather than terminal.

**The deadlock it breaks (observed live 2026-08-11).** A pre-merge branch security review returning `real-vuln` writes that verdict onto the branch's `security-review` row; [[security-agent]] `isRealVulnVerdict` makes the row NOT `completedClean` — deliberately, so the M4 promote gate can never merge a known-vulnerable branch while its fix is un-shipped. The finding then auto-authors a scoped fix spec on its **own** `claude/build-{fix}` branch. But nothing ever re-reviewed the **origin** branch: `enqueueSecurityReviewBranch`'s dedup (2) only re-reviews once a **newer build push lands on that same branch**, and the fix never touches it. So the origin's verdict was **permanent** — `securityGreen` false forever, PR open forever, no card, no error, no retry. PRs #2427 (24h) and #2438 (17h) were both accumulation-complete **and** spec-test green, blocked solely on this.

**What it does.** On a merged build whose spec carries `regression_of_slug` (stamped by `authorSecurityFixSpec` — see [[../../../scripts/builder-worker]]), resolve the origin's latest branch-owning build row; if that branch is still unmerged, enqueue a **forced** branch-mode security review of it (`force: true`). The fix is now on `main`, so the origin branch's diff *vs main* genuinely changed even though its tip did not — precisely the case dedup (2) cannot see. Vault re-reads the diff and writes a NEW authoritative verdict: clean ⇒ `completedClean` ⇒ the promote gate opens on the next pass; still `real-vuln` ⇒ correctly still blocked.
...
```ts
const PROMOTE_STALL_ESCALATE_AFTER_MS = 6 * 60 * 60 * 1000; // 6h
async function escalateStalledPromoteEligibility(adminClient?): Promise<{ scanned, escalated }>
```

The **"waiting on an input that will never arrive"** detector. Every promote signal fails CLOSED (accumulation, spec-test-green, security-green) — correct individually, but the composition had no timeout and no voice: a PR whose gate could never open just sat, indefinitely, behind a `console.warn` nobody reads. The founder's experience was *"I submit specs and they aren't built a day later"*; the system's internal state was *"everything is fine."*

Scans the latest branch-owning build per spec that opened a PR, is unmerged, and is older than `PROMOTE_STALL_ESCALATE_AFTER_MS`; for each not-`eligible` spec raises **one** [[../tables/dashboard_notifications|dashboard_notifications]] card naming the exact failing leg (the `reason` from `isSpecPromoteEligible`, plus a per-leg ✅/❌ breakdown). **Detector only** — never merges, never flips a gate, never marks anything green. Deduped per spec on an undismissed card (`metadata.dedupe_key = promote-stall:{slug}`), so a week-long stall yields one card, not 168.

**Node completeness:** adds **no new node**. It runs inside the existing `platform-director` standing pass (same call site as `backstopPreMergeChecks`), inheriting that node's owner, kill-switch ancestry, and heartbeat — per [[../operational-rules]] § Node completeness.
...
## Suspect-check diagnosis on the deferred-PR escalation

[[../specs/a-verification-check-must-not-demand-a-name-the-builder-has-to-guess]] Phase 2 — a build that DEFERRED its PR because `isSpecAccumulationComplete` reported `complete:false` is escalated by [[roadmap-actions]] `redriveDeferredBuildOrEscalate` after `BUILDER_DEFERRED_REDRIVE_MAX` in-24h redrives. Before Phase 2 the escalation always read as "phase-accumulation checks failed" — the wrong ask when the real defect is a phase-check that pins a builder-chosen literal (`test:graduate-crowned` while the branch registered `test:media-buyer-graduate-scaler`). Now the escalate branch walks the spec's prior `director_activity` `redrive_deferred_build` rows, extracts each row's `metadata.failing_check_keys`, and runs [[../libraries/build-suspect-check]] `detectSuspectCheck`. When the SAME lone check has been the ONLY failing check across ≥3 consecutive builds, the escalation summary is rewritten to `Spec {slug} — check X of phase Y has failed N builds while every other check passed — the check is the likely defect`, includes the branch's near-miss evidence, and appends a `loosen_check` `pending_actions` entry carrying the corrected pattern (computed via the same `detectBuilderChosenNameInGrep` predicate Phase 1 uses at author time) alongside the existing `reclaim_stuck_build` approval. Multi-check current-run failures and streak-breakers stay as-is — the detector fails CLOSED on ambiguity, so a genuine build defect is never mislabeled.
