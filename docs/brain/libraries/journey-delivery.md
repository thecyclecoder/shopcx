...
- **Lead-in shares the ticket-delivery paragraph shaper.** Every HTML branch (email/help-center ticket body, chat CTA, portal CTA, nudge) renders the AI lead-in through `toHtml` re-exported from [[ticket-delivery]], not a raw `` `<p>${leadIn}</p>` `` interpolation — otherwise the blank lines the author wrote get collapsed and the customer sees a wall of run-on prose under the button. The CTA anchor stays its own trailing `<p>...</p>`. The two plain-text branches (SMS / Meta DM) pass the lead-in through untouched. One shaper, one paragraph rule (CLAUDE.md: at most two sentences per paragraph), so the reply path and the journey path can never drift again. Ground truth: ticket `a4e79e9d` (2026-09-15) shipped a five-paragraph lead-in as one paragraph.
- **Cancel journey re-request guard — `directToCancelTerminal` on `LaunchParams`.** For any cancel-intent launch (`triggerIntent ∈ {'cancel_subscription', 'cancel', 'cancellation'}`), `launchJourneyForTicketInner` auto-invokes [[cancel-journey-guard]] `hasRecentSavedRemedy(admin, workspaceId, ticketId)`; when true, it stamps `config_snapshot.directToCancelTerminal = true` on the fresh `journey_sessions` row and drops an internal `[System]` note. Callers can also pass `directToCancelTerminal: true` explicitly (e.g. [[no-progress-guard]] `attemptCancelJourneyResend`). The mini-site ([[../journeys/cancel]]) reads that flag and skips subscription→reason→remedies straight to `confirm_cancel`. Cancellation still requires the customer's own confirm click — the flag skips OFFERS, not the action. Prevents the ticket-`6c12a925` trap where a `saved_remedy` re-launch re-presented the same offer the customer had already rejected in words.
