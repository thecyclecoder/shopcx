...
- `TicketDirectionPath = "playbook" | "journey" | "workflow" | "stateless" | "needs_info"` — the five treatment paths Sol can commit the ticket to on first touch. `playbook` drives an existing playbook; `journey` launches a matched [[../tables/journey_definitions|journey]] (cancellation is the flagship — `cancel-subscription`, the save flow); `workflow` runs a matched [[../tables/workflows|workflow]] (guided smart flow, identified by `trigger_tag`); `stateless` is a single stateless reply; `needs_info` asks the customer for a specific missing piece before any action. Sol picks from the **MECHANISMS CATALOG** block [[../../scripts/builder-worker]] `loadMechanismsCatalog` injects into her brief (all active journeys/workflows/playbooks for the workspace+channel). **At first touch the box delivers the mechanism itself** ([[../../scripts/builder-worker]] `runTicketHandleJob`): `journey` → [[tickets-mutate]] `launchJourney(leadIn=first_reply)` (one message: lead-in + CTA), `workflow` → `runWorkflow`, then the live Direction is **superseded** so the on-reply [[sol-direction-apply]] path can't re-launch it. `playbook` is armed reply-gated ([[sol-mechanism-arm]]). **After a successful `journey` launch (chosen_path OR `plan.launch_journey_slug`) the box also CLOSES the ticket** (founder directive 2026-07-10, `closeTicketOnResolvingReply`) — the delivered lead-in + CTA IS the whole first-touch treatment; a customer reply reopens the ticket and the journey token carries its state, and journey COMPLETION fires the real action independently. Before this a delivered journey left the ticket `open` forever (`mechanismManagesStatus` skipped the every-message-closes rule for journeys — nothing in a CTA-driven journey's lifecycle closed the originating ticket; fc54b9fc: cancel-subscription delivered, never closed). **Workflows are excluded** — `runWorkflow` self-manages status and may drive multiple steps before it's done.
...
### `plan.link_proposal` — account linking for unlinked duplicates

Any `chosen_path` can also set `plan.link_proposal` to propose linking a high-confidence unlinked duplicate account before the mechanism dispatch fires — so a downstream refund/cancel/journey remedy targets the whole linked person instead of dead-ending on the empty half. Phase 2 of [[../specs/account-linking-address-aware-confidence-graded-and-cs-searchable]].

Sol / June authors `link_proposal: { candidate_customer_id, confidence, previously_rejected?, reconfirmed? }` when they see a HIGH-confidence unlinked sibling surfaced by [[account-matching]] `hasHighConfidenceUnlinkedMatch` (the ⚠️ block in [[sonnet-orchestrator-v2]] `get_customer_account`). The writer runs `validateLinkProposal` BEFORE the row lands — it gates on (a) `confidence: "high"` (Phase 1's address/phone corroboration), (b) workspace scope (`candidate_customer_id` is in the ticket's workspace), (c) same-customer refusal, and (d) `previously_rejected: true` requires `reconfirmed: true` (the NEVER-silent-auto-link guard — a weak bulk rejection stays load-bearing until re-affirmed). A low-confidence proposal, a cross-workspace candidate, or a same-customer link is rejected at the write point with `TicketDirectionPlanError`.

The worker's `runTicketHandleJob` calls [[sol-link-proposal]] `applySolLinkProposal` as **step (0) of the mechanism-dispatch try block** — BEFORE the standalone-journey wedge, the chosen-path journey/workflow/playbook launch, and any stateless send. The applier is deterministic + idempotent: it upserts the `customer_links` group (or returns `already_linked` on a duplicate pair), clears stale `customer_link_rejections` on a re-confirm, and stamps an internal `ticket_messages` note with the confidence + signals + the reason Sol/June wrote. On apply it returns one of `linked | reconfirmed | already_linked | low_confidence_skipped | needs_reconfirm | same_customer | candidate_not_in_workspace | candidate_missing` so a failing link that would poison the remedy is surfaced to the human before the box fires the remedy, never silent. When June is the rung in charge, she authors `link_customer_accounts` as the FIRST action in her `approve_remedy` batch ([[cs-director-call]]) — the executor's all-or-surface semantics park the whole batch if the link would fail.

Cross-links: [[account-matching]] · [[sol-link-proposal]] · [[../tables/customer_links]] · [[../tables/customer_link_rejections]].
...
**Journey close keys on the launch signal, not `send_ok` (fix 2026-07-10).** A journey's close does NOT flow through `classifySolBoxTurnAction` — it's a separate box-lane step (`journeyLaunched → closeTicketOnResolvingReply`), because the two journey paths record a successful launch on DIFFERENT flags: an explicit `chosen_path='journey'` sets `send_ok=true`, but the **standalone wedge** (`plan.launch_journey_slug` on a `stateless`/any path) deliberately leaves `send_ok=false` and records success on `launchedStandaloneJourney`. The original close guard `send_ok && (launchedStandaloneJourney || chosen_path==='journey')` was therefore DEAD for the standalone path (its flag can never coincide with `send_ok=true`), so a standalone cancel-subscription CTA shipped but the ticket stayed `open` (ticket `7d030501`, twin of `fc54b9fc`). The box now closes on EITHER launch signal: `launchedStandaloneJourney || (send_ok && chosen_path==='journey')`. Both journey paths set `mechanismManagesStatus=true`, so the every-message-closes classifier is correctly skipped for them (no double close).
...
## TicketDirectionPath ↔ ticket_direction_path enum — two sides of one source of truth

The `TicketDirectionPath` TS union above and the `public.ticket_direction_path` Postgres enum are two halves of the same set — adding a Sol path means editing BOTH the union (plus its `validatePlanForPath` branch) and shipping an `ALTER TYPE ... ADD VALUE IF NOT EXISTS '<path>';` migration in the SAME PR. Skipping the enum side is exactly the drift that killed three Sol sessions between 2026-07-28 and 2026-08-05 ('workflow' listed in the union but missing from the enum → every workflow Direction died on the insert; Amy Schwartz's ticket is the ground-truth case). `scripts/_check-ticket-direction-path-drift.ts` (wired into `npm run predeploy:static` as `check:ticket-direction-path-drift`, Phase 2 of [[../specs/ticket-direction-path-workflow-enum-drift]]) is the rail: parses the union members out of `src/lib/ticket-directions.ts` source text, reads the live enum labels via `pg_enum join pg_type`, and fails predeploy naming any value present in one and missing from the other. When the pooler is unreachable it SKIPS with a clear message and exits 0 (matches [[control-tower/migration-drift]] and the other DB-touching predeploy guards) — the box worker's predeploy has the env and is where the drift class actually lives.

---

[[../README]] · [[../tables/ticket_directions]] · [[move-replacement-offer]] · [[sol-move-dead-end-guard]] · [[sol-link-proposal]] · [[../specs/sol-ticket-direction-artifact-and-first-touch-box-session]] · [[../specs/sol-reads-moved-as-address-update-and-replacement-offer-not-cancel-deadend]] · [[../specs/account-linking-address-aware-confidence-graded-and-cs-searchable]] · [[../goals/sol-ticket-direction-then-cheap-execution]] · [[../functions/cs]] · [[../../CLAUDE]]
