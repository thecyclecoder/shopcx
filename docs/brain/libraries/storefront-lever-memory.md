...
| `computeChapterPriorsFromFunnel` / `seedChapterPriorsFromFunnel` | `({ workspaceId, … }) → …` | Recompute chapter-level priors from real funnel `chapter_dwell`/`cta_click` share ([[../tables/storefront_events]]); `seed…({apply:true})` writes them onto chapter levers. The coarse dwell+CTA prior — the FALLBACK for low-traffic surfaces. |
