...
| `reconcileRequiredOutcomesForFiredActions(ctx, firedActions)` | wire-in | After a rescue action fires, walks pending/failed rows whose (kind + target identity) match a fired action's `(actionType + actionParams)`, then runs [[action-executor]] `verifyActionInDB` against the row's own action shape and CAS-transitions to `verified` iff the live predicate holds. The **failed→verified** path the honor step deliberately doesn't cover — the honor step treats a failed row as terminal and never re-dispatches; the reconciler only READS live DB state to close a row whose predicate the rescue already made true. |
...
- **Target-identity gate — kind alone is NEVER sufficient.** `findMatchingFiredAction` requires the row's kind + every present identity field in its `target_ids` to match the fired action's `actionParams`. `verifyActionInDB`'s broader arms (e.g. `create_return` — confirms ANY non-cancelled return on the ticket) would otherwise let a successful rescue on order A false-close a failed row for order B on the same ticket. A row with empty `target_ids` on an identity-sensitive kind fails-closed (no identity → no proof of same target → row stays open). This is the pre-merge `sec:real-vuln` fix.
...
The reconciler runs on `handleApproveRemedy`'s success path (after `executeSonnetDecision` returns without escalation). For each pending/failed row whose `kind` matches a fired action AND whose `target_ids` identity fields match the fired action's `actionParams` (via `findMatchingFiredAction` + `KIND_IDENTITY_FIELDS`), it re-runs `verifyActionInDB` against the row's own action shape (`outcomeToActionParams`). If the live predicate holds — the director's rescue satisfied THIS row's exact target — CAS from the row's current status → `verified`. If not, the row is left as-is (no false close on a broken read or a different target). The pass writes an internal `[cs-director/approve_remedy] Reconciled N required-outcome row(s)…` sysNote naming each closed row. A row with empty `target_ids` on an identity-sensitive kind (e.g. Sol-queue-executed rows that never populated `target_ids`) is deliberately left open — the security constraint outranks closure; a human re-authors a fresh Direction to retry.
