...
| Export | Notes |
|---|---|
| `enqueueImitationQualityReview({workspaceId, skeletonIds})` | Inserts ONE `agent_jobs` row with `kind='imitation-quality-review'` + `instructions={skeletonIds:[…]}`. No-op on an empty `skeletonIds` (a sweep that ingested nothing new has nothing for Max to review). Called by [[../inngest/creative-scout]] `sweepWorkspace` at the end of each workspace's sweep (query filter: `status='analyzed' AND media_type='static' AND created_at >= sweepStartIso`). |
