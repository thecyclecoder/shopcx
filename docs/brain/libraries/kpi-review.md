...
- `audit-platform-scorecard` step on [[../inngest/platform-director-cron]] (Phase 5) — runs `auditAllKpis` on the standing pass for every cadence, upserts ONE [[../tables/kpi_audit_log]] row per `(workspace, metric, cadence, snapshot_date)` for trend, and on **persistent** drift (≥2 consecutive snapshot_dates over tolerance) opens a [[../tables/loop_alerts]] incident keyed `loop_id='kpi_drift:<metric>:<cadence>'`. A recovered metric auto-resolves the open alert; a single-snapshot blip is logged but NOT alerted (self-healing on timing noise). The open alert feeds [[platform-director]] `reconcileErrorBacklog` (no MONITORED_LOOPS registry entry needed) and surfaces on [[../dashboard/control-tower]] + the daily watch line.
