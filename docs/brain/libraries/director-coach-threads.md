...
A resumed coach turn pins to the thread's stored `box_session_id`. If that session is gone from its owning account (box restart, session-store rotation, expiry — `claude --resume` returns `No conversation found with session ID: <uuid>`), `runDirectorCoachJob` retries the turn ONCE with `sessionId=null` — the closure rebuilds the FULL transcript prompt (via `renderCoachTranscript`) when the session id is null, so a fresh start loses no context. All four resumable box-chat lanes (director-coach, dev-ask, spec-chat/roadmap-chat, ticket-improve) route through the SHARED wrapper [[box-session-resume]] `runBoxTurnWithFreshFallback` + persist via `pickNextSession` so a dead id can never be re-saved. The wrapper preserves the account-pool failover on the retry (a cap during retry still hops → `blocked_on_usage`). See [[../specs/box-chat-resume-falls-back-fresh-on-missing-session]].
