...
## Fresh-fallback on a missing box session
A resumed coach turn pins to the thread's stored `box_session_id`. If that session is gone from its owning account (box restart, session-store rotation, expiry — `claude --resume` returns `No conversation found with session ID: <uuid>`), `runDirectorCoachJob` retries the turn ONCE with `sessionId=null` — the closure already rebuilds the FULL transcript prompt (via `renderCoachTranscript`) when the session id is null, so a fresh start loses no context. The detector is the tiny unit-tested helper [[box-session-resume]] `isMissingSessionError(raw)`; the retry re-runs the same failover closure through `withAccountFailover` so a cap during retry still hops. On success the fresh session id is persisted back to the thread so it self-heals — the thread never re-wedges on the same dead id. Fires exactly once per turn; a fresh retry that also errors flows through the normal `failTurn` path. See [[../specs/box-chat-resume-falls-back-fresh-on-missing-session]] Phase 1 (director-coach); Phase 2 factors the same guard into the sibling resumable lanes (dev-ask, roadmap-chat, ticket-improve).
...
[[../README]] · [[../tables/director_coach_threads]] · [[cockpit-resolver]] · [[god-mode]] · [[../lifecycles/director-cockpits]] · [[director-instructions]] · [[dev-message-threads]] · [[box-session-resume]] · [[../specs/worker-grading-and-director-management]] · [[../../CLAUDE]]
