...
2. **Carry forward tags, playbook/journey state, do_not_reply** to the target if the target doesn't already have them (target's state wins on conflict). **`agent_intervened` is NO LONGER carried forward** (Phase 3 of `docs/brain/specs/human-directives-hard-gates-over-ticket-ai.md`): a merge conveys the customer's full CS journey (context), never control. If the target should be human-owned that's `ai_disabled` (Phase 1) or `analyzer_locked` (Phase 2), plus an explicit `assigned_to`. The source stub still stamps `agent_intervened=true` from the moment a human actually replied on it; it just doesn't propagate to the target.
   - **`assigned_to` still carries forward only when the source's `agent_intervened=true`** (a real human worked it). A bare routine/auto-assignment (assigned to the agent-todo routine with `agent_intervened=false`) must NOT propagate — it would flip the merged ticket's OWNERSHIP to a routine even though no human is on it. (Ida McDonald 2026-06-10: a merged routine-assignment killed her refund flow.) Here `agent_intervened` is a QUALIFIER for propagating ownership control — distinct from Phase 3's "directives beat gates" work, which retired the sonnet-orchestrator half-mode reply that used to fire on `agent_intervened=true`.
   - **`ai_disabled` is DELIBERATELY NOT carried forward.** It's a per-ticket human directive against one specific conversation; folding it onto the surviving ticket would silently disable the AI on unrelated customer threads the target already carries. The surviving ticket keeps its own value (default `false`). Phase 1 of `docs/brain/specs/human-directives-hard-gates-over-ticket-ai.md` — a merge conveys context, never control.
   - **`analyzer_locked` is DELIBERATELY NOT carried forward** for the same reason: a human's "I reviewed this, do not re-open it" applies to one specific conversation, not to whatever unrelated threads the target already carries. Target keeps its own value (default `false`). Phase 2 of the same spec.
