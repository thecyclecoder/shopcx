...
- **LOYALTY block SUMS balance across the link group and lists redemptions across every sibling profile — labeled with the owning email.** `getCustomerAccount` no longer runs the max-pick `admin.from("loyalty_members").in("customer_id", allCustIds).order("points_balance", desc).limit(1)` — it now routes through the [[loyalty]] SDK chokepoints `getMemberByCustomerId` (aggregated canonical + summed balance) and `getMembersInLinkGroup` (every sibling row). The unused-coupon read is scoped `.in("member_id", allMemberIdsInGroup)` and each coupon is tagged with the owning profile's email so the LLM (and any human reviewer) can match a code to the sub/order it can actually apply to. The block also carries an explicit line: *"points are combined across every linked profile above, but a loyalty CODE only works on orders and subscriptions belonging to the profile that earned it"* — a Shopify discount minted with `customerSelection.customers.add` is locked to ONE customer, so a sibling profile's code on the wrong sub is a guaranteed no-op. Precedent: ticket 2b7ea029 (Sandra Lutz) — a 100 + 51 split balance was reported as 100 and every "unused reward" surfaced belonged to a profile that did not own her subscription. Spec: loyalty-coupon-apply-resolves-contract-owning-member-no-doomed-regen Phase 3.
