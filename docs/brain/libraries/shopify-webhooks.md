...
### `handleRefundCreate` — function

```ts
async function handleRefundCreate(workspaceId: string, payload: Record<string, unknown>) : Promise<void>
```

Handler for the Shopify `refunds/create` webhook — the primary vendor-side path into the [[../tables/order_refunds]] mirror ledger. Delegates to [[vendor-refund-mirror]] `insertShopifyRefundMirror`, which is idempotent on both `(order_id, request_key)` (the unique index) AND `vendor_refund_id` (a semantic guard against echoing a refund we already mirrored via [[refund]] `refundOrder`). Records only refunds the vendor already completed — never moves money.
...
- **`handleOrderEvent`'s refund-detection branch calls [[vendor-refund-mirror]] `reconcileShopifyRefundsForOrder` as the fallback path** into [[../tables/order_refunds]]. The primary path is the `refunds/create` webhook (`handleRefundCreate`); the reconcile catches anything that path missed — a refund fired before the topic was registered, a dropped webhook delivery, or a hand reconcile on the Shopify order directly. Reads `/orders/{id}/refunds.json` and mirrors each refund the vendor has completed. Idempotent — a refund already mirrored via `refundOrder` or `handleRefundCreate` is skipped by the vendor_refund_id semantic guard.
