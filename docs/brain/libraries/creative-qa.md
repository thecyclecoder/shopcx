...
### Parse error retry — reformat, never re-review ([[../specs/max-critique-reaches-dahlia-and-the-box-card-shows-one-face]] Phase 1)

When `runQaCreativeCopyViaBoxSession` calls `parseCopyQaVerdict` and the result is `kind:'parse_error'`, the function **resumes Max's session ONCE** with a short re-ask that names the parse error reason verbatim and restates the required envelope (top-level `hard_gate_pass` boolean, `hard_gates` with the four keys, `persuasion_score`, `persuasion_rubric`, `verdict_reason`). This is a FORMATTING correction — Max keeps his judgement identical and re-emits ONLY the envelope. The re-ask succeeds (returns ok with the reformatted verdict) or fails a second time (recorded as a genuine miss; see audit below). A caller that cannot resume (a legacy / scripted dispatcher without `sessionId`) settles the first parse failure as permanent and signals an audit without a re-ask.

The retry uses `buildCopyQcParseErrorReAskPrompt` to construct the re-ask and passes it back through the same `dispatch` closure with a `{ sessionId, sessionConfigDir }` pin to `runBoxLane`. If the session is gone / cache-miss between attempts, `secondDispatch.missingSession` signals that and the outcome is `kind:'parse_error'` (no second-attempt text). The `CopyQcSessionOutcome` discriminator `{ kind: "parse_error", reason, firstRawHead }` carries the first dispatch's raw text (~500 chars) for audit visibility when neither pass nor retry succeeds.
...
## Max verdict-miss / parse-miss STILL bins HELD ([[../specs/a-max-copy-qc-miss-still-bins-the-ad-held-never-drops-it-so-ceo-can-review]] Phase 1 — WIRED, 2026-07-18)

The always-bin oversight model had ONE hole through which a real ad silently vanished before the CEO could review or override: `insertReadyCreative`'s pre-write cold-offer refusal fired even on a bin-ineligible insert. When Max's session parse-errored on every attempt of Dahlia's copy-author loop, `verifyMaxCopyQcForVerdict` (the [[creative-agent]] `stockProduct` closure) returned `{ok:false, maxVerdict:null}` — the `!qcRun.verdict` branch — and the loop settled `outcome.maxCopyQcMissed=true` + `outcome.lastAuthorVerdict=<Dahlia's caption>` + `outcome.reason='max_qc_verdict_missed (floor=9)'` (from the exported `buildMaxQcReviseReason(null)`). `stockProduct`'s always-bin branch reached `insertReadyCreative(maxQcEligible: false)` — but the persister refused with `{kind:'skip', reason:'cold_offer_leak'}` on a variation the canonical loop's own cold-offer gate had never seen, and the batch summary landed `bin_ineligible_insert_failed: max_qc_verdict_missed`. Superfood Tabs attempt 3 was the first live proof: a potentially-good creative silently vanished on a shape technicality (a parse error), not a content failure.

The fix is a small pure predicate at the persister choke-point in [[creative-agent]]: `shouldRefuseColdOfferInsert(audienceTemperature, copyPack, maxQcEligible)` — returns `true` (refuse) IFF `maxQcEligible !== false` AND the audience is `cold` AND `hasColdOfferLeak(joined pack copy)`. A caller stamping `maxQcEligible: false` — the always-bin bin-ineligible path — is intentionally landing HELD for CEO review, so a cold-offer leak becomes just another disposition on the row (visible on the ad detail page; Bianca's `.not("max_qc_eligible","is",false)` filter still hides it from her postable list). A postable insert (TRUE) or a deterministic-mode / kill-switch-off insert (NULL) still refuses on a cold-audience leak — a postable creative with offer language reaching Bianca's ready-to-test is the #1 DTC creative error (dahlia-audience-temperature-marking-and-cold-offer-gate Phase 2). Postability stays gated (Max's 9/10 floor + `override_postable` via [[postability-override]]); the ad is simply always reviewable.

Every ad Dahlia produces (parse / self-score / cold-offer / validator / firewall gates cleared; Max was the only block) now lands in the bin held / ineligible, with the miss reason preserved via `outcome.reason` on the batch-summary row and the `director_activity` ledger row `action_kind='max_qc_below_floor_exhausted'` — never dropped before the CEO can review or override. A genuinely unusable emit (no caption at all — firewall / author-self exhaustion with no `lastAuthorVerdict` surfaced) can still be refused per the pre-Phase-1 discard-and-escalate rail; the caption never cleared claim-safety or parse gates, so there's nothing safe to bin. Pinned by 6 tests in [[../../../src/lib/ads/creative-agent.author.test.ts]] (the four `shouldRefuseColdOfferInsert` matrix cells — false/true/null over a cold-offer-leaking pack + the clean-cold pass-through regression + the verdict-missed loop-outcome shape that proves `lastAuthorVerdict` surfaces on a dispatch/parse miss too, not just below-floor).
