...
| `DEFAULT_TEST_TARGETING` | Meta-valid broad Advantage+ Audience shape — US home+recent geography + `targeting_automation.advantage_audience=1`, with NO hard `age_min`/`age_max`/`genders` gate. Meta rejects an Advantage+ Audience ad set whose `age_min > 25`; the old `age_min:50 / age_max:65 / genders:[2]` default was one of those rejections and froze Bianca's per-test replenish loop. Callers can still override per product/account. |
| `META_ADVANTAGE_AUDIENCE_MAX_AGE_MIN` | Fingerprint = `25`. Meta's ceiling on `targeting.age_min` for an Advantage+ Audience ad set. Exported so any future edit that reintroduces an above-25 hard age minimum on the default template trips a named regression, not a silent 400 at publish. |
...
- **Default targeting MUST publish under Advantage+ Audience.** Meta rejects an Advantage+ Audience ad set (`targeting_automation.advantage_audience=1`) whose `targeting.age_min > 25`, so `DEFAULT_TEST_TARGETING` omits `age_min`, `age_max`, and `genders` and keeps only US home+recent geography + Advantage+ Audience on. The fingerprint `META_ADVANTAGE_AUDIENCE_MAX_AGE_MIN = 25` is exported from the same file and asserted by the `provision-cohort.test.ts` regression pin, so a stray edit that reintroduces the old hard 50-65 combo trips the unit test instead of silently republishing a 400-rejected payload that freezes Bianca's replenish loop. This aligns the code, the tests, and [[../reference/meta-scaling-methodology]] § "Test audience held constant = broad, no age cap".
