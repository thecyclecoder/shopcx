...
- `classifyPredeployViolationScopeIfSafe({ out, changedPathsResult }): { owned, inherited, allInherited, paths } | null` — the Phase 3 fix that safe-wraps `classifyPredeployViolationScope` to prevent stale `changedPaths` from silently routing violations as inherited. Returns `null` if the `git diff` command failed (`changedPathsResult.ok === false`) or if the diff reported zero changed files (`changedPathsResult.paths.length === 0`), forcing the caller to fall back to the repair-it behavior rather than silently skip; otherwise delegates to `classifyPredeployViolationScope`. The type-level guarantee prevents the worker from accidentally using a stale or empty list. Called by [[../../scripts/builder-worker]] `runBuildJob` predeploy:static block only, after every possible repair that modifies the worktree.
