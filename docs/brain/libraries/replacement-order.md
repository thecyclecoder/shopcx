...
- **Every `variantId` is resolved to its Shopify id before the draft is built.** `createReplacementOrder` now calls [[product-variants]] `resolveShopifyVariantId(workspaceId, variantId)` per input item as its first step (before the cap check, before the row insert). A numeric id passes through; our `product_variants.id` UUID is looked up via `shopify_variant_id`. An unresolvable ref (internal-only variant with no Shopify counterpart, bad ref) refuses the batch up front with a specific error ("Replacement variant not on Shopify: <title> (<id>) — no shopify_variant_id for this product_variants row") instead of leaking Shopify's opaque "Product with ID X is no longer available" from deeper in the pipe. An internally-billed renewal (SHOPCX*, no `shopify_order_id`) carries UUIDs on its line items — ticket `1aea6114-7417-421f-99d0-05cce22f2ff6` (SHOPCX272) is the ground truth. The initial `replacements.items[]` row row now stores the RESOLVED numeric Shopify id, so downstream code reading the row sees the same shape it always did.
...
- **Security: `create_replacement_order` amount never sized from caller-supplied fields.** The approval gate in [[june-remedy-approval]] must never size a replacement order from caller-supplied `amount_cents` or `replacement_amount_cents` — the underlying executor (`createReplacementDraftOrder` here + `createReplacementOrder` in [[replacement-order]]) always applies a hardcoded 100% discount and ignores the caller's number. Before [[../specs/replacements-must-work-for-internal-non-shopify-renewal-orders]] Phase 2, a CS Director verdict could set `amount_cents: 1` to bypass the founder gate; now `extractPayloadAmountCents` short-circuits the field to `null` (unknown), forcing the whole batch to gate conservatively. See [[june-remedy-approval]] § Gotchas — "create_replacement_order is NEVER sized from…".
