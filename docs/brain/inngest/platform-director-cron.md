...
- [[../tables/kpi_audit_log]] (the `audit-platform-scorecard` step's per-metric drift verdict — one upserted row per `(workspace, metric, cadence, snapshot_date)`, via [[../libraries/kpi-review]])
- [[../tables/loop_alerts]] (opens / refreshes / resolves a `kpi_drift:<metric>:<cadence>` incident on persistent drift)
