...
- [[../tables/kpi_audit_log]] (the `audit-platform-scorecard` step — one row per metric per cadence per beat, via [[../libraries/kpi-review]] `auditAllKpis`; the trend store the persistent-drift alerter reads)
- [[../tables/loop_alerts]] (the `audit-platform-scorecard` step opens/refreshes/resolves `kpi_drift:<metric>:<cadence>` rows with `owner='platform'` when a KPI is over-tolerance for ≥2 consecutive audits)
