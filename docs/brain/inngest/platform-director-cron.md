...
- [[../tables/platform_scorecard_snapshots]] (the daily scorecard pulse + the **weekly** throughput/quality rollup — one upserted row per daily/weekly KPI per workspace, via [[../libraries/platform-scorecard]])
- [[../tables/loop_heartbeats]] (end-of-run heartbeat)

## Tables read (not written)

- [[../tables/agent_jobs]] (build-console workspace scan + in-flight dedupe; target-build conclusion for the auto-approval grade; the concluded worker actions to grade; **scorecard daily**: merged builds for throughput + repair/regression jobs for derived MTTR; **scorecard weekly**: platform-owned merged builds for specs/week + idea→merge cycle time, build success vs failed/needs_attention, concluded regression jobs)
- [[../tables/approval_decisions]] (the autonomous director approvals to grade; **scorecard daily**: autonomy ratio + escalations; **scorecard weekly**: % approvals untouched)
- [[../tables/director_activity]] (the `escorted_goal` rows that flag a goal the director escorted; **scorecard daily**: `escalated` rows; **scorecard weekly**: `detected_regression｜authored_fix｜dismissed_regression` rows for regressions caught)
