...
## What the pass does (Phase 2)

Phase 1 wires the cron + the dispatchable box lane + the node-completeness trio (owner / kill-switch / heartbeat) and ships a heartbeat-emitting stub lane. Phase 2 implements the pass logic inside `runAdsSupervisorJob` ([[../libraries/ads-supervisor]]): getTestingResults (via [[../libraries/testing-results-sdk]]) → iteration-policies decision tree → should-pause / should-crown list → **Bianca-acted coverage check** (via [[../libraries/ads-supervisor]] `readExecutedIterationActionsForAdsets` + `readRecentKillRailDeferralsForAdsets` + `readRecentScaleRailDeferralsForAdsets` from `director_activity` — recognizing legitimate guardrail deferrals within the last 24h) → Dahlia bin depth vs floor + competitor-seeded proven-angles check → live-ad LF8 QA of headline / primary-text / destination → deduped fix-spec authoring via `authorSpecRowStructured` (owner=growth, machine-only checks) → ONE director-voice digest to #director-growth-max (`postAsGrowthDirector`), suppressing identical no-op digests.

**Coverage model — no false positives on defended deferrals:** an adset is covered for kill iff `iteration_actions` carries a row with `action_type='pause'` AND `status='executed'`, OR Bianca's most recent pass deferred a kill on a guardrail (e.g., `never_pause_object_ids`, `per_account_daily_budget_delta_ceiling`) within the last 24h (tracked via [[../tables/director_activity]] `action_kind='media_buyer_kill_rail_deferred'`). An adset is covered for crown iff `iteration_actions` has a row with `action_type='scale_up'` AND `status='executed'`, OR Bianca deferred a promote within 24h (`action_kind='media_buyer_scale_rail_deferred'`). A deferral older than 24h drops, so a stale hold cannot mask a fresh miss if the guardrail is lifted.
