...
- **Transient transport noise** ([[../specs/error-feed-drop-inngest-transport-http-unreachable]]): `inngest/function.failed` fires for both app throws AND Inngest's own transport failures (`http_unreachable` — Inngest couldn't get a clean reply from our Vercel SDK URL: a deploy-boundary Lambda reap / connection reset against an every-15-min cron, where the function body never threw and the next beat recovers). The handler calls `isTransientInngestTransportError(errName, errMessage)` ([[../libraries/control-tower]]) and passes the result as `recordError`'s **`transient`** flag — so a FIRST sighting is auto-resolved (recorded + grouped, **no page, no repair fan-out**) and it escalates to a real open+page only if the same signature **recurs within an hour** (a chronic always-failing function). The one-off blip that opened `inngest:06e8cf82e141fbaa` (`shopcx-platform-director-cron`) is dropped; a genuine chronic timeout still surfaces.
