...
- Loads the [[../tables/ad_publish_jobs]] row. **Preflight guard:** if the resolved publish path is placement-customized (3-bucket PAC or dual-asset), calls [[../libraries/ads__publish-instagram-identity-guard]] `shouldRefuseForMissingInstagramIdentity` with `placementReady` / `dual` / `j.meta_instagram_user_id`. Refusal marks the job `failed` with reason `missing_instagram_identity` and returns normally (no throw), so `/api/inngest` stops emitting unhandled Graph errors for fixable configuration problems. → installs [[../libraries/meta__app-owner-action-escalation]] `installDefaultAppOwnerActionEscalationHandler(admin)` and scopes the handler to this workspace (via `runWithAppOwnerActionWorkspaceScope`) so a Meta Data Use Checkup 400 raised inside any Graph call below books the deduped CEO card against the RIGHT workspace. → `uploadAdVideo` (re-signed video `file_url`) → `waitForVideoReady` (poll) → `createAdCreative` (asset_feed_spec copy variants) → `createAd` (**PAUSED** unless `publish_active`). **Meta app owner action containment:** if any Graph call throws `metaClass === 'app_owner_action_required'` (canonical: the yearly Data Use Checkup disabling the app until the workspace owner completes it in the Meta App Dashboard), the publisher catches it, marks the job `failed` with reason `meta_app_owner_action_required`, clears `publish_active` to prevent any spend, and returns normally (no throw). The escalation handler installed above already booked the deduped CEO card; retrying Inngest would only flood `/api/inngest` with identical crashes and waste invocations since only a human clearing the gate can fix this class. **Adset error handling:** if `createAd` throws a Graph error, [[../libraries/publish-adset-unavailable-classifier]] `isMetaAdsetUnavailableError` classifies whether the target ad set is unavailable (deleted, archived, or permission-denied) — a permanent config problem, not transient. On match, the job is marked `failed` with reason `meta_adset_unavailable` and the step returns normally (no throw), preserving visibility that the publish failed while preventing `/api/inngest` crashes and Control Tower incidents for fixable ad-set issues. Other unexpected errors from `createAd` still rethrow so real infra regressions surface. → writes `meta_video_id`/`meta_creative_id`/`meta_ad_id` + `publish_status`. The ad name is `ad_publish_jobs.ad_name` when set (engine-created `[ie]` drafts — Iteration Engine 6b), else the campaign name. Graph v21.0 via [[../libraries/meta-ads]]. **6b write-back:** when the job carries a `recommendation_id`, on publish it flips that [[../tables/iteration_recommendations]] row to `status='executed'` with the meta ids (or `failed` if adset unavailable). Routes: `POST /api/ads/campaigns/[id]/publish` (+ `/meta-copy`, `GET /api/ads/meta`); also fired by [[meta-performance]] `meta-execute-recommendation`. See [[../lifecycles/ad-publish]].
