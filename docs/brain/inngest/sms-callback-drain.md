...
- **Control Tower heartbeat:** after the `rollup` step, an `emit-heartbeat` step calls `emitCronHeartbeat("received-sms-rollup-cron", { ok: true, produced: { emitted, flagged } })`. Fires on BOTH the no-work path (`emitted=flagged=0`) and the processed-work path so the watchdog can distinguish a healthy idle tick from a dead Inngest schedule — same pattern as [[monthly-revenue-snapshot]] and every other monitored cron. Registered as a platform-owned `cron` loop in [[../libraries/control-tower-registry]] with a 20 min liveness window and a `registeredAt` grace anchor so the initial deploy → first-firing gap can't false-page `registered_not_firing` before the first beat lands (received-sms-rollup-cron-heartbeat Phase 2 Fix 1).
