...
- **Allergen-hold skip:** an order under an active `hold_status='placed' AND hold_kind='allergen'` (see [[../tables/orders]] hold columns + [[../libraries/order-holds]]) is a protective hold raised by the ticket-escalation path — the sweep returns `skipped-allergen-hold` and MUST NOT hand the parcel off to the warehouse. Same rail as the fraud-held skip; a released hold flips the order right back into the eligible set on the next tick.
