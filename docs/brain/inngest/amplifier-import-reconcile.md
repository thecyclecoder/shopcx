...
- **Heartbeat:** `emitCronHeartbeat('amplifier-import-reconcile', {ok, produced, detail, durationMs})` at end of run with a `{scanned, imported, failed, skipped_fraud, skipped_no_skus, skipped_non_storefront, grace_cutoff}` payload. Non-fatal — a heartbeat write cannot fail the sweep.
