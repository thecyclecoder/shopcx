...
- **Self-monitoring:** emits its own `spec-drift-reconcile` heartbeat at the end (`emitCronHeartbeat`), registered in `src/lib/control-tower/registry.ts` so a dead reconciler shows as a stale cron tile.
