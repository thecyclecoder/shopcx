...
- **kind='drift' | 'frustration'** — the drafted reply is HELD (no `ticket_messages` outbound row for this turn — the gate returns BEFORE Sonnet/playbook runs, so no reply is even drafted). One [[../tables/ticket_resolution_events]] row is staged with `reasoning='sol:inflection-<kind>'` and the classifier `evidence` blob stashed in the jsonb `chosen` column. On `frustration` AND `ai_channel_config.sol_frustration_holding_message_enabled !== false` (migration-default `true`), the holding message is deferred via [[../libraries/holding-message-defer]] `enqueuePendingHolding` — it is written as a PENDING row with `pending_send_at = now + HOLDING_DEFER_MS` (90 seconds) rather than sent synchronously, so a fast re-session answer is not preceded by a stall message (both go to the customer only if the investigation is genuinely slow). The [[../inngest/deliver-pending-send]] cron dispatches the pending row once the deadline passes UNLESS a substantive reply lands first and calls `cancelPendingHoldingMessagesForTicket` at its send chokepoint. Then [[../libraries/inflection-detector]] `reSessionSol` supersedes the live Direction (compare-and-set) and enqueues a new `agent_jobs` row `kind='ticket-handle'` `instructions.reason='inflection'` for the box worker's `runTicketHandleJob` to author a fresh Direction. Drift bounces are silent by design.
...
**Pending-question override in `classify-playbook-msg`.** The classifier resolves the playbook step currently awaiting a customer answer (the same step `executePlaybookStep` would run — [[../tables/playbook_steps]] `step_order = tickets.playbook_step`) and grabs the literal question text from the most recent outbound external `author_type='ai'` [[../tables/ticket_messages]] row. Both are interpolated into the Haiku prompt as `THE PLAYBOOK JUST ASKED …`, alongside two hard rules at the same authority level as `ACCEPT/REJECT OVERRIDES EVERYTHING`: (1) **ANSWERS THE PENDING QUESTION OVERRIDES EVERYTHING** — a yes/no, confirmation, choice, address, order-# answer to what was asked is always PLAYBOOK, beating the lean-NEW_TOPIC default; (2) the pure-gratitude rule fires ONLY on a bare thank-you with no substantive content — "Thank you. Yes confirming this is the correct address." is a confirmation, not gratitude. Ticket 8e2c87d6 (Suzanne Ross 2026-08-24) is the ground-truth incident: the Replacement Order playbook asked "did you not receive your order at all?", she answered "I did not receive the order at all" — classified NEW_TOPIC — then answered the address follow-up and the pure-gratitude rule swallowed the confirmation; no replacement was ever created. See [[../specs/playbook-drift-classifier-sees-the-pending-question]].

**Repeat-question loop guard on `pb-send` / `pb-adv-send-*` ([[../libraries/playbook-repeat-guard]]).** Before every customer-facing playbook reply — the first `pb-send` off `executePlaybookStep` AND each `pb-adv-send-${advCount}` in the auto-advance loop — the handler reads the most recent outbound external `author_type='ai'` [[../tables/ticket_messages]] row and calls `detectRepeatQuestion` on `{ pending, lastOutbound }`. On `repeat:true` (normalized exact / substring / Jaccard ≥ 0.80 over ≥ 5 filtered tokens) the reply is NOT sent — the handler drops a `[System] Repeat-question loop guard tripped — <note>. Not resending; escalating instead.` internal note that names the repeated question, then routes through the SAME `raiseHoldingMessageEscalation` closure the `escalate_api_failure` branch uses (ticket → open + escalated, `SILENT_TURN_HOLDING_MESSAGE` to the customer, Slack header `🚨 *Playbook Repeat-Question Loop*`). In the auto-advance loop the trip additionally `break`s out so a chained duplicate doesn't stack a second escalation. A playbook that asks the same thing twice has lost state, not gathered information; a third identical question is never the right move — Suzanne Ross ticket 8e2c87d6 (2026-08-24) was asked the identical "did you not receive your order at all?" prompt twice and the identical address-confirm prompt twice, then the orchestrator fabricated a "currently in transit" reassurance on a shipment whose last carrier scan was 11 days old. See [[../specs/playbook-drift-classifier-sees-the-pending-question]] § Phase 2.
