...
- **Delivery audit (Phase-2 of [[../specs/growth-storefront-experiment-delivery-verification]]) runs BEFORE the bandit refresh.** Calls `auditExperimentDelivery` ([[../libraries/storefront-experiment-delivery-audit]]); per `failed_to_deliver` row writes one `director_activity` of `action_kind='detected_undelivered_experiment'` ([[../tables/director_activity]]; `director_function='growth'`, metadata `{experiment_id, lander_type, hours_since_start, sessions_count:0, exposures_count:0}`) and stamps `storefront_experiments.last_decision.delivery_flag='failed_to_deliver'` ([[../tables/storefront_experiments]]). Each per-experiment write is its own `step.run` (`audit-failed-${experiment_id}`) so an Inngest retry replays only what didn't complete — idempotent per refresh pass. The bandit refresh then refuses to promote/kill any experiment carrying that flag (holds it with `rule='failed_to_deliver'`), preserving the flag for the Director brief (`loadUndeliveredExperiments` [[../libraries/storefront-experiment-delivery-audit]]).
