...
- **What it does:** calls `runControlTowerMonitor()` — builds the read-only snapshot (`buildControlTowerSnapshot`, shared with the dashboard) over every loop in the registry (`src/lib/control-tower/registry.ts`), then:
  ...
  - **CRON FRESHNESS + NEVER-FIRED** — each registered cron's latest [[../tables/loop_heartbeats]] beat within its `livenessWindowMs` (cadence + grace). Stale → red ("cron X hasn't run in Yh"). **Never-fired keys off the ALL-TIME beat count, not 0-since-deploy** ([[../specs/control-tower-complete-coverage]] Phase 2, accuracy-fixed by [[../specs/control-tower-monitor-accuracy]] Phase 1): a cron with **any** historical beat is being invoked → at most a freshness alert, never `never_fired`. Only a cron with **0 beats in all of history** whose deploy has been live longer than its window (`deployRefAgeMs`: the box worker's `started_at` once its `running_sha` matches `VERCEL_GIT_COMMIT_SHA`) is **red `never_fired`** ("registered but has never run — Inngest not invoking it"); a genuinely-fresh cron (or unknown deploy age) stays **amber "awaiting first run"**. Closes the gap where this very cron sat amber-forever for days because a deploy never re-synced Inngest. The cron + agent-kind beats come from the `control_tower_loop_beats` RPC (grouped per-loop read; replaced the global 600-beat window that false-flagged low-frequency crons + 500-ed). **`never_fired` only fires on a TRUSTWORTHY zero** ([[../specs/control-tower-never-fired-guard-beat-read]]): the RPC's `error` is inspected and `beatsReadOk` is threaded into `evalCron` — a failed/timed-out read (statement-timeout `57014`) returns no rows → an empty `countByLoop` that reads as "0 beats" for *every* loop, which is **unknown, not zero**, so the red is suppressed (loops stay amber) until a read actually succeeds. Closed the false-positive where a live every-minute cron (e.g. `slack-roadmap-notify`, 564 beats) flapped `never_fired` hourly whenever the beat read timed out.
