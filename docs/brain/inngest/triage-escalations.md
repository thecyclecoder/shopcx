...
**Who produces the routine-owned state.** Every system escalation path *defaults* to the routine — `escalated_to = null` (keep `escalated_at` + `escalation_reason`): [[../libraries/ticket-analyzer]] (severity/actionability re-open — see the contract below), the orchestrator `escalate` action (`src/lib/action-executor.ts`), the **playbook guard-block** path (`src/lib/action-executor.ts` `claimGuardBlocksInlineSend` — `blocked_unbacked_claim:*` when a reply asserts a cancel / pause / skip / swap effect that no attached action backs), the workflow executor's `escalate` (`src/lib/workflow-executor.ts`), and portal remediation (`src/lib/portal/remediation.ts`). A human can also pick **🤖 AI Routine** in the ticket escalate dropdown ([[../dashboard/tickets__id]]). See [[../specs/escalate-to-routine-by-default]].

**Escalation-source-agnostic selection ([[../specs/guard-block-escalations-reach-junes-triage-not-left-unreviewed]]).** The cron's eligibility gate is **ticket-level only** — `escalated_at` set, `escalated_to` null, `status` not archived/closed — and NEVER reads `escalation_reason`. An orchestrator escalation, an analyzer-rail escalation, and a playbook guard-block escalation (`blocked_unbacked_claim:*`) are all triage candidates on the same terms. The invariant is pinned by the pure predicate `passesJuneReviewSelection` in `src/lib/inngest/triage-escalations.ts` (unit-tested in `src/lib/inngest/triage-escalations.selection.test.ts`) and re-applied defensively on the fetched row-set so a future SQL edit can't quietly leak an ineligible ticket or silently source-filter a real one. Derived-from-ticket 472310cc-f35f-4631-8e3a-11d7ee7b585f — a `blocked_unbacked_claim` escalation that sat open + escalated with zero `triage_runs` — is the failing state this contract exists to prevent.
...
2. **Prior-review guard (lifecycle-aware)** — skip a ticket ONLY when its most recent `triage_runs.created_at` is at or after the ticket's current `escalated_at`. An older review from a PRIOR escalation (that has since been resolved and the ticket re-escalated) does not block a fresh review — the current escalation lifecycle is unreviewed and gets its own cs-director-call ([[../specs/triage-escalations-prior-review-lifecycle-guard]]). Pinned by the pure predicate `priorReviewCoversCurrentEscalation` in `src/lib/inngest/triage-escalations.ts` (unit-tested in `src/lib/inngest/triage-escalations.selection.test.ts`). Phase 2 of [[../specs/june-review-replaces-solver-skeptic-quorum-triage]] adds an on-demand second-opinion path for genuinely borderline cases (the exception, not a routine re-run) — that path is orthogonal to this guard.
