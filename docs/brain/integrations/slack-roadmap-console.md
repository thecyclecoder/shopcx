...
- **`/build <slug> [instructions]`** → `queueRoadmapBuild` (one active build per spec for a plain build; `alreadyActive` surfaced as an ephemeral. `/build` with instructions, like `/bug`, enqueues behind an active build instead — see below).
- **`/bug <slug> <desc>`** → fix-build (`instructions` scoped; spec stays ✅, no spec edit). If a build is already active for the spec the fix is **never dropped** — it's enqueued as a distinct `queued` follow-up (`queuedBehindActive`) that runs after the active build, and the ephemeral says "queued to run next" with the new job's short id. See [[../specs/fix-report-issue-dropped]].
