...
| Endpoint | Method | Purpose |
|---|---|---|
| `/act_{id}/campaigns` | POST | Create a PAUSED ABO `OUTCOME_SALES` campaign. Requires `is_adset_budget_sharing_enabled=false` when NO campaign budget is set (2026-07-07). CBO branch sets `daily_budget`/`lifetime_budget` in minor units. **Advantage+ Sales knobs (Bianca M4 cold-scaler, 2026-07-17)**: `existing_customer_budget_percentage` (integer 0-100 — `0` = new-customer-only, the shape the cold-scaler campaign is minted with; `null`/omitted keeps the existing ABO test-campaign body unchanged) + `smart_promotion_type="AUTOMATED_SHOPPING_ADS"` (flags the campaign as Advantage+ Sales so Meta activates the new-customer surface). Both surfaces are pass-through: absent by default so nothing about the existing test-campaign creation changes; present only when Bianca's cold-scaler `getOrCreateColdScalerCampaign` mints the cohort's scaler campaign. |
...
Wired via `createCampaign` + `createAdSet` + `getOrCreateTestingCampaign` + `getOrCreateColdScalerCampaign` in [[../libraries/meta-ads]]. The loop reuses one shared `"MB — Testing (ABO)"` campaign per account (idempotent find-or-create by name), then creates one PAUSED ad set per concept under it — going-live is a separate governed step in `src/lib/meta/recommendation-execute.ts`, never automatic. The Bianca cold-scaler graduate verb ([[../specs/bianca-cold-scaler-graduate-crowned-winners-to-advantage-plus-new-customers]]) mints one PAUSED consolidated Advantage+/CBO `OUTCOME_SALES` campaign per cohort with `existing_customer_budget_percentage=0` (new-customer-only) + `smart_promotion_type="AUTOMATED_SHOPPING_ADS"` and stamps its id on `media_buyer_cold_scaler_cohorts.scaler_meta_campaign_id` via `setColdScalerCampaignId` (compare-and-set).
