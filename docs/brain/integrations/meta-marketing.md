...
- **Reconnect-required** — a class distinct from app-owner-action-required: Meta has invalidated the stored per-workspace user token itself. The app-level gate is CLEAR — the App Dashboard has nothing left to do — and only OAuth re-consent restores access. Classified by [[../libraries/meta__graph-retry]] `classifyReconnectRequired` on HTTP 400 when the message contains `"api access blocked"`. `graphFetchJson` short-circuits the retry loop and tags the throw with `metaClass='reconnect_required'`; the CEO card is raised through [[../libraries/meta__reconnect-required-escalation]] `escalateReconnectRequired`, which CONFIRMS THE TOKEN IS DEAD via a `debug_token` probe before raising the card — a single-sighting string trigger MUST NOT be enough. Card links to `/dashboard/settings/integrations/meta` (never the App Dashboard). Introduced by **[[../specs/meta-reconnect-required-class]] Phase 1** — the 2026-08-02 incident where "API access blocked." was misrouting the founder to the App Dashboard while ad spend accrued unmeasured.
- **Human-blocked diagnostic — how to tell the two classes apart from the observed symptom (2026-08-02).** Meta served BOTH phrasings during the same incident, roughly 40 minutes apart. The distinguishing test is the app-token / user-token asymmetry:
  - **app token 200 + user token 400** → the stored credential is dead → `reconnect_required` → **re-authorize OAuth at `/dashboard/settings/integrations/meta`**.
  - **both blocked with a Data Use Checkup phrasing** → `app_owner_action_required` → **complete the flagged item in the Meta App Dashboard**.
  Note that Meta served `"API access disrupted…"` and `"API access blocked."` as DIFFERENT states of the same incident, and that the second phrasing was observed EXACTLY ONCE — hence the confirm-before-escalate `debug_token` probe on the reconnect_required side. Both classes contain at the same 5 human-blocked call sites via [[../libraries/meta__graph-retry]] `isHumanBlockedGraphError` — a new human-blocked class only requires editing that one predicate, not five copy-pasted string comparisons.
