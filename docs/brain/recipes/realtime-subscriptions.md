...
- **⚠️ Never join `public.workspace_members` from an RLS policy — it recurses.** `workspace_members`' own SELECT policy is self-referential (`workspace_id IN (SELECT workspace_id FROM workspace_members WHERE user_id = auth.uid())`), so ANY policy (incl. a `realtime.messages` broadcast policy) that does a raw `… IN (SELECT … FROM workspace_members …)` throws `infinite recursion detected in policy for relation "workspace_members"`. In the Realtime authorization path that error is **swallowed and the subscriber is silently denied** — the channel goes green, zero events arrive. Use a **`SECURITY DEFINER` helper** that reads membership as the function owner (bypassing RLS); `auth.uid()` still resolves inside it. See `public.user_can_read_box_topic` (migration `20261204120000`) — the pattern for any workspace-scoped policy. **This also means service-role tests are false positives** for RLS: service_role bypasses RLS, so it never exercises the policy the browser hits — always verify the authenticated path (simulate with `set role authenticated` + `set_config('request.jwt.claims', …)` inside a transaction).
