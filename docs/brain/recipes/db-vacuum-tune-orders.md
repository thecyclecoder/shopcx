# Vacuum / autovacuum-tune orders (owner-approval-only)

The recipe behind the [[../libraries/db-health|DB Health Agent]]'s `vacuum_tuning` fix kind applied to `public.orders` — the surface-don't-apply response to the `dbhealth:bloat:orders` signature (cause `bloat_vacuum_lag`). Escalation-shaped like [[raise-work-mem]]: a `VACUUM` reclaims space but takes I/O; a per-table autovacuum re-tune ripples until it finds the right steady state. This recipe is the bounded, reversible starting move — **no data is deleted**.

**Source of truth:** `supabase/migrations/20260704120000_orders_autovacuum_scale_factor.sql` + `scripts/apply-orders-autovacuum-migration.ts`.

## When to apply

The DB Health Agent's [[../libraries/db-health|bloat pass]] (`DB_HEALTH_SIZE_LOOP_ID`) surfaces a `dbhealth:bloat:orders` proposal when the `orders` table crosses two floors together:
...
Diagnosed cause: the default cluster `autovacuum_vacuum_scale_factor = 0.20` means autovacuum only fires after **20%** of the table is dead. On a hot, write-heavy table like `orders` — subscription-fulfillment churn + address/status/UTM updates — that leaves a large minority of dead rows to be scanned between passes. The bloat pass's finding *is* this lag.

## The tune (per-table, no cluster-wide change)

Scoped to `public.orders` via `ALTER TABLE ... SET (reloptions)`. Cluster defaults for every other table stay put — the isolation is the point (mirrors [[raise-work-mem]]'s "scope to one role" story).
...
- **`autovacuum_vacuum_scale_factor = 0.05`** — fire at 5% dead, not 20% (4× more often). Autovacuum's fire predicate is `dead_tuples > threshold + scale_factor × reltuples`; at `orders` scale (millions of live rows) the scale_factor term dominates, so 4× tighter gives ≈4× more passes — enough to hold the dead-tuple ratio below the DB Health Agent's `BLOAT_DEAD_RATIO_FLAG` (20%) between runs. Not the max-aggression 0.01 floor, which would run autovacuum continuously and steal write throughput from the fulfillment path.
- **`autovacuum_analyze_scale_factor = 0.02`** — refresh planner stats at 2% churn. Keeps `pg_stats` fresh so plans don't drift as UTMs / fulfillment status / addresses roll through the table.
- **`autovacuum_vacuum_threshold = 1000`** — floor. Prevents a small table from being chased; at `orders` size the scale_factor dominates, but the floor guards against edge cases (post-truncate, dev fixtures).

## The one-off `VACUUM (ANALYZE)`

The reloptions above stop the bloat from **recurring**, but existing dead-tuple backlog waits for the next natural pass to be reclaimed. The apply-script issues one `VACUUM (ANALYZE) public.orders` after the ALTER TABLE — clears the current backlog and refreshes stats immediately so the DB Health Agent's next bloat pass reads a fresh `last_autovacuum` + a sub-threshold `n_dead_tup / (n_live_tup + n_dead_tup)`.
...
- **`VACUUM` ≠ `VACUUM FULL`.** This recipe uses plain `VACUUM (ANALYZE)`, which is non-blocking (an `AccessShareLock`, coexists with reads and writes). `VACUUM FULL` rewrites the table and takes an `AccessExclusiveLock` — never in this recipe; it would freeze `orders` writes for the duration.
...
- **The tune is the guardrail, not a silver bullet.** If subscription-fulfillment churn genuinely grows past what 5% can absorb (rare — the table's steady-state churn is well under this), the DB Health Agent will re-flag `dbhealth:bloat:orders` after the 7d re-propose window, and the next tune tightens further (0.03 → 0.02) or the root cause is investigated (an untuned bulk-update batch, a runaway sync).
...
- [[../tables/orders]] — the target table (columns, indexes, prior DB Health fix history).
