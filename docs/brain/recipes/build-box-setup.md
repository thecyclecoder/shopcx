...
**Cap reset times.** When an account hits its wall the CLI message carries the reset clock time, **localized to the running env's timezone** — the box runs in **UTC** so it always reads `resets 7pm (UTC)` / `resets 2am (UTC)` (the same limit shows `resets 10pm (America/Puerto_Rico)` in a PR-timezone terminal). `parseResetTime` (in `builder-worker.ts`) reads that UTC hour → the account's `cappedUntil` is its REAL reset, not a flat now+5h guess. The box view (`/dashboard/roadmap/box`) renders the reset as an **AST clock time** (`America/Puerto_Rico`, the CEO's tz) — e.g. "resets 10:00 PM AST (~9h)". (Future-time display uses a `until()` helper, not `elapsed()` — `elapsed()` clamps a future time to "~0s".)
...
**Universal rollover (account-failover-everywhere, 2026-06-24).** The account pool is now **fundamental to every box `claude -p` run**, not just builds. Builds + the interactive dev-ask/coach (Ask-Ada) paths use `withAccountFailover` (in-call hop: on the 5-hour wall, mark the account capped + re-run on a healthy one, else park `blocked_on_usage`). Every other autonomous lane — repair, regression, security-review, ticket-improve (Improve), spec-test, migration-fix, pr-resolve, storefront-optimizer, triage (one account pinned per ticket — its revise resumes the solver session), product-seed, and the platform-director's own passes — calls `pickHealthyConfigDir()` to **start on a healthy account** instead of the capped default (RR1, where they used to just error); a rare mid-run cap re-runs on the next pass. **DB-driven health:** `restoreAccountCapsOnBoot()` restores each account's `cappedUntil` from the last heartbeat on boot, so a restart no longer resets accounts to healthy + re-probes the wall (the cap/failover-event spam after every restart is gone).
