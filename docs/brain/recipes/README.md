...
- [[db-vacuum-tune-ticket_messages]] — owner-approval-only per-table autovacuum tune + one-off `VACUUM (ANALYZE)` on `public.ticket_messages` (the DB Health Agent's `vacuum_tuning` fix for the `dbhealth:bloat:ticket_messages` signature, trend variant — no data is deleted; reloption math + rollback + verification)
