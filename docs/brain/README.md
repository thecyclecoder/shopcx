...
| [inngest/](inngest/) | One page per `src/lib/inngest/*.ts` — trigger event/cron, downstream events sent, tables read/written | 101 |
...
| [libraries/](libraries/) | One page per `src/lib/*.ts` — exports + signatures + callers + gotchas | 445 |
| [lifecycles/](lifecycles/) | Long-form narrative — end-to-end traces of key flows. Each wikilinks 5+ reference pages and ends with the src/lib files involved | 41 |
