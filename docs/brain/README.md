...
|---|---|---|
| [tables/](tables/) | One page per `public.*` table — columns, FKs (both directions), common queries, gotchas | 209 |
| [inngest/](inngest/) | One page per `src/lib/inngest/*.ts` — trigger event/cron, downstream events sent, tables read/written | 80 |
...
| [libraries/](libraries/) | One page per `src/lib/*.ts` — exports + signatures + callers + gotchas | 279 |
