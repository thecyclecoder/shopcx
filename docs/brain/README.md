...
|---|---|---|
| [tables/](tables/) | One page per `public.*` table — columns, FKs (both directions), common queries, gotchas | 180 |
| [inngest/](inngest/) | One page per `src/lib/inngest/*.ts` — trigger event/cron, downstream events sent, tables read/written | 67 |
| [integrations/](integrations/) | One page per external API — auth model, credential location, key endpoints, rate limits, retry pattern, gotchas | 20 |
