...
**⭐ Dual-persona session card — fused pre-merge only (`box-page-shows-both-avatars-for-ping-pong-ad-creative-sessions`).** A fused pre-merge `spec-test` lane (`kind='spec-test' && fused_pre_merge=true`) renders BOTH personas' avatars side-by-side + a dual title + a static sub-task label chip — Vera + Vault emit both verdicts off the SAME loaded diff in ONE session, so the collaboration is genuinely simultaneous (`fusedPreMergeInfo`). Every other kind renders a single avatar via `personaForKind(kind)`.
...
- The ping-pong is legible in the LABEL rather than the avatar: a `pingPongLabel(kind)` chip still names the pairing ("ad-creative ping-pong") next to the kind chip while the photo shows only the current actor. The fused-pre-merge lane keeps its dual render — that is a genuinely simultaneous two-agent session.
