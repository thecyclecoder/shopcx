...
**⭐ Dual-persona session cards (`box-page-shows-both-avatars-for-ping-pong-ad-creative-sessions`).** A session that has TWO collaborators under ONE parent job renders BOTH personas' avatars side-by-side (overlapping via `-space-x-2`) + a dual title + a static sub-task label chip, so the collaboration is visible at a glance and the card doesn't read as one long single-agent session. Today ONE flavor exists: an ad-creative-copy-author ping-pong (`kind='ad-creative-copy-author'`) — Dahlia (ad-creative) authors a creative and Max (ad-creative-copy-qc / growth grader) independently grades it, bouncing back and forth across the revise loop under ONE parent job (`pingPongInfo`). The `LaneCell` (in-flight) + `QueuedJobsLog` (queued) both compose `dual = pingPongInfo(kind)`; every other kind falls back to the single-avatar path via `personaForKind(kind)`. Pure display — no change to job/lane logic. **Historical note:** an earlier `fused_pre_merge` flavor rendered Vera + Vault together on a `spec-test` lane against a `claude/*` branch, but the fused pre-merge session was retired 2026-07-17 (see [[../libraries/spec-check-runner]] § graduate-vera) — a `spec-test` lane now shows Vera alone (her deterministic run) and the security review is Vault's own separate lane.
