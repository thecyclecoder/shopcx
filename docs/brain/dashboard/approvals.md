...
- **Each card answers "what is this, whose is it, where in the plan"** without a click-through — chips for the **Spec** (title/slug) · **Goal** · **Milestone** · **Phase needing approval**, a **type** label (from `agent_jobs.kind`), a **status** badge (Needs you / Awaiting / Approved / Declined / Escalated), an **autonomous** badge, and a `from {raiser} → routed to {approver}` / `decided by {Ada (autonomous)|You|Henry (CEO)}` line. Personas resolved via [[../libraries/agent-personas|personas.ts]] `getPersona`.
- **Filters** — a segmented control (**All activity · Needs you · Approved · Declined**, with live counts) + a text filter + Refresh.
...
- **Escalated = routed to CEO.** The "needs you" badge + filter count only pending requests routed to the fail-safe root (`routed_to_function='ceo'`/unrouted). Everything else is a log.
