...
## Migration target (Phase 2 preview — not built yet)

Phase 2 adds `src/lib/refund.ts` exporting `refundOrder(workspaceId, orderId, amountCents, reason, opts)` that mirrors the returns Inngest branch (row 1 above): read the order, if `braintree_transaction_id` is present call `refundBraintreeTransaction`, else call `partialRefundByAmount`. Phase 3 migrates every **raw** row above (5, 6, 7) — plus the **indirect** row 4 for the goodwill/immediate-refund case — onto that single dispatcher, so no callsite outside `src/lib/refund.ts` + `src/lib/shopify-order-actions.ts` + `src/lib/integrations/braintree.ts` can ever fire a refund mutation again.
