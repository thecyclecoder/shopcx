...
### Cold-scaler adset — 7-day minimum-age grace before ANY pause (CEO 2026-07-26)

A **cold-scaler** adset (the [[../libraries/cold-scaler-cohort|SCALING campaign]]'s children, minted by [[../specs/bianca-cold-scaler-graduate-crowned-winners-to-advantage-plus-new-customers]]) is on the **scaler rail**, not the test rail — it targets cold-prospecting audiences and **spends before it converts** as Meta ramps distribution. On day 1–6 a fresh scaler routinely shows $200–$500 spend with 0 in early revenue; that shape LOOKS like a test-dud on any spend-vs-purchases predicate but is the *normal* cold-audience ramp. Cutting it at day 1–6 destroys the ramp and forfeits the CAC:LTV verdict the scaler exists to reach.

**The rule ([[../specs/bianca-loser-kill-excludes-cold-scaler-adsets-plus-7day-grace]] Phase 2, CEO Dylan 2026-07-26):** any code path that would pause a cold-scaler adset MUST first check the adset's age (`meta_adsets.meta_created_time`, or live Meta `created_time`) and SKIP the pause when age < **`COLD_SCALER_MIN_AGE_DAYS_BEFORE_PAUSE` (7 days)**. The verdict window is the CAC:LTV-based sensor at week granularity ([[../libraries/media-buyer__cold-scaler-cac-ltv-sensor]]), not a leading-signal trim on day 3.

**Why 7 days:** matches the "7+ days AND ~8–10 purchases at target" bar for crowning a test winner ([[#the-decision-tree-ceo-dylan-2026-07-12--the-media-buyer-test-loop-verdict-bands|decision tree above]]) — a cold-scaler adset earns the same patient window before it can be judged (the direction is inverted for scaler adsets vs test adsets, but the temporal floor is the same). Enforced by [[../libraries/cold-scaler-cohort]] `isColdScalerAdsetOldEnoughToPause` — the belt-and-suspenders guard for any FUTURE scaler-pause consumer (graduate cleanup, owner action, future CAC:LTV-based scaler-pause loop). Today no built path pauses a scaler (Phase 1 removed the only one — Bianca's mis-scoped test decision-tree kill); this constant is the durable floor for the ones that come next.

**Live incident that motivated the rule:** Ashwavana Zen Relax scaler adset 120249611797950682 (created 2026-07-23) was paused twice on 2026-07-25 / 07-26 (age 2–3 days, $502 spend, 0 ROAS) by Bianca's test loser-kill scan before Phase 1 excluded scaler campaigns from it. Under the 7-day grace + Phase 1 exclusion, both cuts are impossible: Phase 1 removes the adset from the test loser candidate set outright; Phase 2 would refuse any pause-time predicate that reached the age check.
