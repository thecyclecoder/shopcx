...
- **A new cron / worker / agent-kind is incomplete without a Control Tower registry entry.** The [[specs/control-tower|Control Tower]] is the objective-owner's window from the north star — where we see whether each proxy-optimizing loop is still alive and doing its job, and which **pages** rather than silently trusting. Three silent failures bit us by luck (idle triage cron, a swallowed scorecard upsert, a stale escalated ticket); the fix is that every loop watches itself.
