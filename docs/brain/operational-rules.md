...
- **Every `needs_approval` is decided in the routed Agents-hub inbox (single entry point).** (Updated 2026-06-23 — [[specs/approval-routing-engine]] Phase 4 retired the scattered entry points.) The build-box paused callout (`src/app/dashboard/roadmap/box/page.tsx`) links a `needs_approval` job to `/dashboard/agents?view=inbox`, NOT a per-kind standalone surface; the Control Tower's repair / db-health / coverage feeds are monitoring-only and link into the same inbox. A new agent-proposal kind needs **no** approval-surface wiring — the box reconciler ([[libraries/approval-inbox]]) already surfaces every `needs_approval` in the inbox, routed up the org chart (else the CEO). Decide a simple approve/decline inline; a rich one in the inbox-launched modal; a kind whose decision genuinely can't be modal-ized (storefront hero preview) may deep-link to its canonical surface as a **documented exception** (logged), never a silent scatter. (The old box `approvalHref` 404-prone `/dashboard/roadmap/{slug}` fallthrough is gone; an inspect-only link for failed/needs_input jobs still defaults safely.)
