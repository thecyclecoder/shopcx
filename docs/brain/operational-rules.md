...
- **A new cron / worker / agent-kind / inline AI agent is incomplete without a Control Tower registry entry.** The [[specs/control-tower|Control Tower]] is the objective-owner's window from the north star — where we see whether each proxy-optimizing loop is still alive and doing its job, and which **pages** rather than silently trusting. Three silent failures bit us by luck (idle triage cron, a swallowed scorecard upsert, a stale escalated ticket); a fourth class — the inline AI agents (QC grader, journey delivery, fraud screen) that run server-side per ticket/order and could silently stop without any tile — was closed by [[specs/control-tower-agent-coverage]]. The fix is that every loop watches itself. **This covers any new server-side AI entry point** (a model call that acts on its own, without a human gating each run); a human-invoked route or a draft-only generator is exempt (log the reason in the [[libraries/control-tower]] audit table).
- **Shipping a new loop means, in the same PR:** (1) add a `MonitoredLoop` to `src/lib/control-tower/registry.ts` (`src/lib/control-tower/`, [[libraries/control-tower]]) with its cadence + liveness window (or per-kind stuck threshold; for an inline agent its `windowMs` + `workSignal` + error-rate config), and (2) **emit a heartbeat at the END of each run** — a cron via `emitCronHeartbeat()` in a `step.run("emit-heartbeat", …)` before its return; an agent-kind via the worker's `launch()` finally; an inline AI agent via `emitInlineAgentHeartbeat()` in a try/finally. The [[inngest/control-tower-monitor]] cron then evaluates it (liveness / cron-freshness / stuck-jobs / inline silent-while-work + error-rate) and opens a de-duped, owner-paging alert ([[tables/loop_alerts]]) on violation. Without the entry, the monitor can't see the loop and a silent failure goes unnoticed.
- **Assertions must not false-positive on a genuinely-idle loop** (no escalations to triage = green, not red; an inline AI agent with no work in its window = green). Liveness/freshness/stuck are the P1 checks; the inline silent-while-work + error-rate checks shipped in agent-coverage Phase 1; the per-decision orchestrator assertion is agent-coverage Phase 2.
