...
**Status is derived from children, never manually set.** A spec's planned/in_progress/shipped status is the rollup of its `spec_phases` (computed at read time); a milestone's is the rollup of its specs; a goal's `complete` is all-milestones-complete. The ONLY stored statuses are genuine *inputs*: a phase's build/ship state (`stampPhaseBuilt` records the `build_sha` when a phase commits on its `claude/build-{slug}` branch → `in_progress`; `stampPhaseShipped` flips it `shipped` only on the **main promotion** — Gate A whole-spec for a one-off spec, or the atomic Gate C for a goal-bound spec via `applyGoalPromotionEffects`; `markPhaseInProgress` on build start), a goal's greenlight (proposed→greenlit), and a spec's explicit lifecycle override (in_review / deferred / folded). Phases ACCUMULATE on the spec branch and ship ATOMICALLY — never one-phase-per-main-merge ([[lifecycles/spec-goal-branch-pm-flow]]). Never write a rollup status — it can only drift from the children. See [[specs/derive-rollup-status-retire-stored-columns]].
