...
- **Vale (spec-review)** — **RETIRED** ([[../specs/retire-vale-spec-review-becomes-deterministic-authoring-gate]] Phase 1–3). The Vale LLM lane is replaced by the DETERMINISTIC spec-review gate ([[../libraries/spec-review-gate]]) that runs synchronously at the authoring chokepoint. A malformed spec is rejected on the spot with the exact named defect (`Phase 1 appears twice`, `no **Owner:** line`, `Parent does not resolve`, `Blocked-by [[x]] does not resolve`, `customer_id table with no data-tool plan`, `Phase N has no ### Verification block`); a well-formed spec passes by construction. The gate is MONITORED infra ([[../libraries/control-tower/registry]] `MONITORED_LOOPS` `spec-review-gate` entry), not a graded worker — Ada supervises the invariant ("nothing malformed enters the pipeline") through the monitor + [[../libraries/spec-review-gate]]'s hard fail-at-write path; there is no LLM verdict to sweep.
...
- **CEO Control Tower — universal on/off switches + org-mirror monitoring** ✅ (folded → [[../lifecycles/control-tower]]) — one CEO-only kill-switch cascade (department → director → agent → tool) unified with the Control Tower as one monitoring-plus-control surface. M1–M6 shipped: the canonical [[../libraries/control-tower-node-registry|node registry]] (one org tree, drift reconciled), the [[../tables/kill_switches]] primitive + [[../libraries/kill-switch-resolver|cascade resolver]], switch enforcement at the `claim_agent_job` RPC + cron/tool chokepoints ([[../libraries/node-ancestry-sync]]), the persona-first three-tier CT with inline switch controls + the 5-min monitor-tick floor ([[../libraries/control-tower]] `assertRegistryInvariants`), the per-department [[../libraries/control-tower-infra-tab|Infra tab]] + [[../libraries/control-tower-self-audit|orphan-node self-audit]], and the north-star **node-completeness rule** (owner + switch + heartbeat or it's incomplete). Becomes the enforcer for every future node.
