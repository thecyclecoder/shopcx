...
## Verification
- Run the weekly compute for a workspace → `select metric_key, value, prior_value, delta_pct from platform_scorecard_snapshots where cadence='weekly' and snapshot_date=<this week>;` → one row per weekly KPI (`specs_per_week｜build_success_rate｜idea_to_merge_hours｜approvals_untouched_pct｜worker_grade_rollup｜regressions_caught`).
- `select detail from platform_scorecard_snapshots where metric_key='worker_grade_rollup' and cadence='weekly' …` → a JSON breakdown with one entry per graded `agent_kind` (cross-check against `computeAgentRollup`).
- Re-run the same week → row count unchanged (idempotent).
- Spot-check `build_success_rate` against `select status, count(*) from agent_jobs where kind='build' and updated_at >= <week start> group by 1;` → `value` = merged ÷ (merged + failed).
