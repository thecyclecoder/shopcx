...
## Verification
- On `/dashboard/agents`, view the Platform director → expect Tao/monitor, Devi/db_health, Cole/coverage-register listed as workers (invisible before this spec).
- On `/dashboard/agents/workers`, find Remi/regression → expect an **inactive** badge (never fired), while a beating cron/lane reads active.
- Run `npx tsx scripts/audit-agent-roster.ts` → expect "0 drift"; delete a rostered loop locally and re-run → expect it names that exact persona/lane mismatch.
