...
## Verification
- [ ] `pack_size` populated for all Active coffee ASINs; matches the validated table (1pk: B08KYMN52M/B0BV4WHWCX/B0BKR169VT/B0BLR2B936/B0FGHBP2QY · 2pk: B08C47SJ5B/B0BV4XY3L7/B0BLQRD681).
- [ ] Backfilled coffee non-renewal for Jun 7–20 ≈ $5.2k–$6.3k (≈32% of Amazon non-renewal).
- [ ] Conservation: Σ per-product = aggregate for every (date, bucket) backfilled.
- [ ] ROAS dashboard `/analytics/roas` overall ROAS unchanged before/after (reads `daily_amazon_order_snapshots`, untouched).
- [ ] Every new table/library/inngest change has a `docs/brain/` page in the same PR.
