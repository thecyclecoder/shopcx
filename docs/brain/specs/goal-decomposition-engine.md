...
## Build note — planning output commits to main (not a separate planning PR)
Phase 4 says "one PR for the planning output," but a queued `build` job reads its spec from `origin/main` (the box worker bases every build on `origin/main`). So a planning PR would race: builds queued before it merges can't find their specs. Resolved by **committing the approved specs + goal-doc update straight to main via the GitHub Contents API** — exactly the existing `chat/finalize` authoring path this spec told us to reuse (it already commits specs to main), and consistent with the billing model (authoring is cheap/docs-only). The approve-tree gate already gated *direction*; the per-spec **build PRs remain the code merge-gate**. Net: still two gates (approve-tree → merge each build PR), just no redundant docs-only PR in the middle.
