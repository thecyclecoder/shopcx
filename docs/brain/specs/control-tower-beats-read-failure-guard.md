...
**Shipped:** `buildControlTowerSnapshot` (monitor.ts) now captures the `control_tower_loop_beats` RPC `error` (`beatsError`) alongside `data`, and derives `beatsReadFailed = beatsError != null`. That flag is threaded into `evalCron`, which — when the read failed — treats the empty `everBeatCount`/null `latest` as UNKNOWN (not a true zero) and suppresses both the `never_fired` and `cron_freshness` reds, returning amber `"beat read unavailable — status unknown"` instead of paging. Same conservative posture as the existing `deployAgeMs==null` guard. Brain page [[../libraries/control-tower]] updated; `npx tsc --noEmit` clean.
