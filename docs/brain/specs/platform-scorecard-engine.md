...
## Verification
- Apply first: `npx tsx scripts/apply-platform-scorecard-snapshots-migration.ts` → expect `✓ table present: platform_scorecard_snapshots`. In the DB, `\d platform_scorecard_snapshots` shows the `(workspace_id, metric_key, cadence, snapshot_date)` unique + both indexes.
- Run the daily compute once for a workspace → `select metric_key, value, prior_value, delta_pct from platform_scorecard_snapshots where cadence='daily' and snapshot_date=current_date;` → one row per daily KPI (`loop_health｜error_backlog｜error_mttr_hours｜build_throughput｜autonomy_ratio｜escalations`).
- Re-run the same day → `select count(*) …` is unchanged (idempotent upsert, no duplicate rows).
- After a daily `platform-director-cron` tick → confirm the `snapshot-platform-scorecard` step ran (Inngest run history) and the rows above exist with no error.
