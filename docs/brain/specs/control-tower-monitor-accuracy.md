...
**Needs prod apply:** `npx tsx scripts/apply-control-tower-loop-beats-migration.ts` (creates the RPC). The new snapshot code calls the RPC, so the migration must land with/before the deploy.
