...
- ⏳ `instructions` JSON = `{thread_id, mode}` where `mode ∈ {turn, propose_spec, approve_action}`. `spec_slug` carries the thread id (mirrors spec-chat's reuse of the column as a thread handle).
...
## Verification
- **Sidebar + route:** `src/app/dashboard/sidebar.tsx` contains a "Message Center" entry under the Developer section pointing at `/dashboard/developer/messages`; the page loads for an owner and 404s/blocks for non-owners.
- **Job plumbing:** `agent_jobs.kind='dev-ask'` is claimable via `claim_agent_job(['dev-ask'])` on a concurrency-1 lane; probe `agent_jobs` for a completed `dev-ask` row after one turn (`status='completed'`, `claude_session_id` set).
- **Round-trip turn:** from the UI, send a question; within the poll window the thread's `turn_status` goes `thinking → idle` and an assistant message appears; the box session id is persisted on the thread row for resume.
- **Analytics correctness:** ask a countable DB question whose answer can be cross-checked with a direct read-only probe (`probe-db`); the two match. Confirm via worktree inspection that the query ran as an **uncommitted** `scripts/_*.ts` and no `docs/brain/`/`src/` files were modified.
- **Read-only enforcement:** confirm `git status` in the thread worktree shows no committed changes after a pure-investigation turn; no product files touched.
- **Approval gate:** prompt it toward a mutation; verify it emits a `pending_actions` card and the target table is unchanged in the DB until Approve is clicked, after which the change is visible and was executed by worker code (not mid-session).
- **Spec handoff:** trigger a gap → spec; confirm a `docs/brain/specs/{slug}.md` lands on main with a valid `**Owner:**`/`**Parent:**` line, and (if Send & build) a `kind='build'` `agent_jobs` row is queued.
- **Max billing:** worker logs/heartbeat show the `dev-ask` session launched with `ANTHROPIC_API_KEY` unset (no API spend); brain DB-query reference page exists; `npx tsc --noEmit` is clean.
