...
- **Cross-account sessions DON'T resume (learned 2026-06-22):** a `claude --resume <session-id>` only works under the **same** config dir that created it — switching accounts mid-job orphaned sessions ("No conversation found with session ID"). So: a **resume must stick to the account that owns its session**; if that account is capped, the job **starts fresh** (clear `claude_session_id`) on another account rather than failing. Round-robin assignment is therefore **per-new-build**, not per-step within a build.
...
- A resume whose session-owning account is capped → **starts fresh** on a healthy account (clears `claude_session_id`), not a "No conversation found" failure.
...
## Phase 1 — round-robin assignment + usage-cap rotation in the worker ⏳
Account pool + least-recently-used/round-robin `CLAUDE_CONFIG_DIR` selection per new build in the build env; usage-cap detection (vs 529-transient) pulls an account from rotation; resume sticks to its session's account or starts fresh; `blocked_on_usage` auto-resuming state when all capped. Brain: [[../recipes/build-the-box]] (if present) · [[../operational-rules]] · [[control-tower]].
