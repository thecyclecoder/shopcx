...
**Deferred:** split from [[pdp-refinement-pass]] — not needed now: the parent's promise was a *repeatable refinement workflow*, which shipped in P1 (global code/pipeline, tsc-clean, folded into [[../lifecycles/product-intelligence]]) and is proven on Superfood Tabs in P2. Running the same founder-verification-gated pass on the remaining products is operational fan-out — no spec or goal depends on it landing now, and the parent (a perpetual CMO mandate) carries no deadline.
