...
- **Phase 2, `shipping_protection_convert` validated on the real sub (audit `4b831caa` = subscription `e4589de9`):** applied via [[migration-fix-remove-line]]'s `scripts/fix-e4589de9-remove-line-protection.ts --apply` (the sub also needed a free-line removal, so the two fixes were applied together). `shipping_protection_convert {amount_cents: 395, baseline_cents: 5996}` → `applyMigrationFix` ok → `verifyMigration` `status='passed'` (`pricing_preserved` ✅); sub row `shipping_protection_added=true`, `shipping_protection_amount_cents=395`, no "Shipping Protection" line in `items[]`, **Superfood Tabs still at 5996 (override unchanged)**; audit baseline 6391¢ → 5996¢; row cleared from `/dashboard/migrations`; renews at 5996 + 395 = **6391**. (The early standalone `fix-4b831caa-shipping-protection.ts` one-off was buggy — it matched on the *audit* id as a `subscription_id` prefix and used a stale $3.75 — and has been removed; the real fix went through the `e4589de9` script above.)
