...
- **Phase 1** — `src/lib/migrate-to-internal.ts`: `findBillableCustomer` now returns `{ customerId, paymentMethodId }` (the billable member's default `customer_payment_methods` row id), and the flip sets `subscriptions.payment_method_id` to it. Mirrors the recovery pin in `src/lib/portal/handlers/payment-method-update.ts`. Default-card fallback unchanged.
- **Phase 2** — `appstleLinesToInternalItems` resolves each line by `shopify_variant_id` then `sku`; an unmappable line is dropped and returned as a `DroppedLine`. The migration writes a `dropped_unmappable_items` note to `migration_audits.notes` (new `jsonb` column — `supabase/migrations/20260623120000_migration_audits_notes.sql`, apply via `scripts/apply-migration-audits-notes-migration.ts`) and fires a **critical** `notifyOpsAlert` for any *paid* drop. `recordMigrationAudit` (`src/lib/migration-audit.ts`) accepts `droppedLines`.

## Verification

> **Prereq:** apply `20260623120000_migration_audits_notes.sql` (adds `migration_audits.notes`) before exercising Phase 2.

- **Pin (Phase 1).** Migrate a sub for a customer with a default Braintree card (add a default card in the portal → strangler migration runs) → `subscriptions.payment_method_id` for the flipped sub equals that customer's default `customer_payment_methods.id` (not null); the portal sub-detail shows the pinned card; the next renewal charges it explicitly.
- **No-card no-regression (Phase 1).** Migrate path for a customer with no card → the sub is skipped by the billable rule (`payment_method_id` never set); a sub that bills via the link-group default still renews via the fallback (unchanged).
- **Map-by-sku (Phase 2).** Migrate a sub whose live Appstle line's `shopify_variant_id` isn't synced but whose `sku` matches an internal `product_variants.sku` → the migrated `items[]` line carries the internal UUID `variant_id` + `product_id` + `sku`; `migration_audits` `items_on_uuids` passes.
- **$0 unmappable drop (Therese's case, Phase 2).** Migrate a sub with a $0 line that matches no variant by id or sku → the line is absent from `items[]`; `migration_audits.notes` carries a `dropped_unmappable_items` entry for it; `items_on_uuids` passes and the audit clears (no infinite retry).
- **Paid unmappable escalation (Phase 2).** Migrate a sub with a paid line that matches no variant → the line is dropped, the note records it with `paid: true`, AND a critical `#alerts-critical` Slack alert names the sub + dropped line so a human re-adds the variant or refunds.
