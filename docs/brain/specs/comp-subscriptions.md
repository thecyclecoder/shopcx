...
> **Fold note (comp-transaction-type-constraint, 2026-06-22):** both `type='comp'` ledger inserts above were silently violating `transactions_type_check` — the inline `CHECK` from `20260520160000_transactions.sql` only admitted the original four types, and neither insert checked its returned error, so the rows never landed. Fixed in [[comp-transaction-type-constraint]]: migration `20260622170000_comp_transaction_type.sql` extends the CHECK to include `'comp'`, and both inserts in `internal-subscription-renewals.ts` (`comp-gate-failed-transaction`, `comp-transaction`) now `throw` on `error` so a future constraint break is loud, not swallowed. The fail-closed `failed_comp_transaction` row now actually lands. See [[../lifecycles/subscription-billing.md]] · [[../tables/transactions]].
