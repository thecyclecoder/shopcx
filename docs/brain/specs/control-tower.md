...
- **Renewal integrity (the highest-value assertion — build this well):** when an internal sub is **due**, it must renew correctly or we get paged. A silently-missed renewal = lost revenue + quiet churn, invisible today.
  - **Due-but-not-renewed:** an `is_internal` + `status='active'` sub with `next_billing_date <= now` that, past a grace window (the daily renewal cycle + a few hours), has **not advanced `next_billing_date`** and has **no renewal order** for this cycle — and is **not** in a known-handled state (open **dunning**, `paused`/`cancelled`, or a fail-closed comp) → alert "N due subs didn't renew: {ids}". The core signal.
  - **Outcome distribution:** `internal-subscription-renewal-cron` emits per-cycle outcome counts (`charged / skipped:no_payment_method / skipped:zero_total / declined→dunning / comp_shipped / error`); a **spike** in skips/declines/errors vs baseline → alert ("renewal decline rate {x}% / {n} no-PM skips") — catches a systemic break (e.g. bad Braintree creds declining everyone) even when each decline individually "routed to dunning".
  - **Stuck dunning:** a sub in dunning past its retry schedule with no resolution → surface.
  - A sub correctly in dunning is **not** flagged as missed; a healthy cycle → green tile with the outcome breakdown.
