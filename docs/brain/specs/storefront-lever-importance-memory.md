...
## Verification
- Run `npx tsx scripts/apply-storefront-lever-memory-migration.ts` → expect `✓ public.storefront_levers has 12 columns`, `✓ public.storefront_lever_importance has 15 columns`, and a `✓ seeded N levers (top: …)` line with `N ≥ 25` (the 9 chapter + 12 component + 3 renewal-offer migration baseline; live can exceed if hand-tuned). In Supabase confirm the self-FK `parent_lever_id` (`storefront_levers_parent_idx`) and the unique index `storefront_lever_importance_cell_uniq` on `(lever_id, product_id, lander_type, audience)`.
- Probe taxonomy SHAPE (literal-prior values legitimately drift once `seedChapterPriorsFromFunnel({apply:true})` reseeds chapters from live funnel data — a designed Phase 1 behavior — so this assertion is shape-only): `select chapter, lever_key, prior from storefront_levers where level='chapter' order by prior desc limit 5;` → `hero` is row #1; `pricing_table` is in the top 3; `persist_to_renewal_offer` is present in the result set (shipped after the [[storefront-renewal-offer-lever|renewal-offer]] migration). `select count(*) from storefront_levers where parent_lever_id is not null;` → > 0 (components hang off their chapter). `select count(*) from storefront_levers;` → ≥ 25.
