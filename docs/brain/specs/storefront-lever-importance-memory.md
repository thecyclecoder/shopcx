...
## Verification
- Run `npx tsx scripts/apply-storefront-lever-memory-migration.ts` → expect `✓ public.storefront_levers has 12 columns`, `✓ public.storefront_lever_importance has 15 columns`, and `✓ seeded 22 levers (top: hero=0.9, …)`. In Supabase confirm the self-FK `parent_lever_id` (`storefront_levers_parent_idx`) and the unique index `storefront_lever_importance_cell_uniq` on `(lever_id, product_id, lander_type, audience)`.
- `select chapter, lever_key, prior from storefront_levers order by prior desc;` → expect `hero` (0.90) top, `pricing_table` (0.78) #2, `social_proof` (0.62) #3 — the funnel-informed CRO ranking; components (`image` 0.62, `headline` 0.58, …) hang off their chapter via `parent_lever_id`.
...
- On `/dashboard/storefront/funnel` → expect a "What the agent believes matters" panel listing levers with current `importance`, the `vs prior` delta, scope, tests, and last-tested age.
