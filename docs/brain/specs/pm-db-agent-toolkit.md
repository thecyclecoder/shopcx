...
## Safety / invariants
- **Rollup status is DERIVED, never stored.** A spec's status = `rollup(its phases)`; a milestone's = `rollup(its specs)`; a goal's *complete* = all-milestones-complete. The ONLY stored statuses are genuine *inputs*: a phase's merge state (the leaf source of truth, set when its PR merges) and a goal's *greenlight* decision (proposed→greenlit — a CEO input, not derivable). The toolkit's readers (`getSpec`/`listSpecs`/`getMilestone`/`getGoal`) **compute** the rollup; they never trust a stored status column. Storing a derivable status is the exact drift source we spent 2026-06-25 reconciling table-by-table ("spec says in_progress but all phases shipped") — derivation makes that drift class *structurally impossible*, where a sync trigger only makes it *less likely*. The stored `specs.status` / `goal_milestones.status` columns are demoted to a non-authoritative cache and dropped by [[derive-rollup-status-retire-stored-columns]] (expand-contract) once every reader derives.
