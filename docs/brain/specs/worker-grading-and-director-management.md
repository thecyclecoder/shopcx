...
## Phases
- **P1 — grading store + grader ✅** — migration `worker_action_grades` (mirror [[../tables/director_decision_grades]]) + `worker_grader_prompts` (CEO-calibratable rubric, mirror `director_grader_prompts`); `src/lib/agents/worker-grader.ts` (`gradeConcludedWorkerActions` batched, `computeWorkerRollup` last-10, `detectGradeDropCoaching` >1.5-drop-or-<7 → `coachWorker`). Brain: [[../tables/worker_action_grades]] · [[../tables/worker_grader_prompts]] · [[../libraries/worker-grader]]. _(Migration authored, not yet applied — needs prod approval.)_
