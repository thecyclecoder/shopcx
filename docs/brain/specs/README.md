...
- ⏳ **Homepage rebuild (direct-response, Tabs-led)** — [[homepage-rebuild]]. Rebuild the Shopify homepage as a trust-and-routing engine for ad-aware brand searchers + repeat reorderers. Hero = Superfood Tabs; full-catalog merchandising incl. non-advertised Ashwavana Zen Relax + Creatine Prime+; 30-day MBG, third-party tested, ABC/CBS/NBC/FOX press bar (logos added as theme assets). Images auto-sourced (product featuredImages + Files lifestyle) — zero uploads. Built via the theme pipeline ([[shopify-theme-via-shopcx]]); staging mechanism TBD (no API theme-duplication).

---

**Cross-cutting (storefront × ad builder):**
- 🚧 **Ad & Lander Quality Scorecard** — [[ad-lander-scorecard]]. **P1 + P2 shipped 2026-06-17** → [[../dashboard/storefront__ad-scorecard]]. Groups `storefront_sessions` by `utm_campaign` (= ad creative), `utm_content` (= Meta ad id), and `landing_url` variant/angle to rank which **ads send the most engaged/ATC/lead/purchasing traffic** and which **lander variants convert it best** — the feedback instrument for [[killer-statics]] + [[advertorial-landers]]. Built entirely on existing data (sessions + events + leads + orders). Attribution: engagement per-session, ad purchases via `orders.attributed_utm_campaign` (first-touch), lander purchases session-scoped, internal/bot excluded, min-volume gate. Meta spend/ROAS (via Marketing API keyed on `meta_ad_id`) parked as future roadmap — needs a Meta insights read integration we don't have yet.
