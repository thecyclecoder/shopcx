...
- ✅ **Shopify theme management via ShopCX (AI-driven, short-term)** — **verified + archived 2026-06-18** ([[../archive]]). Chat→build theme edits ship to the live store via GitHub commits (Option A); Shopify's GitHub integration auto-deploys. Reconciliation run: 32 genuinely-drifted files committed to catch the repo up to live (98 JSON files correctly skipped as serialization-only); re-run shows 0 diff. Folded → [[../libraries/shopify-theme]] (`src/lib/shopify-theme.ts` + `scripts/reconcile-shopify-theme.ts`), [[../recipes/edit-shopify-theme]] (workflow + single-writer guardrail), [[../integrations/shopify]] § Theme management.
- ⏳ **Homepage rebuild (direct-response, Tabs-led)** — [[homepage-rebuild]]. Rebuild the Shopify homepage as a trust-and-routing engine for ad-aware brand searchers + repeat reorderers. Hero = Superfood Tabs; full-catalog merchandising incl. non-advertised Ashwavana Zen Relax + Creatine Prime+; 30-day MBG, third-party tested, ABC/CBS/NBC/FOX press bar (logos added as theme assets). Images auto-sourced (product featuredImages + Files lifestyle) — zero uploads. Built via the theme pipeline ([[../recipes/edit-shopify-theme]]); staging mechanism TBD (no API theme-duplication).
