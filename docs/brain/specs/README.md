...
**Open sub-work:**
- ⏳ **Portal account handoff + login chat + Help Center** — [[portal-account-handoff-and-help-center]]. Makes `portal.superfoodscompany.com` the single account destination. Shopify theme account slide-out → portal, auto-authenticating logged-in Shopify customers via a new App-Proxy SSO endpoint (`/api/portal/sso` mints a magic-link token; Liquid `{% if customer %}` does the login branch, no extra round-trip); drawer redesigned to one CTA + a "what you can do" showcase; live-chat widget on the portal **login page only** (login-help for the locked-out); new searchable **Help Center** sidebar item surfacing all KB articles in-app. Spec'd 2026-06-17 — build via worktree `/goal`.
