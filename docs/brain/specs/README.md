...
**Open sub-work:**
- ⏳ **Portal: finish "Resources" sidebar** — spec'd: [[blog-resources]]. Import the 36 Superfood Scoop blog articles into a `posts` object, AI-classify them into product resources (+ which products + grouping), migrate images off Shopify, and surface them in Resources with search + product→grouping navigation. Ready to build + run via workflow.
