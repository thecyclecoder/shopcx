...
- **Sonnet returns trigger_intent instead of journey name**: The action executor looks up handlers by name AND trigger_intent (case-insensitive). Sonnet may return "cancel_subscription" (trigger_intent) or "Cancel Subscription" (name) — both work. Same applies to playbooks and workflows.
- **Multi-step responses stored as `{"undefined": ...}`**: The discount builder creates steps with `key: "consent"` but the mini-site's response storage used `form.id` which is undefined. Always use `form.id || form.key` when referencing a step's identifier. This silently broke marketing subscriptions, coupon applications, and action logging — everything appeared to work (coupon email sent) but no actual actions executed.
